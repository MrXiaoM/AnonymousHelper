 ...
 * <p>
 *
 * The below code sample illustrates how to set, examine, and get the flags for
 * a message.
 * 
 * <pre>
 *
...
public class Flags implements Cloneable, Serializable {
	...
	public static final class Flag {
		/**
		 * This message has been answered. This flag is set by clients to indicate that
		 * this message has been answered to.
		 */
		public static final Flag ANSWERED = new Flag(ANSWERED_BIT);

		/**
		 * This message is marked deleted. Clients set this flag to mark a message as
		 * deleted. The expunge operation on a folder removes all messages in that
		...
		/**
		 * This message is flagged. No semantic is defined for this flag. Clients alter
		 * this flag.
		 */
		public static final Flag FLAGGED = new Flag(FLAGGED_BIT);

		/**
		 * This message is recent. Folder implementations set this flag to indicate that
		 * this message is new to this folder, that is, it has arrived since the last
		 * time this folder was opened.
		 * <p>
		 *
		 ...
		 * this Message's content is returned to the client in some form. The
		 * <code>getInputStream</code> and <code>getContent</code> methods on Message
		 ...
		 * <p>
		 *
		 * The implementation sets this flag. Clients cannot alter this flag but can use
		 * it to determine if a folder supports user defined flags by using
	...
	/**
	 * Remove any flags <strong>not</strong> in the given Flags object. Useful for
	 * clearing flags not supported by a server. If the given Flags object includes
	 * the Flags.Flag.USER flag, all user flags in this Flags object are retained.
	 *
	...
	/**
	 * Check whether all the flags in the specified Flags object are present in this
	 * Flags object.
	 *
	...
	/**
	 * Return all the user flags in this Flags object. Returns an array of size zero
	 * if no flags are set.
	 *
	...
	/**
	 * Return a string representation of this Flags object. Note that the exact
	 * format of the string is subject to change.
	 */
	 ...
	 * 
	 * // check copy constructor with only system flags Flags fc = new Flags(f1); if
	 * (f1.equals(fc) && fc.equals(f1)) System.out.println("success"); else
	 * System.out.println("fail");
	 * 
	 * // check clone with only system flags fc = (Flags)f1.clone(); if
	 * (f1.equals(fc) && fc.equals(f1)) System.out.println("success"); else
	 * System.out.println("fail");
	 * 
	 * // add a user flag and make sure it still works right f1.add("MyFlag");
	 * 
	 * // shouldn't be equal here if (!f1.equals(fc) && !fc.equals(f1))
	 * System.out.println("success"); else System.out.println("fail");
	 * 
	 * // check clone fc = (Flags)f1.clone(); if (f1.equals(fc) && fc.equals(f1))
	 * System.out.println("success"); else System.out.println("fail");
	 * 
	 * // make sure user flag hash tables are separate fc.add("AnotherFlag"); if
	 * (!f1.equals(fc) && !fc.equals(f1)) System.out.println("success"); else
	 * System.out.println("fail");
	 * 
	 * // check copy constructor fc = new Flags(f1); if (f1.equals(fc) &&
	 * fc.equals(f1)) System.out.println("success"); else
	 * System.out.println("fail");
	 * 
	 * // another new flags object Flags f2 = new Flags(Flags.Flag.ANSWERED);
	 * f2.add("MyFlag");
	 * 
	 * if (f1.contains(Flags.Flag.DELETED)) System.out.println("success"); else
	 * System.out.println("fail");
	 * 
	 * if (f1.contains(Flags.Flag.SEEN)) System.out.println("success"); else
	 * System.out.println("fail");
	 * 
	 * if (f1.contains(Flags.Flag.RECENT)) System.out.println("success"); else
	 * System.out.println("fail");
	 * 
	 * if (f1.contains("MyFlag")) System.out.println("success"); else
	 * System.out.println("fail");
	 * 
	 * if (f2.contains(Flags.Flag.ANSWERED)) System.out.println("success"); else
	 * System.out.println("fail");
	 * 
	 * 
	 * System.out.println("----------------");
	 * 
	 * String[] s = f1.getUserFlags(); for (int i = 0; i < s.length; i++)
	 * System.out.println(s[i]); System.out.println("----------------"); s =
	 * f2.getUserFlags(); for (int i = 0; i < s.length; i++)
	 * System.out.println(s[i]);
	 * 
	 * System.out.println("----------------");
	 * 
	 * if (f1.contains(f2)) // this should be true System.out.println("success");
	 * else System.out.println("fail");
	 * 
	 * if (!f2.contains(f1)) // this should be false System.out.println("success");
	 * else System.out.println("fail");
	 * 
	 * Flags f3 = new Flags(); f3.add(Flags.Flag.DELETED); f3.add(Flags.Flag.SEEN);
	 * f3.add(Flags.Flag.RECENT); f3.add(Flags.Flag.ANSWERED);
	 * f3.add("ANOTHERFLAG"); f3.add("MYFLAG");
	 * 
	 * f1.add("AnotherFlag");
	 * 
	 * if (f1.equals(f3)) System.out.println("equals success"); else
	 * System.out.println("fail"); if (f3.equals(f1)) System.out.println("equals
	 * success"); else System.out.println("fail"); System.out.println("f1 hash code
