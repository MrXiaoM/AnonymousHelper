...
public class URLName {
	...
	/**
	 * Construct a URLName from the string. Parses out all the possible information
	 * (protocol, host, port, file, username, password).
	 *
	...
	/**
	 * Compares two URLNames. The result is true if and only if the argument is not
	 * null and is a URLName object that represents the same URLName as this object.
	 * Two URLName objects are equal if they have the same protocol and the same
	 * host, the same port number on the host, the same username, and the same file
	 * on the host. The fields (host, username, file) are also considered the same
	 ...
	 * protocols, so "imap://host" and "imap://host:143" would not compare as equal.
	 * <p>
	 *
	...
	public boolean equals(Object obj) {
		...
		// compare protocols
		if (!(protocol == u2.protocol || (protocol != null && protocol.equals(u2.protocol))))
			return false;
	...
	/**
	 * Get the IP address of our host. Look up the name the first time and remember
	 * that we've done so, whether the lookup fails or not.
	 */
	 ...
	 * "protocol://userid:password@host:119/file", "http://funny/folder/file.html",
	 * "http://funny/folder/file.html#ref", "http://funny/folder/file.html#",
	 * "http://funny/#ref", "imap://jmr:secret@labyrinth//var/mail/jmr",
	 ...
	 * 
	 * URLName url = new URLName("protocol", "host", 119, "file", "userid",
	 * "password"); System.out.println("Test URL: " + url.toString()); if
	 * (argv.length == 0) { for (int i = 0; i < testURLNames.length; i++) {
	 * print(testURLNames[i]); System.out.println(); } } else { for (int i = 0; i <
	 * argv.length; i++) { print(argv[i]); System.out.println(); } if (argv.length
	 * == 2) { URLName u1 = new URLName(argv[0]); URLName u2 = new URLName(argv[1]);
	 ...
	 * 
	 * private static void print(String name) { URLName url = new URLName(name);
