...
public abstract class Transport extends Service {
	...
	/**
	 * Send a message. The message will be sent to all recipient addresses specified
	 * in the message (as returned from the <code>Message</code> method
	 * <code>getAllRecipients</code>), using message transports appropriate to each
	 * address. The <code>send</code> method calls the <code>saveChanges</code>
	 * method on the message before sending it.
	 * <p>
	 *
	 * If any of the recipient addresses is detected to be invalid by the Transport
	 * during message submission, a SendFailedException is thrown. Clients can get
	 * more detail about the failure by examining the exception. Whether or not the
	 * message is still sent successfully to any valid addresses depends on the
	 * Transport implementation. See SendFailedException for more details. Note also
	 * that success does not imply that the message was delivered to the ultimate
	 * recipient, as failures may occur in later stages of delivery. Once a
	 * Transport accepts a message for delivery to a recipient, failures that occur
	 * later should be reported to the user via another mechanism, such as returning
	 * the undeliverable message.
	 * <p>
	 *
	 * In typical usage, a SendFailedException reflects an error detected by the
	 * server. The details of the SendFailedException will usually contain the error
	 * message from the server (such as an SMTP error message). An address may be
	 * detected as invalid for a variety of reasons - the address may not exist, the
	 * address may have invalid syntax, the address may have exceeded its quota,
	 * etc.
	 * <p>
	 *
	 * Note that <code>send</code> is a static method that creates and manages its
	 * own connection. Any connection associated with any Transport instance used to
	 * invoke this method is ignored and not used. This method should only be
	 * invoked using the form <code>Transport.send(msg);</code>, and should never be
	 * invoked using an instance variable.
	 *
	 * @param msg the message to send
	 * @exception SendFailedException if the message could not be sent to some or
	 ...
	 * Send a message. The message will be sent to all recipient addresses specified
	 * in the message (as returned from the <code>Message</code> method
	 * <code>getAllRecipients</code>). The <code>send</code> method calls the
	 * <code>saveChanges</code> method on the message before sending it.
	 * <p>
	 *
	...
	private static void send0(Message msg, Address[] addresses, String user, String password)
		...
		/*
		 * protocols is a map containing the addresses indexed by address type
		 */
	 ...
	 * TransportListener registered on this Transport. Also, if any of the addresses
	 * is invalid, a SendFailedException is thrown. Whether or not the message is
	 * still sent succesfully to any valid addresses depends on the Transport
	 * implementation.
	 * <p>
	 *
	...
	/**
	 * Notify all TransportListeners. Transport implementations are expected to use
	 * this method to broadcast TransportEvents.
	 * <p>
	 *
