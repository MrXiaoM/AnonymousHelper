...
public class InternetHeaders {
	private static final boolean ignoreWhitespaceLines = PropUtil
			.getBooleanSystemProperty("mail.mime.ignorewhitespacelines", false);
	...
	protected static final class InternetHeader extends Header {
		/*
		 * Note that the value field from the superclass isn't used in this class. We
		 * extract the value from the line field as needed. We store the line rather
		 * than just the value to ensure that we can get back the exact original line,
		 * with the original whitespace, etc.
		 */
	...
	static class MatchEnum {
		...
		/*
		 * Constructor. Initialize the enumeration for the entire List of headers, the
		 * set of headers, whether to return matching or non-matching headers, and
		 * whether to return header lines or Header objects.
		 */
	 ...
	 * preferred order of headers. Headers are never actually removed from the list,
	 * they're converted into placeholder entries. New headers are added after
	 * existing headers of the same name (or before in the case of
	 ...
	 * the header from the body. The input stream is left positioned at the start of
	 * the body. The header lines are stored internally.
	 * <p>
	 *
	 * For efficiency, wrap a BufferedInputStream around the actual input stream and
	...
	/**
	 * Read and parse the given RFC822 message stream till the blank line separating
	 * the header from the body. Store the header lines inside this InternetHeaders
	 * object. The order of header lines is preserved.
	 * <p>
	 *
	 * Note that the header lines are added into this InternetHeaders object, so any
	 * existing headers in this object will not be affected. Headers are added to
	 * the end of the existing list of headers, in order.
	 *
	 * @param is RFC822 input stream
	 * @exception MessagingException for any I/O error reading the stream
	 */
	 ...
	 * <p>
	 *
	 * The current implementation knows about the preferred order of most well-known
	 * headers and will insert headers in that order. In addition, it knows that
	 * <code>Received</code> headers should be inserted in reverse order (newest
	 * before oldest), and that they should appear at the beginning of the headers,
	 * preceeded only by a possible <code>Return-Path</code> header.
	 * <p>
	 *
	 * Note that RFC822 headers can only contain US-ASCII characters.
	 *
