 ...
 * <a href="http://www.ietf.org/rfc/rfc2822.txt" target="_top">RFC 2822</a>.
 * <p>
 *
 ...
 * syntax for a full date and time specification. Though folding white space is
 * permitted throughout the date-time specification, it is RECOMMENDED that a
 * single space be used in each place that FWS appears (whether it is required
 * or optional); some older implementations may not interpret other occurrences
 * of folding white space correctly.
 * 
 * <pre>
 ...
 * </pre>
 * 
 * The day is the numeric day of the month. The year is any numeric year 1900 or
 * later.
 * <p>
 ...
 * represent. The "+" or "-" indicates whether the time-of-day is ahead of
 * (i.e., east of) or behind (i.e., west of) Universal Time. The first two
 * digits indicate the number of hours difference from Universal Time, and the
 * last two digits indicate the number of minutes difference from Universal
 * Time. (Hence, +hhmm means +(hh * 60 + mm) minutes, and -hhmm means -(hh * 60
 * + mm) minutes). The form "+0000" SHOULD be used to indicate a time zone at
 ...
 * zone other than Universal Time and therefore indicates that the date-time
 ...
 * numeric day-of-month MUST be between 1 and the number of days allowed for the
 ...
 * see [STD12]), and the zone MUST be within the range -9959 through +9959.
 *
...
public class MailDateFormat extends SimpleDateFormat {
	...
	/**
	 * Formats the given date in the format specified by RFC 2822 in the current
	 * TimeZone.
	 *
	 ...
	 * <li>RFC 2822 specifies that a zone of "-0000" indicates that the date-time
	 * contains no information about the local time zone. This class uses the UTC
	...
	public Date parse(String text, ParsePosition pos) {
		...
		return isLenient() ? new Rfc2822LenientParser(text, pos).parse() : new Rfc2822StrictParser(text, pos).parse();
	}

	/**
	 * This method always throws an UnsupportedOperationException and should not be
	 * used because RFC 2822 mandates a specific calendar.
	 *
	...
	/**
	 * This method always throws an UnsupportedOperationException and should not be
	 * used because RFC 2822 mandates a specific number format.
	 *
	 * @throws UnsupportedOperationException if this method is invoked
	 */
	@Override
	...
	/**
	 * This method always throws an UnsupportedOperationException and should not be
	 * used because RFC 2822 mandates a specific pattern.
	 *
	 * @throws UnsupportedOperationException if this method is invoked
	 * @since JavaMail 1.6
	 */
	...
	/**
	 * This method always throws an UnsupportedOperationException and should not be
	 * used because RFC 2822 mandates another strategy for interpreting 2-digits
	 * years.
	 *
	...
	/**
	 * This method always throws an UnsupportedOperationException and should not be
	 * used because RFC 2822 mandates specific date format symbols.
	 *
	 * @throws UnsupportedOperationException if this method is invoked
	 * @since JavaMail 1.6
	 */
	 ...
	 * @throws IllegalArgumentException if this instance's Calendar is non-lenient
	 *                                  and any of the parameters have invalid
	 *                                  values, or if dayName is not consistent with
	 *                                  day-month-year
	 */
	 ...
	 * <ul>
	 * <li>no exceptions are thrown, except for java.text.ParseException from parse*
	 * methods
	 * <li>when parse* throws ParseException OR get* returns INVALID_CHAR OR skip*
	 * returns false OR peek* is invoked, then pos.getIndex() on method exit is the
	 * same as it was on method entry
	 * </ul>
	 */
	private static abstract class AbstractDateParser {
		...
		/**
		 * @return the number of minutes to be added to the time in the local time zone,
		 *         in order to obtain the equivalent time in the UTC time zone. Returns
		 *         0 if the date-time contains no information about the local time zone.
		 */
