 ...
 * <a href="http://www.ietf.org/rfc/rfc822.txt" target="_top">RFC822</a>.
...
public class InternetAddress extends Address implements Cloneable {
	 ...
	 * <code>InternetAddress(String address, boolean strict)</code> constructor does
	 * when <code>strict</code> is <code>true</code>. This constructor is equivalent
	 * to <code>InternetAddress(address, false)</code>.
	 *
	...
	/**
	 * Parse the given string and create an InternetAddress. If <code>strict</code>
	...
	/**
	 * Construct an InternetAddress given the address and personal name. The address
	 * is assumed to be a syntactically valid RFC822 address.
	 *
	 * @param address  the address in RFC822 format
	 * @param personal the personal name
	 * @exception UnsupportedEncodingException if the personal name can't be encoded
	...
	/**
	 * Set the personal name. If the name contains non US-ASCII characters, then the
	 * name will be encoded using the specified charset as per RFC 2047. If the name
	 * contains only US-ASCII characters, no encoding is done and the name is used
	 * as is.
	 * <p>
	 *
	 * @param name    personal name
	 * @param charset MIME charset to be used to encode the name as per RFC 2047
	...
	/**
	 * Set the personal name. If the name contains non US-ASCII characters, then the
	 * name will be encoded using the platform's default charset. If the name
	 * contains only US-ASCII characters, no encoding is done and the name is used
	 * as is.
	 * <p>
	 *
	...
	/**
	 * Get the personal name. If the name is encoded as per RFC 2047, it is decoded
	...
	/**
	 * Convert this address into a RFC 822 / RFC 2047 encoded address. The resulting
	 * string contains only US-ASCII characters, and hence is mail-safe.
	 *
	 ...
	 * separated by LWSP. Now, a word that contains LWSP is supposed to be quoted,
	 * and this is exactly what the MimeUtility.quote() method does. However, when
	 * dealing with a phrase, any LWSP encountered can be construed to be the
	 * separator between words, and not part of the words themselves. To deal with
	 * this funkiness, we have the below variant of MimeUtility.quote(), which
	 * essentially ignores LWSP when deciding whether to quote a word.
	 *
	 ...
	 * @exception ClassCastException if any address object in the given array is not
	 *                               an InternetAddress object. Note that this is a
	 *                               RuntimeException.
	...
	/**
	 * Convert the given array of InternetAddress objects into a comma separated
	 * sequence of address strings. The resulting string contains Unicode
	 * characters.
	 * <p>
	 *
	 * @param addresses array of InternetAddress objects
	 ...
	 * taken up in the field into which the resulting address sequence string is to
	 * be inserted. It is used to determine the line-break positions in the
	 * resulting address sequence string.
	 *
	 * @param addresses array of InternetAddress objects
	 * @param used      number of character positions already used, in the field
	 *                  into which the address string is to be inserted.
	 * @exception ClassCastException if any address object in the given array is not
	 *                               an InternetAddress object. Note that this is a
	 *                               RuntimeException.
	 * @return comma separated string of addresses
	 ...
	 */
	/*
	 * XXX - This is exactly the same as the above, except it uses toUnicodeString
	 * instead of toString. XXX - Since the line length restrictions are in bytes,
	 * not characters, we convert all non-ASCII addresses to UTF-8 byte strings,
	 * which we then convert to ISO-8859-1 Strings where every character respresents
	 * one UTF-8 byte. At the end we reverse the conversion to get back to a correct
	 * Unicode string. This is a hack to allow all the other character-based methods
	...
	/**
	 * A package-private version of getLocalAddress that doesn't swallow the
	 * exception. Used by MimeMessage.setFrom() to report the reason for the
	 * failure.
	 */
	// package-private
	...
	/**
	 * Get the local host name from InetAddress and return it in a form suitable for
	 * use in an email address.
	 */
	...
	/**
	 * Is the address an IPv4 or IPv6 address literal, which needs to be enclosed in
	 * "[]" in an email address? IPv4 literals contain decimal digits and dots, IPv6
	 * literals contain hex digits, dots, and colons. We're lazy and don't check the
	 * exact syntax, just the allowed characters; strings that have only the allowed
	 * characters in a literal but don't meet the syntax requirements for a literal
	 * definitely can't be a host name and thus will fail later when used as an
	 * address literal.
	 */
	 ...
	 * <code>strict</code> is false, simple email addresses separated by spaces are
	 * also allowed. If <code>strict</code> is true, many (but not all) of the
	 * RFC822 syntax rules are enforced. In particular, even if <code>strict</code>
	 * is true, addresses composed of simple names (with no "@domain" part) are
	 * allowed. Such "illegal" addresses are not uncommon in real messages.
	 * <p>
	 *
	...
	/**
	 * Parse the given sequence of addresses into InternetAddress objects. If
	 * <code>strict</code> is false, the full syntax rules for individual addresses
	 * are not enforced. If <code>strict</code> is true, many (but not all) of the
	 ...
	 * when the strict flag is false and enforces more rules when the strict flag is
	 * true. If the strict flag is false and the parse is successful in separating
	 * out an email address or addresses, the syntax of the addresses themselves is
	 * not checked.
	 *
	 * @param addresslist comma separated address strings
	 * @param strict      enforce RFC822 syntax
	...
	private static InternetAddress[] parse(String s, boolean strict, boolean parseHdr) throws AddressException {
		...
		for (start = end = -1, index = 0; index < length; index++) {
			...
			case ';':
				...
				if (in_group) {
					in_group = false;
					/*
					 * If parsing headers, but not strictly, peek ahead. If next char is "@", treat
					 * the group name like the local part of the address, e.g.,
			...
			case ',': // end of an address, probably
				...
				/*
				 * If the personal name field has an "@" and the address field does not, assume
				 * they were reversed, e.g., ``"joe doe" (john.doe@example.com)''.
				 */
			...
			case ':':
				...
				if (parseHdr && !strict) {
					/*
					 * If next char is a special character that can't occur at the start of a valid
					 * address, treat the group name as the entire address, e.g., "Date:, Tue",
					 * "Re:@foo".
					 */
					if (index + 1 < length) {
						...
						if (addressSpecials.indexOf(nc) >= 0) {
							...
							/*
							 * Handle a common error: ``Undisclosed-Recipient:@example.com;''
							 *
							 * Scan ahead. If we find a semicolon before one of these other special
							 * characters, consider it to be a group after all.
							 */
		...
		if (start >= 0) {
			/*
			 * The last token, add this to our InternetAddress list. Note that this block of
			 * code should be identical to the block above for "case ','".
			 */
			if (end == -1)
				end = length;
	 ...
	 * current implementation checks many, but not all, syntax rules. Note that even
	 * though the syntax of the address may be correct, there's no guarantee that a
	...
	/**
	 * Check that the address is a valid "mailbox" per RFC822. (We also allow simple
	 * names.)
	 *
	 * XXX - much more to check XXX - doesn't handle domain-literals properly (but
	 * no one uses them)
	 */
	private static void checkAddress(String addr, boolean routeAddr, boolean validate) throws AddressException {
		...
		/*
		 * The rest should be "local@domain", but we allow simply "local" unless called
		 * from validate.
		 *
		...
		for (i = start; i < len; i++) {
			...
			if (c == '"') {
				...
				/*
				 * CRLF followed by whitespace is allowed in a quoted string. We allowed naked
				 * LF, but ensure LF is always followed by whitespace to prevent spoofing the
				 * end of the header.
				 */
		 ...
		 * objects; it writes them out as headers and then parses the headers when the
		 * addresses are requested. In order to support the case where a "simple"
		 * address is used, but the address also has a personal name and thus looks like
		 * it should be a valid RFC822 address when parsed, we only check this if we're
	...
	/**
	 * Indicates whether this address is an RFC 822 group address. Note that a group
	 * address is different than the mailing list addresses supported by most mail
	 * servers. Group addresses are rarely used; see RFC 822 for details.
	 *
	 ...
	 * <code>strict</code> parameter controls whether the group list is parsed using
	 * strict RFC 822 rules or not. The parsing is done using the
	 ...
	 * argv.length; i++) { InternetAddress[] a = InternetAddress.parse(argv[i]); for
	 * (int j = 0; j < a.length; j++) { System.out.println("arg " + i + " address "
	 * + j + ": " + a[j]); System.out.println("\tAddress: " + a[j].getAddress() +
	 * "\tPersonal: " + a[j].getPersonal()); } if (a.length > 1) {
