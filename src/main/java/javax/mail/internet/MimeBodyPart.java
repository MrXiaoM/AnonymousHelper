...
public class MimeBodyPart extends BodyPart implements MimePart {
	...
	private static final boolean setDefaultTextCharset = PropUtil
			.getBooleanSystemProperty("mail.mime.setdefaulttextcharset", true);

	private static final boolean setContentTypeFileName = PropUtil
			.getBooleanSystemProperty("mail.mime.setcontenttypefilename", true);
	...
	private static final boolean ignoreMultipartEncoding = PropUtil
			.getBooleanSystemProperty("mail.mime.ignoremultipartencoding", true);
	...
	/**
	 * If the data for this body part was supplied by an InputStream that implements
	 * the SharedInputStream interface, <code>contentStream</code> is another such
	 * stream representing the content of this body part. In this case,
	...
	/**
	 * An empty MimeBodyPart object is created. This body part maybe filled in by a
	 * client constructing a multipart message.
	 */
	public MimeBodyPart() {
		super();
	...
	/**
	 * Constructs a MimeBodyPart by reading and parsing the data from the specified
	 * input stream. The parser consumes data till the end of the given input
	 * stream. The input stream must start at the beginning of a valid MIME body
	 * part and must terminate at the end of that body part.
	 * <p>
	 *
	 ...
	 * the MimeMultipart parser will extract each body part's bytes from a multipart
	 * stream and feed them into this constructor, without the delimiter strings.
	 *
	 ...
	 * the content type of the content of this body part. This value must not be
	 * null. If this field is unavailable, "text/plain" should be returned.
	 * <p>
	 *
	 * This implementation uses <code>getHeader(name)</code> to obtain the requisite
	 * header field.
	 *
	 ...
	 * @exception IllegalStateException if this body part is obtained from a
	 *                                  READ_ONLY folder.
	...
	/**
	 * Set the "Content-ID" header field of this body part. If the <code>cid</code>
	 ...
	 * The Content-Language header is defined by RFC 1766. Returns <code>null</code>
	...
	/**
	 * Set the Content-Language header of this MimePart. The Content-Language header
	 * is defined by RFC 1766.
	 *
	 ...
	 * @exception IllegalStateException if this body part is obtained from a
	 *                                  READ_ONLY folder.
	 * @exception MessagingException    otherwise; an UnsupportedEncodingException
	 *                                  may be included in the exception chain if
	 *                                  the charset conversion fails.
	 */
	@Override
	 ...
	 * header field of this body part. If its not available, returns the value of
	 * the "name" parameter from the "Content-Type" header field of this body part.
	 * Returns <code>null</code> if both are absent.
	 * <p>
	 *
	 * If the <code>mail.mime.decodefilename</code> System property is set to true,
	 ...
	 * this body part. For compatibility with older mailers, the "name" parameter of
	 * the "Content-Type" header is also set.
	 * <p>
	 *
	 ...
	 * Message object and was created by parsing a stream, the object is cached and
	 * returned in subsequent calls so that modifications to the content will not be
	 * lost.
	 *
	 * @return Object
	 * @exception IOException        this is typically thrown by the DataHandler.
	...
	/**
	 * This method provides the mechanism to set this body part's content. The given
	 ...
	 * name is used as the file name for this part and the data in the file is used
	 * as the data for this part. The encoding will be chosen appropriately for the
	 * file data. The disposition of this part is set to {@link Part#ATTACHMENT
	 * Part.ATTACHMENT}.
	 *
	 ...
	 * Content-Transfer-Encoding to provide the data for this part. If contentType
	 * or encoding are null, appropriate values will be chosen. The simple file name
	 * is used as the file name for this part and the data in the file is used as
	 * the data for this part. The disposition of this part is set to
	...
	/**
	 * Save the contents of this part in the specified file. The content is decoded
	 * and saved, without any of the MIME headers.
	 *
	 ...
	 * characters, so a header that contains non US-ASCII characters must be encoded
	 ...
	 * headers. Typical headers that get set here are <code>Content-Type</code> and
	 * <code>Content-Transfer-Encoding</code>. Headers might need to be updated in
	 * two cases:
	 *
	 * <br>
	 ...
	 *
	 * <br>
	 * - A message read in from a Store will have obtained all its headers from the
	 * store, and so doesn't need this. However, if this message is editable and if
	 * any edits have been made to either the content or message structure, we might
	...
	static void setDisposition(MimePart part, String disposition) throws MessagingException {
		if (disposition == null)
			...
			if (s != null) {
				 ...
				 *
				 * Override disposition, but attempt to retain existing disposition parameters
				 */
				ContentDisposition cd = new ContentDisposition(s);
	...
	static void setDescription(MimePart part, String description, String charset) throws MessagingException {
		...
		try {
			part.setHeader("Content-Description",
					MimeUtility.fold(21, MimeUtility.encodeText(description, charset, null)));
	...
	static String getEncoding(MimePart part) throws MessagingException {
		...
		// of tokenizer.
		if (s.equalsIgnoreCase("7bit") || s.equalsIgnoreCase("8bit") || s.equalsIgnoreCase("quoted-printable")
				|| s.equalsIgnoreCase("binary") || s.equalsIgnoreCase("base64"))
			return s;
	...
	/**
	 * Restrict the encoding to values allowed for the Content-Type of the specified
	 * MimePart. Returns either the original encoding or null.
	 */
	static String restrictEncoding(MimePart part, String encoding) throws MessagingException {
		...
		try {
			/*
			 * multipart and message types aren't allowed to have encodings except for the
			 * three mentioned above. If it's one of these types, ignore the encoding.
			 */
	...
	static void updateHeaders(MimePart part) throws MessagingException {
		...
		try {
			...
			/*
			 * If this is a multipart, give sub-parts a chance to update their headers. Even
			 * though the data for this multipart may have come from a stream, one of the
			...
			/*
			 * If this is our own MimePartDataHandler, we can't update any of the headers.
			 *
			 * If this is a MimePartDataHandler coming from another part, we need to copy
			 * over the content headers from the other part. Note that the
			 * MimePartDataHandler still refers to the original data and the original
			 * MimePart.
			 */
			...
			if (!composite) { // not allowed on composite parts
				...
				if (needCTHeader && setDefaultTextCharset && cType.match("text/*")
						&& cType.getParameter("charset") == null) {
					/*
					 * Set a default charset for text parts. We really should examine the data to
					 * determine whether or not it's all ASCII, but that's too expensive so we make
					 * an assumption: If we chose 7bit encoding for this data, it's probably ASCII.
					 * (MimeUtility.getEncoding will choose 7bit only in this case, but someone
					 * might've set the Content-Transfer-Encoding header manually.)
					 */
					String charset;
			...
			if (needCTHeader) {
				/*
				 * Pull out "filename" from Content-Disposition, and use that to set the "name"
				 * parameter. This is to satisfy older MUAs (DtMail, Roam and probably a bunch
				 * of others).
				 */
				if (setContentTypeFileName) {
					String s = part.getHeader("Content-Disposition", null);
	...
	static void writeTo(MimePart part, OutputStream os, String[] ignoreList) throws IOException, MessagingException {
		...
		try {
			/*
			 * If the data for this part comes from a stream, and is already encoded, just
			 * copy it to the output stream without decoding and reencoding it.
			 */
			DataHandler dh = part.getDataHandler();
			if (dh instanceof MimePartDataHandler) {
	...
	/**
	 * A special DataHandler used only as a marker to indicate that the source of
	 * the data is a MimePart (that is, a byte array or a stream). This prevents
	 * updateHeaders from trying to change the headers for such data. In particular,
