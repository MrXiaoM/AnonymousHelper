 ...
 * over SMTP), one must do <blockquote>
 * 
 * <pre>
 *
...
public class MimeUtility {
	...
	private static final boolean encodeEolStrict = PropUtil.getBooleanSystemProperty("mail.mime.encodeeol.strict",
			false);
	private static final boolean ignoreUnknownEncoding = PropUtil
			.getBooleanSystemProperty("mail.mime.ignoreunknownencoding", false);
	private static final boolean allowUtf8 = PropUtil.getBooleanSystemProperty("mail.mime.allowutf8", false);
	/*
	 * The following two properties allow disabling the fold() and unfold() methods
	 * and reverting to the previous behavior. They should never need to be changed
	 * and are here only because of my paranoid concern with compatibility.
	 */
	private static final boolean foldEncodedWords = PropUtil.getBooleanSystemProperty("mail.mime.foldencodedwords",
			false);
	private static final boolean foldText = PropUtil.getBooleanSystemProperty("mail.mime.foldtext", true);

	/**
	 * Get the Content-Transfer-Encoding that should be applied to the input stream
	 * of this DataSource, to make it mail-safe.
	 * <p>
	 *
	 * The algorithm used here is: <br>
	 * <ul>
	 * <li>If the DataSource implements {@link EncodingAware}, ask it what encoding
	 * to use. If it returns non-null, return that value.
	 * <li>If the primary type of this datasource is "text" and if all the bytes in
	 * its input stream are US-ASCII, then the encoding is "7bit". If more than half
	 * of the bytes are non-US-ASCII, then the encoding is "base64". If less than
	 * half of the bytes are non-US-ASCII, then the encoding is "quoted-printable".
	...
	/**
	 * Determine whether the charset in the Content-Type is compatible with ASCII or
	 * not. A charset is compatible with ASCII if the encoded byte stream
	 * representing the Unicode string "\r\n" is the ASCII characters CR and LF. For
	 * example, the utf-16be charset is not compatible with ASCII.
	 *
	...
	private static boolean nonAsciiCharset(ContentType ct) {
		...
		if (bool == null) {
			try {
				byte[] b = "\r\n".getBytes(charset);
				bool = Boolean.valueOf(b.length != 2 || b[0] != 015 || b[1] != 012);
	 ...
	 * <code>DataHandler</code> created with an object and a MIME type (for example,
	 * a "text/plain" String) because all the I/O is done in this thread. In the
	 * case requiring an <code>InputStream</code> the <code>DataHandler</code> uses
	 * a thread, a pair of pipe streams, and the <code>writeTo</code> method to
	 * produce the data.
	 * <p>
	 *
	...
	public static String getEncoding(DataHandler dh) {
		 ...
		 * method is typically faster. If the DataHandler was created with an object,
		 * this method is much faster. To distinguish the two cases, we use a heuristic.
		 * A DataHandler created with an object will always have a null name. A
		 * DataHandler created with a DataSource will usually have a non-null name.
		 *
		 * XXX - This is actually quite a disgusting hack, but it makes a common case
	...
	/**
	 * Decode the given input stream. The Input stream returned is the decoded input
	 * stream. All the encodings defined in RFC 2045 are supported here. They
	 * include "base64", "quoted-printable", "7bit", "8bit", and "binary". In
	...
	public static InputStream decode(InputStream is, String encoding) throws MessagingException {
		...
		else if (encoding.equalsIgnoreCase("binary") || encoding.equalsIgnoreCase("7bit")
	...
	/**
	 * Wrap an encoder around the given output stream. All the encodings defined in
	 * RFC 2045 are supported here. They include "base64", "quoted-printable",
	 ...
	 * string contains only US-ASCII characters, it is returned as-is. If the string
	 * contains non US-ASCII characters, it is first character-encoded using the
	 * platform's default charset, then transfer-encoded using either the B or Q
	 ...
	 * string contains only US-ASCII characters, it is returned as-is. If the string
	 * contains non US-ASCII characters, it is first character-encoded using the
	 * specified charset, then transfer-encoded using either the B or Q encoding.
	 * The resulting bytes are then returned as a Unicode string containing only
	 * ASCII characters.
	 * <p>
	 *
	 * Note that this method should be used to encode only "unstructured" RFC 822
	 ...
	 * @param encoding the encoding to be used. Currently supported values are "B"
	 *                 and "Q". If this parameter is null, then the "Q" encoding is
	 *                 used if most of characters to be encoded are in the ASCII
	 *                 charset, otherwise "B" encoding is used.
	 * @return Unicode string containing only US-ASCII characters
	 ...
	 * <p>
	 *
	 * The string is decoded using the algorithm specified in RFC 2047, Section 6.1.
	...
	public static String decodeText(String etext) throws UnsupportedEncodingException {
		/*
		 * We look for sequences separated by "linear-white-space". (as per RFC 2047,
		 * Section 6.1) RFC 822 defines "linear-white-space" as SPACE | HT | CR | NL.
		 */
		String lwsp = " \t\n\r";
		StringTokenizer st;
		 ...
		 * later decoding logic. This handles the most common case of unencoded headers
		 * efficiently.
		 */
		if (etext.indexOf("=?") == -1)
			return etext;
	...
	/*
	 * Encode the given string. The parameter 'encodingWord' should be true if a RFC
	 * 822 "word" token is being encoded and false if a RFC 822 "text" token is
	 * being encoded. This is because the "Q" encoding defined in RFC 2047 has more
	 * restrictions when encoding "word" tokens. (Sigh)
	 */
	...
	private static void doEncode(String string, boolean b64, String jcharset, int avail, String prefix, boolean first,
			boolean encodingWord, StringBuffer buf) throws UnsupportedEncodingException {
	...
	/**
	 * The string is parsed using the rules in RFC 2047 and RFC 2231 for parsing an
	 * "encoded-word". If the parse fails, a ParseException is thrown. Otherwise, it
	 * is transfer-decoded, and then charset-converted into Unicode. If the
	...
	public static String decodeWord(String eword) throws ParseException, UnsupportedEncodingException {
		 ...
		 *
		 * if (decodeStrict && pos != eword.length() - 2) throw new ParseException(
		...
		try {
			...
			/*
			 * An unknown charset of the form ISO-XXX-XXX, will cause the JDK to throw an
			 * IllegalArgumentException ... Since the JDK will attempt to create a classname
			 * using this string, but valid classnames must not contain the character '-',
			 * and this results in an IllegalArgumentException, rather than the expected
	 ...
	 * <p>
	 *
	 * The <code>HeaderTokenizer</code> class defines two special sets of delimiters
	 * - MIME and RFC 822.
	 * <p>
	 *
	...
	public static String quote(String word, String specials) {
		...
		for (int i = 0; i < len; i++) {
			...
			if (c == '"' || c == '\\' || c == '\r' || c == '\n') {
				...
				sb.append('"');
				return sb.toString();
			} else if (c < 040 || (c >= 0177 && !allowUtf8) || specials.indexOf(c) >= 0)
	 ...
	 * characters, if possible. If there are more than 76 non-whitespace characters
	 * consecutively, the string is folded at the first whitespace after that
	 * sequence. The parameter <code>used</code> indicates how many characters have
	 * been used in the current line; it is usually the length of the header name.
	 * <p>
	 *
	...
	/**
	 * If the String or StringBuilder has any embedded newlines, make sure they're
	 * followed by whitespace, to prevent header injection errors.
	 */
	...
	/**
	 * Unfold a folded header. Any line breaks that aren't escaped and are followed
	 * by whitespace are removed.
	 *
	 ...
	 * @return the Java charset equivalent. If a suitable mapping is not available,
	 *         the passed in charset is itself returned.
	 */
	 ...
	 * @return the MIME/IANA equivalent. If a mapping is not possible, the passed in
	 *         charset itself is returned.
	 * @since JavaMail 1.1
	 */
	 ...
	 * If the System property <code>mail.mime.charset</code> is set, a system
	 * charset corresponding to this MIME charset will be returned.
	 * <p>
	 * 
	...
	public static String getDefaultJavaCharset() {
		if (defaultJavaCharset == null) {
			/*
			 * If mail.mime.charset is set, it controls the default Java charset as well.
			 */
			String mimecs = null;
			try {
	...
	static {
		...
		try {
			...
			InputStream is = javax.mail.internet.MimeUtility.class
	 ...
	 * @param max             maximum bytes to check for. The special value ALL
	 *                        indicates that all the bytes in this input stream must
	 *                        be checked.
	 * @param breakOnNonAscii if <code>true</code>, then terminate the the check
	 *                        when the first non-US-ASCII character is found.
	 * @return ALL_ASCII if all characters in the string belong to the US-ASCII
	 *         charset. MOSTLY_ASCII if more than half of the available characters
	 *         are US-ASCII characters. Else MOSTLY_NONASCII.
	 */
