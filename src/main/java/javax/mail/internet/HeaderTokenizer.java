...
public class HeaderTokenizer {
	...
	public static class Token {
		...
		/**
		 * Token type indicating a quoted string. The value field contains the string
		 * without the quotes.
		 */
		public static final int QUOTEDSTRING = -2;

		/**
		 * Token type indicating a comment. The value field contains the comment string
		 * without the comment start and end symbols.
		 */
		...
		/**
		 * Return the type of the token. If the token represents a delimiter or a
		 * control character, the type is that character itself, converted to an
		 * integer. Otherwise, it's value is one of the following:
		 * <ul>
		 * <li><code>ATOM</code> A sequence of ASCII characters delimited by either
		 * SPACE, CTL, "(", &lt;"&gt; or the specified SPECIALS
		 * <li><code>QUOTEDSTRING</code> A sequence of ASCII characters within quotes
		 * <li><code>COMMENT</code> A sequence of ASCII characters within "(" and ")".
		...
		/**
		 * Returns the value of the token just read. When the current token is a quoted
		 * string, this field contains the body of the string, without the quotes. When
		 * the current token is a comment, this field contains the body of the comment.
		 *
		 * @return token value
		 */
	 ...
	 *                     These are usually <code>RFC822</code> or
	 *                     <code>MIME</code>
	 * @param skipComments If true, comments are skipped and not returned as tokens
	 */
	 ...
	 * <p>
	 *
	 * Clients sit in a loop calling next() to parse successive tokens until an EOF
	 * Token is returned.
	 *
	...
	/**
	 * Parses the next token from this String. If endOfAtom is not NUL, the token
	 * extends until the endOfAtom character is seen, or to the end of the header.
	 * This method is useful when parsing headers that don't obey the MIME
	 * specification, e.g., by failing to quote parameter values that contain
	 * spaces.
	 *
	...
	/**
	 * Parses the next token from this String. endOfAtom is handled as above. If
	 * keepEscapes is true, any backslash escapes are preserved in the returned
	 * string. This method is useful when parsing headers that don't obey the MIME
	 * specification, e.g., by failing to escape backslashes in the filename
	 * parameter.
	 *
	 * @param endOfAtom   if not NUL, character marking end of token
	...
	/*
	 * Process escape sequences and embedded LWSPs from a comment or quoted string.
	 */
