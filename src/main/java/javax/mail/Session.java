...
public final class Session {
	...
	public static synchronized Session getDefaultInstance(Properties props, Authenticator authenticator) {
		if (defaultSession == null) {
			...
			else if (defaultSession.authenticator != null && authenticator != null && defaultSession.authenticator
					.getClass().getClassLoader() == authenticator.getClass().getClassLoader())
	 ...
	 * <code>mail.debug</code> in the Properties object passed in to the constructor
	 * to true. The value of the <code>mail.debug</code> property is used to
	...
	public synchronized void setDebug(boolean debug) {
		this.debug = debug;
		initLogger();
		logger.log(Level.CONFIG, "setDebug: JavaMail version {0}", Version.version);
	}
	 ...
	 * @exception NoSuchProviderException If a provider for the given protocol is
	 *                                    not found.
	 */
	...
	/**
	 * Set the passed Provider to be the default implementation for the protocol in
	 * Provider.protocol overriding any previous values.
	 *
	 * @param provider Currently configured Provider which will be set as the
	 *                 default for the protocol
	 * @exception NoSuchProviderException If the provider passed in is invalid.
	 */
	 ...
	 * <code>mail.store.protocol</code> property specifies the desired protocol. If
	 * an appropriate Store object is not obtained, NoSuchProviderException is
	 * thrown
	 *
	...
	/**
	 * Get a Store object that implements the specified protocol. If an appropriate
	 * Store object cannot be obtained, NoSuchProviderException is thrown.
	 *
	 ...
	 * cannot be obtained, NoSuchProviderException is thrown.
	 *
	 ...
	 * @exception NoSuchProviderException If a provider for the given URLName is not
	 *                                    found.
	 */
	 ...
	 * @exception NoSuchProviderException If a provider for the given Provider is
	 *                                    not found.
	 */
	 ...
	 *
	 * The "scheme" part of the URL string (Refer RFC 1738) is used to locate the
	 * Store protocol. The rest of the URL string (that is, the "schemepart", as per
	 * RFC 1738) is used by that Store in a protocol dependent manner to locate and
	 * instantiate the appropriate Folder object.
	 * <p>
	 *
	 ...
	 * @exception NoSuchProviderException If a provider for the given URLName is not
	 *                                    found.
	 * @exception MessagingException      if the Folder could not be located or
	 *                                    created.
	 */
	 ...
	 * @return a Transport object
	 * @exception NoSuchProviderException If provider for the given protocol is not
	 *                                    found.
	 */
	...
	/**
	 * Get an instance of the transport specified in the Provider. Instantiates the
	 * transport and returns it.
	 * 
	 * @param provider Transport Provider that will be instantiated
	 * @return Instantiated Transport
	 * @exception NoSuchProviderException If provider for the given provider is not
	 *                                    found.
	 */
	...
	/**
	 * Get a Transport object that can transport a Message of the specified address
	 * type.
	 *
	 * @param address an address for which a Transport is needed
	 * @return A Transport object
	 * @see javax.mail.Address
	 * @exception NoSuchProviderException If provider for the Address type is not
	 *                                    found
	 */
	 ...
	 * @return A Transport object
	 * @exception NoSuchProviderException If no provider or the provider was the
	 *                                    wrong class.
	 */
	 ...
	 * @exception NoSuchProviderException thrown when the class cannot be found or
	 *                                    when it does not have the correct
	 *                                    constructor (Session, URLName), or if it
	 *                                    is not derived from Service.
	 */
	...
	/**
	 * Save a PasswordAuthentication for this (store or transport) URLName. If pw is
	 * null the entry corresponding to the URLName is removed.
	 * <p>
	 * This is normally used only by the store or transport implementations to allow
	 * authentication information to be shared among multiple uses of a session.
	 *
	 ...
	 * @param prompt          any additional String to show as part of the prompt;
	 *                        may be null.
	...
	private void loadProviders(Class<?> cl) {
		...
		if (providers.size() == 0) {
			...
			addProvider(new Provider(Provider.Type.STORE, "imap", "com.sun.mail.imap.IMAPStore", "Oracle",
					Version.version));
			addProvider(new Provider(Provider.Type.STORE, "imaps", "com.sun.mail.imap.IMAPSSLStore", "Oracle",
					Version.version));
			addProvider(new Provider(Provider.Type.STORE, "pop3", "com.sun.mail.pop3.POP3Store", "Oracle",
					Version.version));
			addProvider(new Provider(Provider.Type.STORE, "pop3s", "com.sun.mail.pop3.POP3SSLStore", "Oracle",
					Version.version));
			addProvider(new Provider(Provider.Type.TRANSPORT, "smtp", "com.sun.mail.smtp.SMTPTransport", "Oracle",
					Version.version));
			addProvider(new Provider(Provider.Type.TRANSPORT, "smtps", "com.sun.mail.smtp.SMTPSSLTransport", "Oracle",
					Version.version));
		}
	...
	private void loadProvidersFromStream(InputStream is) throws IOException {
		if (is != null) {
			...
			while ((currLine = lis.readLine()) != null) {
				...
				if (type == null || protocol == null || className == null || protocol.length() <= 0
						|| className.length() <= 0) {
