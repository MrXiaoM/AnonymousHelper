		...
		for (;;) { // read until CRLF with no preceeding literal
			...
			if (count > 0) {
				...
				if (count + incrementSlop > avail) {
					...
					ba.grow(minIncrement > count + incrementSlop - avail ? minIncrement
							: count + incrementSlop - avail);
					buffer = ba.getBytes();
				}

				/*
				 * read() might not return all the bytes in one shot, so call repeatedly till we
				 * are done
				 */
				int actual;
				while (count > 0) {
