...
public class Protocol {
	...
	/**
	 * Returns whether this Protocol supports non-synchronizing literals Default is
	 * false. Subclasses should override this if required
	 *
	...
	public boolean hasResponse() {
		/*
		 * XXX - Really should peek ahead in the buffer to see if there's a *complete*
		 * response available, but if there isn't who's going to read more data into the
		 * buffer until there is?
		 */
		try {
	...
	/**
	 * Convenience routine to handle simple IAP commands that do not have responses
	...
	/**
	 * Start TLS on the current connection. <code>cmd</code> is the command to issue
	 * to start TLS negotiation. If the command succeeds, we begin TLS negotiation.
	 * If the socket is already an SSLSocket this is a nop and the command is not
	 * issued.
	 *
	...
	/**
	 * Start compression on the current connection. <code>cmd</code> is the command
	 * to issue to start compression. If the command succeeds, we begin compression.
	 *
	 * @param cmd the command to issue
	 * @exception IOException       for I/O errors
	 * @exception ProtocolException for protocol failures
	...
	public synchronized void startCompression(String cmd) throws IOException, ProtocolException {
		/*
		 * The Deflator.SYNC_FLUSH support requires JDK 1.7 so use reflection to allow
		 * compiling on 1.5 but running on 1.7.
		 */
		Class<DeflaterOutputStream> dc = DeflaterOutputStream.class;
		Constructor<DeflaterOutputStream> cons = null;
		try {
			cons = dc.getConstructor(OutputStream.class, Deflater.class, boolean.class);
		...
		int level = PropUtil.getIntProperty(props, prefix + ".compress.level", Deflater.DEFAULT_COMPRESSION);
		int strategy = PropUtil.getIntProperty(props, prefix + ".compress.strategy", Deflater.DEFAULT_STRATEGY);
		if (logger.isLoggable(Level.FINE))
			logger.log(Level.FINE, "Creating Deflater with compression level {0} and strategy {1}",
		...
		try {
			traceOutput = new TraceOutputStream(cons.newInstance(socket.getOutputStream(), def, true), traceLogger);
		} catch (Exception ex) {
	...
	/**
	 * Does the server support UTF-8? This implementation returns false. Subclasses
	...
	/**
	 * Get the name of the local host. The property &lt;prefix&gt;.localhost
	 * overrides &lt;prefix&gt;.localaddress, which overrides what InetAddress would
	 * tell us.
	 *
