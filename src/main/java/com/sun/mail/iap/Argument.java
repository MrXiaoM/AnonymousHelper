...
public class Argument {
	...
	/**
	 * Write out given string as an ASTRING, depending on the type of the characters
	 * inside the string. The string should contain only ASCII characters.
	 * <p>
	 *
	...
	/**
	 * Write out given string as an NSTRING, depending on the type of the characters
	 * inside the string. The string should contain only ASCII characters.
	 * <p>
	 *
	 * @param s String to write out
	...
	/**
	 * Write out given string as an Atom. Note that an Atom can contain only certain
	 * US-ASCII characters. No validation is done on the characters in the string.
	 * 
	 * @param s String
	 * @return this
	 */
	...
	private void nastring(byte[] bytes, Protocol protocol, boolean doQuote) throws IOException, ProtocolException {
		...
		/*
		 * Make sure the (case-independent) string "NIL" is always quoted, so as not to
		 * be confused with a real NIL (handled above in nstring). This is more than is
		 * necessary, but it's rare to begin with and this makes it safer than doing the
		 * test in nstring above in case some code calls writeString when it should call
		 * writeNString.
		 */
		if (!quote && bytes.length == 3 && (bytes[0] == 'N' || bytes[0] == 'n') && (bytes[1] == 'I' || bytes[1] == 'i')
				&& (bytes[2] == 'L' || bytes[2] == 'l'))
			quote = true;
