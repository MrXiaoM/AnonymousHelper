...
public class BASE64DecoderStream extends FilterInputStream {
	...
	private int decode(byte[] outbuf, int pos, int len) throws IOException {
		...
		while (len >= 3) {
			/*
			 * We need 4 valid base64 characters before we start decoding. We skip anything
			 * that's not a valid base64 character (usually just CRLF).
			 */
			int got = 0;
			int val = 0;
			while (got < 4) {
					...
					if (i == -1) {
						...
						if (!ignoreErrors)
							throw new DecodingException(
									"BASE64Decoder: Error in encoded stream: " + "needed 4 valid base64 characters "
											+ "but only got " + got + " before EOF" + recentChars());
						...
						if (got < 2 && !ignoreErrors)
							throw new DecodingException("BASE64Decoder: Error in encoded stream: "
									+ "needed at least 2 valid base64 characters," + " but only got " + got
									+ " before padding character (=)" + recentChars());
	 ...
	 * but not to decode the entire content of a MIME part.
	 *
