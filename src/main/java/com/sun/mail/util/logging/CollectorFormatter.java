 ...
 *
 * <li>&lt;formatter-name&gt;.comparator.reverse a boolean <tt>true</tt> to
 * collect the min <tt>LogRecord</tt> or <tt>false</tt> to collect the max
 * <tt>LogRecord</tt>. (defaults to <tt>false</tt>)
 *
 * <li>&lt;formatter-name&gt;.format the {@linkplain java.text.MessageFormat
 * MessageFormat} string used to format the collected summary statistics. The
 * arguments are explained in detail in the
...
public class CollectorFormatter extends Formatter {
	...
	/**
	 * The last accepted record. Synchronized access is preferred over volatile for
	 * this class.
	 */
	...
	private long count;
	/**
	 * The number of log produced containing at least one log record. Only
	 * incremented when this formatter is reset.
	 */
	 ...
	 * @throws UndeclaredThrowableException if there are problems loading from the
	 *                                      LogManager.
	 */
	...
	/**
	 * Accumulates log records which will be used to produce the final output. The
	 * output is generated using the {@link #getTail} method which also resets this
	 * formatter back to its original state.
	 *
	...
	/**
	 * Formats the collected LogRecord and summary statistics. The collected results
	 * are reset after calling this method. The {@linkplain java.text.MessageFormat
	 * java.text} argument indexes are assigned to the following properties:
	 *
	 * <ol start='0'>
	 * <li>{@code head} the {@linkplain Formatter#getHead(java.util.logging.Handler)
	 * head} string returned from the target formatter and
	 * {@linkplain #finish(java.lang.String) finished} by this formatter.
	 * <li>{@code formatted} the current log record
	 * {@linkplain Formatter#format(java.util.logging.LogRecord) formatted} by the
	 * target formatter and {@linkplain #finish(java.lang.String) finished} by this
	 * formatter. If the formatter is null then record is formatted by this
	 * {@linkplain #formatMessage(java.util.logging.LogRecord) formatter}.
	 * <li>{@code tail} the {@linkplain Formatter#getTail(java.util.logging.Handler)
	 * tail} string returned from the target formatter and
	 * {@linkplain #finish(java.lang.String) finished} by this formatter.
	 * <li>{@code count} the total number of log records {@linkplain #format
	 * consumed} by this formatter.
	 * <li>{@code remaining} the count minus one.
	 * <li>{@code thrown} the total number of log records {@linkplain #format
	 ...
	 * {@linkplain #format consumed} by this formatter. If the count is zero then
	 * this is set to the previous max or approximate start time if there was no
	 * previous max. By default this parameter is defined as a number. The format
	 * type and format style rules from the {@linkplain java.text.MessageFormat}
	 * should be used to convert this from milliseconds to a date or time.
	 * <li>{@code maxMillis} the most recent log record
	 * {@linkplain java.util.logging.LogRecord#getMillis event time}
	 * {@linkplain #format consumed} by this formatter. If the count is zero then
	 * this is set to the {@linkplain System#currentTimeMillis() current time}. By
	 ...
	 * <li>{@code startTime} the approximate start time in milliseconds. By default
	 * this parameter is defined as a number. The format type and format style rules
	 * from the {@linkplain java.text.MessageFormat} should be used to convert this
	 * from milliseconds to a date or time.
	 * <li>{@code currentTime} the {@linkplain System#currentTimeMillis() current
	 * time} in milliseconds. By default this parameter is defined as a number. The
	 * format type and format style rules from the
	 ...
	 * <li>{@code generation} the number times this method produced output with at
	 * least one {@linkplain #format consumed} log record. This can be used to track
	 * the number of complete reports this formatter has produced.
	 * </ol>
	 *
	 * <p>
	 * Some example formats:<br>
	 * <ul>
	 * <li>{@code com.sun.mail.util.logging.CollectorFormatter.format={0}{1}{2}{4,choice,-1#|0#|0<...
	 * {4,number,integer} more}\n}
	 * <p>
	 * This prints the head ({@code {0}}), format ({@code {1}}), and tail
	 * ({@code {2}}) from the target formatter followed by the number of remaining
	 * ({@code {4}}) log records consumed by this formatter if there are any
	 * remaining records.
	 * 
	 * <pre>
	 ...
	 * </pre>
	 * 
	 * <li>{@code com.sun.mail.util.logging.CollectorFormatter.format=These {3}
	 * messages occurred between\n{7,date,EEE, MMM dd HH:mm:ss:S ZZZ yyyy} and
	 * {8,time,EEE, MMM dd HH:mm:ss:S ZZZ yyyy}\n}
	 * <p>
	 * This prints the count ({@code {3}}) followed by the date and time of the
	 * eldest log record ({@code {7}}) and the date and time of the most recent log
	 * record ({@code {8}}).
	 * 
	 * <pre>
	 ...
	 * 
	 * <li>{@code com.sun.mail.util.logging.CollectorFormatter.format=These {3}
	 * messages occurred between {9,choice,86400000#{7,date} {7,time} and
	 * {8,time}|86400000<{7,date} and {8,date}}\n}
	 * <p>
	 * This prints the count ({@code {3}}) and then chooses the format based on the
	 * elapsed time ({@code {9}}). If the elapsed time is less than one day then the
	 * eldest log record ({@code {7}}) date and time is formatted followed by just
	 * the time of the most recent log record ({@code {8}}. Otherwise, the just the
	 * date of the eldest log record ({@code {7}}) and just the date of most recent
	 * log record ({@code {8}} is formatted.
	 * 
	 * <pre>
	 ...
	 * </pre>
	 * 
	 * <li>{@code com.sun.mail.util.logging.CollectorFormatter.format={13} alert
	 * reports since {10,date}.\n}
	 * <p>
	...
	/**
	 * Updates the summary statistics only if the expected record matches the last
	 * record. The update record is not stored.
	 *
	...
	private synchronized boolean accept(final LogRecord e, final LogRecord u) {
		/**
		 * LogRecord methods must be called before the check of the last stored record
		 * to guard against subclasses of LogRecord that might attempt to reset the
		 * state by triggering a call to getTail.
		 */
	...
	private String formatRecord(final Handler h, final boolean reset) {
		 ...
		 */
		return mf.format(new Object[] { finish(head), finish(msg), finish(tail), c, (c - 1L), t, (c - t), msl, msh,
				(msh - msl), INIT_TIME, now, (now - INIT_TIME), g });
	}

	/**
	 * Applied to the head, format, and tail returned by the target formatter. This
	 * implementation trims all input strings.
	 *
	...
	/**
	 * Gets and creates the comparator from the LogManager or returns the default
	 * comparator.
	 *
	 * @param p the class name prefix.
	 * @return the comparator or null.
	 * @throws IllegalArgumentException     if it was specified that the comparator
	 *                                      should be reversed but no initial
	 *                                      comparator was specified.
	 * @throws NullPointerException         if the given argument is null.
	 * @throws UndeclaredThrowableException if the comparator can not be created.
	 */
