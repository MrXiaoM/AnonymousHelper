...
final class LogManagerProperties extends Properties {
	...
	/**
	 * Holds the method used to convert and instant to a zoned date time if running
	 * on JDK 9 later.
	 */
	...
	/**
	 * Caches the read only reflection class names string array. Declared volatile
	 * for safe publishing only. The VO_VOLATILE_REFERENCE_TO_ARRAY warning is a
	 * false positive.
	 */
	...
	/**
	 * Create a properties object from the default logging configuration file. Since
	 * the LogManager is not available in restricted environments, only the default
	...
	private static Properties readConfiguration() {
		/**
		 * Load the properties file so the default settings are available when user code
		 * creates a logging object. The class loader for the restricted LogManager
		 * can't access these classes to attach them to a logger or handler on startup.
		 * Creating logging objects at this point is both useless and risky.
		 */
	...
	/**
	 * Gets LogManger property for the running JVM. If the LogManager doesn't exist
	 * then the default LogManger properties are used.
	 *
	 ...
	 *                           not have {@code LoggingPermission("control")}.
	 * @since JavaMail 1.5.3
	 */
	 ...
	 * when the LogManager is not present. This requires
	...
	private static void checkLoggingAccess() {
		/**
		 * Some environments selectively enforce logging permissions by allowing access
		 * to loggers but not allowing access to handlers. This is an indirect way of
		 * checking for LoggingPermission when the LogManager is not present. The root
		 * logger will lazy create handlers so the global logger is used instead as it
		 * is a known named logger with well defined behavior. If the global logger is a
		 * subclass then fallback to using the SecurityManager.
		 */
		boolean checked = false;
	 ...
	 * @return null if LogRecord doesn't support nanoseconds otherwise a new zoned
	 ...
	 * @throws NoSuchMethodException       if the given service does not have a
	 *                                     method to get the local host name as a
	 *                                     string.
	 * @throws SecurityException           if unable to inspect properties of
	 *                                     object.
	 * @since JavaMail 1.5.3
	 */
	 ...
	 * @throws SecurityException           if reflective access to the java.time
	 ...
	 * @throws LinkageError                if the linkage fails.
	 * @throws ExceptionInInitializerError if the static initializer fails.
	 * @throws Exception                   to match the error method of the
	 *                                     ErrorManager.
	 ...
	 * {@link Comparator}. If the given comparator declares a public reverseOrder
	 * method that method is called first and the return value is used. If that
	 * method is not declared or the caller does not have access then a comparator
	 * wrapping the given comparator is returned.
	 *
	...
	/**
	 * Determines if the given class name is a reflection class name responsible for
	 * invoking methods and or constructors.
	 *
	 * @param name the fully qualified class name.
	 * @return true if the given class name
	 * @throws ClassNotFoundException      if the class name was not found.
	 ...
	 *
	 * This method performs indirect and direct calls on a throwable to capture the
	 * standard class names and the implementation class names.
	 *
	...
	/**
	 * Throws the given error if the cause is an error otherwise the given error is
	 * wrapped.
	 *
	...
	/**
	 * This code is modified from the LogManager, which explictly states searching
	 * the system class loader first, then the context class loader. There is
	 * resistance (compatibility) to change this behavior to simply searching the
	...
	/**
	 * Returns a properties object that contains a snapshot of the current state.
	 * This method violates the clone contract so that no instances of
	...
	/**
	 * Searches defaults, then searches the log manager if available or the system
	 * properties by the prefix property, and then by the key itself.
	 *
	...
	public synchronized String getProperty(final String key) {
		...
		if (value == null) {
			...
			/**
			 * Copy the log manager properties as we read them. If a value is no longer
			 * present in the LogManager read it from here. The reason this works is because
			 * LogManager.reset() closes all attached handlers therefore, stale values only
			 * exist in closed handlers.
			 */
			if (value != null) {
				super.put(key, value);
			} else {
	...
	/**
	 * Calls getProperty directly. If getProperty returns null the default value is
	 * returned.
	 *
	...
	/**
	 * Required to work with PropUtil. Calls getProperty directly if the given key
	 * is a string. Otherwise, performs a get operation on the defaults followed by
	 * the normal hash table get.
	 *
	 * @param key any key.
	...
	/**
	 * Required to work with PropUtil. An updated copy of the key is fetched from
	 * the log manager if the key doesn't exist in this properties.
	 *
	 * @param key any key.
	...
	/**
	 * Required to work with PropUtil. An updated copy of the key is fetched from
	 * the log manager prior to returning.
	 *
	 * @param key any key.
	 * @return the value for the key or null.
	 * @since JavaMail 1.4.5
	...
	/**
	 * It is assumed that this method will never be called. The prefix value is not
	 ...
	 * instance of a String and this properties doesn't contain a copy of the key.
	 *
	...
	/**
	 * Creates a public snapshot of this properties object using the given parent
	 * properties.
	 *
	...
	/**
	 * It is assumed that this method will never be called. We return a safe copy
	 * for export to avoid locking this properties object or the defaults during
	 * write.
	 *
