 ...
 * email server. The code to manually setup this handler can be as simple as the
 * following:
 *
 * <pre>
 ...
 *
 * <p>
 * <b>Configuration:</b> The LogManager should define at least one or more
 * recipient addresses and a mail host for outgoing email. The code to setup
 * this handler via the logging properties can be as simple as the following:
 *
 * <pre>
 ...
 *
 * <ul>
 * <li>&lt;handler-name&gt;.attachment.filters a comma separated list of
 * <tt>Filter</tt> class names used to create each attachment. The literal
 * <tt>null</tt> is reserved for attachments that do not require filtering.
 * (defaults to the {@linkplain java.util.logging.Handler#getFilter() body}
 * filter)
 *
 * <li>&lt;handler-name&gt;.attachment.formatters a comma separated list of
 * <tt>Formatter</tt> class names used to create each attachment. (default is no
 * attachments)
 *
 * <li>&lt;handler-name&gt;.attachment.names a comma separated list of names or
 * <tt>Formatter</tt> class names of each attachment. All control characters are
 * removed from the attachment names. (default is
 * {@linkplain java.util.logging.Formatter#toString() toString} of the
 ...
 * {@linkplain Authenticator#getDefaultUserName() default} user name. (default
 * is <tt>null</tt>)
 *
 * <li>&lt;handler-name&gt;.capacity the max number of <tt>LogRecord</tt>
 * objects include in each email message. (defaults to <tt>1000</tt>)
 *
 * <li>&lt;handler-name&gt;.comparator name of a
 * {@linkplain java.util.Comparator} class used to sort the published
 * <tt>LogRecord</tt> objects prior to all formatting. (defaults to
 * <tt>null</tt> meaning records are unsorted).
 *
 * <li>&lt;handler-name&gt;.comparator.reverse a boolean <tt>true</tt> to
 * reverse the order of the specified comparator or <tt>false</tt> to retain the
 * original order. (defaults to <tt>false</tt>)
 *
 ...
 * platform encoding).
 *
 * <li>&lt;handler-name&gt;.errorManager name of an <tt>ErrorManager</tt> class
 * used to handle any configuration or mail transport problems. (defaults to
 * <tt>java.util.logging.ErrorManager</tt>)
 *
 * <li>&lt;handler-name&gt;.filter name of a <tt>Filter</tt> class used for the
 * body of the message. (defaults to <tt>null</tt>, allow all records)
 *
 * <li>&lt;handler-name&gt;.formatter name of a <tt>Formatter</tt> class used to
 * format the body of this message. (defaults to
 * <tt>java.util.logging.SimpleFormatter</tt>)
 *
 * <li>&lt;handler-name&gt;.level specifies the default level for this
 * <tt>Handler</tt> (defaults to <tt>Level.WARNING</tt>).
 *
 * <li>&lt;handler-name&gt;.mail.bcc a comma separated list of addresses which
 * will be blind carbon copied. Typically, this is set to the recipients that
 * may need to be privately notified of a log message or notified that a log
 * message was sent to a third party such as a support team. The empty string
 * can be used to specify no blind carbon copied address. (defaults to
 * <tt>null</tt>, none)
 *
 * <li>&lt;handler-name&gt;.mail.cc a comma separated list of addresses which
 * will be carbon copied. Typically, this is set to the recipients that may need
 * to be notified of a log message but, are not required to provide direct
 * support. The empty string can be used to specify no carbon copied address.
 * (defaults to <tt>null</tt>, none)
 *
 * <li>&lt;handler-name&gt;.mail.from a comma separated list of addresses which
 * will be from addresses. Typically, this is set to the email address
 * identifying the user running the application. The empty string can be used to
 * override the default behavior and specify no from address. (defaults to the
 * {@linkplain javax.mail.Message#setFrom() local address})
 *
 * <li>&lt;handler-name&gt;.mail.host the host name or IP address of the email
 * server. (defaults to <tt>null</tt>, use {@linkplain Transport#protocolConnect
 * default} <tt>Java Mail</tt> behavior)
 *
 * <li>&lt;handler-name&gt;.mail.reply.to a comma separated list of addresses
 * which will be reply-to addresses. Typically, this is set to the recipients
 * that provide support for the application itself. The empty string can be used
 * to specify no reply-to address. (defaults to <tt>null</tt>, none)
 *
 * <li>&lt;handler-name&gt;.mail.to a comma separated list of addresses which
 * will be send-to addresses. Typically, this is set to the recipients that
 * provide support for the application, system, and/or supporting
 * infrastructure. The empty string can be used to specify no send-to address
 * which overrides the default behavior. (defaults to
 * {@linkplain javax.mail.internet.InternetAddress#getLocalAddress local
 * address}.)
 *
 * <li>&lt;handler-name&gt;.mail.sender a single address identifying sender of
 * the email; never equal to the from address. Typically, this is set to the
 * email address identifying the application itself. The empty string can be
 * used to specify no sender address. (defaults to <tt>null</tt>, none)
 *
 * <li>&lt;handler-name&gt;.subject the name of a <tt>Formatter</tt> class or
 * string literal used to create the subject line. The empty string can be used
 * to specify no subject. All control characters are removed from the subject
 * line. (defaults to {@linkplain com.sun.mail.util.logging.CollectorFormatter
 * CollectorFormatter}.)
 *
 * <li>&lt;handler-name&gt;.pushFilter the name of a <tt>Filter</tt> class used
 * to trigger an early push. (defaults to <tt>null</tt>, no early push)
 *
 * <li>&lt;handler-name&gt;.pushLevel the level which will trigger an early
 * push. (defaults to <tt>Level.OFF</tt>, only push when full)
 *
 * <li>&lt;handler-name&gt;.verify <a name="verify">used</a> to verify the
 * <tt>Handler</tt> configuration prior to a push.
 * <ul>
 ...
 * <li>If set to a value of <tt>local</tt> the <tt>Handler</tt> will verify all
 * of settings of the local machine.
 * <li>If set to a value of <tt>resolve</tt>, the <tt>Handler</tt> will verify
 * all local settings and try to resolve the remote host name with the domain
 * name server.
 * <li>If set to a value of <tt>login</tt>, the <tt>Handler</tt> will verify all
 * local settings and try to establish a connection with the email server.
 * <li>If set to a value of <tt>remote</tt>, the <tt>Handler</tt> will verify
 * all local settings, try to establish a connection with the email server, and
 * try to verify the envelope of the email message.
 * </ul>
 * If this <tt>Handler</tt> is only implicitly closed by the
 * <tt>LogManager</tt>, then verification should be turned on. (defaults to
 * <tt>null</tt>, no verify).
 * </ul>
 *
 * <p>
 * <b>Normalization:</b> The error manager, filters, and formatters when loaded
 * from the LogManager are converted into canonical form inside the MailHandler.
 * The pool of interned values is limited to each MailHandler object such that
 * no two MailHandler objects created by the LogManager will be created sharing
 ...
 * {@linkplain java.lang.Object#equals(java.lang.Object) equals} and
 * {@linkplain java.lang.Object#hashCode() hashCode} methods. The recommended
 * code to use for stateless filters and formatters is:
 * 
 * <pre>
 ...
 *
 * <p>
 * <b>Sorting:</b> All <tt>LogRecord</tt> objects are ordered prior to
 * formatting if this <tt>Handler</tt> has a non null comparator. Developers
 * might be interested in sorting the formatted email by thread id, time, and
 * sequence properties of a <tt>LogRecord</tt>. Where as system administrators
 * might be interested in sorting the formatted email by thrown, level, time,
 * and sequence properties of a <tt>LogRecord</tt>. If comparator for this
 * handler is <tt>null</tt> then the order is unspecified.
 *
 * <p>
 * <b>Formatting:</b> The main message body is formatted using the
 * <tt>Formatter</tt> returned by <tt>getFormatter()</tt>. Only records that
 * pass the filter returned by <tt>getFilter()</tt> will be included in the
 * message body. The subject <tt>Formatter</tt> will see all <tt>LogRecord</tt>
 * objects that were published regardless of the current <tt>Filter</tt>. The
 * MIME type of the message body can be
 * {@linkplain FileTypeMap#setDefaultFileTypeMap overridden} by adding a MIME
 * {@linkplain MimetypesFileTypeMap entry} using the simple class name of the
 * body formatter as the file extension. The MIME type of the attachments can be
 * overridden by changing the attachment file name extension or by editing the
 * default MIME entry for a specific file name extension.
 *
 * <p>
 * <b>Attachments:</b> This <tt>Handler</tt> allows multiple attachments per
 * each email message. The presence of an attachment formatter will change the
 * content type of the email message to a multi-part message. The attachment
 * order maps directly to the array index order in this <tt>Handler</tt> with
 * zero index being the first attachment. The number of attachment formatters
 * controls the number of attachments per email and the content type of each
 * attachment. The attachment filters determine if a <tt>LogRecord</tt> will be
 * included in an attachment. If an attachment filter is <tt>null</tt> then all
 * records are included for that attachment. Attachments without content will be
 ...
 *
 * <p>
 * <b>Push Level and Push Filter:</b> The push method, push level, and optional
 * push filter can be used to conditionally trigger a push at or prior to full
 * capacity. When a push occurs, the current buffer is formatted into an email
 * and is sent to the email server. If the push method, push level, or push
 * filter trigger a push then the outgoing email is flagged as high importance
 * with urgent priority.
 *
 * <p>
 * <b>Buffering:</b> Log records that are published are stored in an internal
 * buffer. When this buffer reaches capacity the existing records are formatted
 * and sent in an email. Any published records can be sent before reaching
 * capacity by explictly calling the <tt>flush</tt>, <tt>push</tt>, or
 * <tt>close</tt> methods. If a circular buffer is required then this handler
 * can be wrapped with a {@linkplain java.util.logging.MemoryHandler} typically
 * with an equivalent capacity, level, and push level.
 *
 * <p>
 * <b>Error Handling:</b> If the transport of an email message fails, the email
 * is converted to a {@linkplain javax.mail.internet.MimeMessage#writeTo raw}
 ...
 * store,
 * {@linkplain javax.mail.internet.MimeMessage#MimeMessage( javax.mail.Session, java.io.InputStream)
 * reconstruct}, and resend the original MimeMessage. The message parameter
 * string is <b>not</b> a raw email if it starts with value returned from
 * <tt>Level.SEVERE.getName()</tt>. Custom error managers can use the following
 * test to determine if the <tt>msg</tt> parameter from this handler is a raw
 * email:
 *
 * <pre>
...
public class MailHandler extends Handler {
	 ...
	 * WebappClassLoader.clearReferencesStaticFinal() method will ignore this field.
	 */
	 ...
	 * InnocuousThread.eraseThreadLocals() can remove thread local values. The MUTEX
	 * has 5 states: 1. A null value meaning default state of not publishing. 2.
	 * MUTEX_PUBLISH on first entry of a push or publish. 3. The index of the first
	 * filter to accept a log record. 4. MUTEX_REPORT when cycle of records is
	 * detected. 5. MUTEXT_LINKAGE when a linkage error is reported.
	 */
	private static final ThreadLocal<Integer> MUTEX = new ThreadLocal<>();
	/**
	 * The marker object used to report a publishing state. This must be less than
	 * the body filter index (-1).
	 */
	private static final Integer MUTEX_PUBLISH = -2;
	/**
	 * The used for the error reporting state. This must be less than the PUBLISH
	 * state.
	 */
	private static final Integer MUTEX_REPORT = -4;
	/**
	 * The used for linkage error reporting. This must be less than the REPORT
	 * state.
	 */
	...
	/**
	 * Determines if we are inside of a push. Makes the handler properties read-only
	 * during a push.
	 */
	...
	/**
	 * Holds the session object used to generate emails. Sessions can be shared by
	 * multiple threads. See JDK-6228391 and K 6278.
	 */
	private Session session;
	/**
	 * A mapping of log record to matching filter index. Negative one is used to
	 * track the body filter. Zero and greater is used to track the attachment
	 * parts. All indexes less than or equal to the matched value have already seen
	...
	private int size;
	/**
	 * The maximum number of log records to format per email. Used to roughly bound
	 * the size of an email. Every time the capacity is reached, the handler will
	 * push. The capacity will be negative if this handler is closed. Negative
	 * values are used to ensure all records are pushed.
	 */
	private int capacity;
	/**
	 * Used to order all log records prior to formatting. The main email body and
	 * all attachments use the order determined by this comparator. If no comparator
	 * is present the log records will be in no specified order.
	 */
	private Comparator<? super LogRecord> comparator;
	/**
	 * Holds the formatter used to create the subject line of the email. A subject
	 * formatter is not required for the email message. All published records pass
	 * through the subject formatter.
	 */
	private Formatter subjectFormatter;
	/**
	 * Holds the push level for this handler. This is only required if an email must
	 * be sent prior to shutdown or before the buffer is full.
	 */
	private Level pushLevel;
	/**
	 * Holds the push filter for trigger conditions requiring an early push. Only
	 * gets called if the given log record is greater than or equal to the push
	 * level and the push level is not Level.OFF.
	 */
	private Filter pushFilter;
	/**
	 * Holds the entry and body filter for this handler. There is no way to un-seal
	 * the super handler.
	 */
	private volatile Filter filter;
	/**
	 * Holds the level for this handler. There is no way to un-seal the super
	 * handler.
	 */
	private volatile Level logLevel = Level.ALL;
	/**
	 * Holds the filters for each attachment. Filters are optional for each
	 * attachment. This is declared volatile because this is treated as
	 * copy-on-write. The VO_VOLATILE_REFERENCE_TO_ARRAY warning is a false
	 * positive.
	 */
	private volatile Filter[] attachmentFilters;
	/**
	 * Holds the encoding name for this handler. There is no way to un-seal the
	 * super handler.
	 */
	...
	/**
	 * Holds the formatters that create the content for each attachment. Each
	 * formatter maps directly to an attachment. The formatters getHead, format, and
	 * getTail methods are only called if one or more log records pass through the
	 * attachment filters.
	 */
	private Formatter[] attachmentFormatters;
	/**
	 * Holds the formatters that create the file name for each attachment. Each
	 * formatter must produce a non null and non empty name. The final file name
	 * will be the concatenation of one getHead call, plus all of the format calls,
	 * plus one getTail call.
	 */
	private Formatter[] attachmentNames;
	/**
	 * Used to override the content type for the body and set the content type for
	 * each attachment.
	 */
	private FileTypeMap contentTypes;
	/**
	 * Holds the error manager for this handler. There is no way to un-seal the
	 * super handler.
	 */
	private volatile ErrorManager errorManager = defaultErrorManager();

	/**
	 * Creates a <tt>MailHandler</tt> that is configured by the <tt>LogManager</tt>
	 * configuration properties.
	 * 
	 * @throws SecurityException if a security manager exists and the caller does
	 *                           not have <tt>LoggingPermission("control")</tt>.
	 */
	public MailHandler() {
	...
	/**
	 * Creates a <tt>MailHandler</tt> that is configured by the <tt>LogManager</tt>
	 * configuration properties but overrides the <tt>LogManager</tt> capacity with
	 * the given capacity.
	 * 
	...
	/**
	 * Creates a mail handler with the given mail properties. The key/value pairs
	 * are defined in the <tt>Java Mail API</tt> documentation. This
	 * <tt>Handler</tt> will also search the <tt>LogManager</tt> for defaults if
	 * needed.
	 * 
	 ...
	 * However it does <b>not</b> check whether the <tt>LogRecord</tt> would result
	 * in a "push" of the buffer contents.
	 * <p>
	 * 
	 ...
	 * <p>
	 * The <tt>isLoggable</tt> method is called to check if the given log record is
	 * loggable. If the given record is loggable, it is copied into an internal
	 * buffer. Then the record's level property is compared with the push level. If
	 * the given level of the <tt>LogRecord</tt> is greater than or equal to the
	 * push level then the push filter is called. If no push filter exists, the push
	 * filter returns true, or the capacity of the internal buffer has been reached
	 * then all buffered records are formatted into one email and sent to the
	 * server.
	 *
	...
	public void publish(final LogRecord record) {
		/**
		 * It is possible for the handler to be closed after the call to isLoggable. In
		 * that case, the current thread will push to ensure that all published records
		 * are sent. See close().
		 */
	...
	/**
	 * Report to the error manager that a logging loop was detected and we are going
	 * to break the cycle of messages. It is possible that a custom error manager
	 * could continue the cycle in which case we will stop trying to report errors.
	 * 
		...
		if (idx == null || idx > MUTEX_REPORT) {
			...
			try {
				...
				if (record != null) {
					...
					msg = "Log record " + record.getSequenceNumber() + " was not published. " + head(f)
							+ format(f, record) + tail(f, "");
				} else {
					msg = null;
				}
	...
	/**
	 * Used to detect reentrance by the current thread to the publish method. This
	 * mutex is thread local scope and will not block other threads. The state is
	 * advanced on if the current thread is in a reset state.
	 * 
	...
	/**
	 * Releases the mutex held by the current thread. This mutex is thread local
	 * scope and will not block other threads.
	 * 
	 * @since JavaMail 1.4.6
	 */
	...
	/**
	 * Clear previous matches when the filters are modified and there are existing
	 * log records that were matched.
	 * 
	 ...
	 * This contract is defined by the {@code org.glassfish.hk2.api.PostConstruct}
	 * interface. If this class is loaded via a lifecycle managed environment other
	 * than HK2 then it is recommended that this method is called either directly or
	 * through extending this class to signal that this object is ready for use.
	 *
	...
	/**
	 * A callback method for when this object is about to be decommissioned. This
	 * contract is defined by the {@code org.glassfish.hk2.api.PreDestory}
	 * interface. If this class is loaded via a lifecycle managed environment other
	 * than HK2 then it is recommended that this method is called either directly or
	 * through extending this class to signal that this object will be destroyed.
	 *
	 * @since JavaMail 1.5.3
	 */
	 ...
	 * urgent priority. The internal buffer is then cleared. Does nothing if called
	 * from inside a push.
	 * 
	 * @see #flush()
	 */
	...
	/**
	 * Pushes any buffered records to the email server as normal priority. The
	 * internal buffer is then cleared. Does nothing if called from inside a push.
	 * 
	 * @see #push()
	 */
	@Override
	...
	/**
	 * Prevents any other records from being published. Pushes any buffered records
	 * to the email server as normal priority. The internal buffer is then cleared.
	 * Once this handler is closed it will remain closed.
	 * <p>
	 * If this <tt>Handler</tt> is only implicitly closed by the
	 * <tt>LogManager</tt>, then <a href="#verify">verification</a> should be turned
	 * on.
	 * 
	 * @throws SecurityException if a security manager exists and the caller does
	 *                           not have <tt>LoggingPermission("control")</tt>.
		...
		try {
			...
			synchronized (this) {
				try {
					...
					/**
					 * The sign bit of the capacity is set to ensure that records that have passed
					 * isLoggable, but have yet to be added to the internal buffer, are immediately
					 * pushed as an email.
					 */
					if (this.capacity > 0) {
	 ...
	 * <tt>Filter</tt> (if it is non-null) to check if the <tt>LogRecord</tt> should
	 ...
	 *
	 * @return The encoding name. May be null, which indicates the default encoding
	 *         should be used.
	 */
	@Override
	 ...
	 * <p>
	 * The encoding should be set before any <tt>LogRecords</tt> are written to the
	 * <tt>Handler</tt>.
	 *
	 * @param encoding The name of a supported character encoding. May be null, to
	...
	/**
	 * Set the character encoding used by this handler. This method does not check
	...
	/**
	 * Gets the push level. The default is <tt>Level.OFF</tt> meaning that this
	 * <tt>Handler</tt> will only push when the internal buffer is full.
	 * 
	...
	/**
	 * Sets the push level. This level is used to trigger a push so that all pending
	 * records are formatted and sent to the email server. When the push level
	 * triggers a send, the resulting email is flagged as high importance with
	 * urgent priority.
	 * 
	 ...
	 * <tt>LogRecord</tt> level was greater than the push level. If this filter
	 * returns <tt>true</tt>, all pending records are formatted and sent to the
	 * email server. When the push filter triggers a send, the resulting email is
	 * flagged as high importance with urgent priority.
	 * 
	...
	/**
	 * Gets the comparator used to order all <tt>LogRecord</tt> objects prior to
	 * formatting. If <tt>null</tt> then the order is unspecified.
	 * 
	...
	/**
	 * Sets the comparator used to order all <tt>LogRecord</tt> objects prior to
	 * formatting. If <tt>null</tt> then the order is unspecified.
	 * 
	...
	/**
	 * Gets the number of log records the internal buffer can hold. When capacity is
	 * reached, <tt>Handler</tt> will format all <tt>LogRecord</tt> objects into one
	 * email message.
	 * 
	 * @return the capacity.
	 */
	 ...
	 * Sets the <tt>Authenticator</tt> used to login to the email server.
	 * 
	 * @param password a password, empty array can be used to only supply a user
	 *                 name set by <tt>mail.user</tt> property, or null if no
	...
	/**
	 * Sets the mail properties used for the session. The key/value pairs are
	 * defined in the <tt>Java Mail API</tt> documentation. This <tt>Handler</tt>
	 * will also search the <tt>LogManager</tt> for defaults if needed.
	 * 
	 * @param props a non <tt>null</tt> properties object.
	 * @throws SecurityException     if a security manager exists and the caller
	 *                               does not have
	...
	/**
	 * A private hook to handle overrides when the public method is declared non
	 * final. See public method for details.
	 * 
	...
	/**
	 * Gets the attachment filters. If the attachment filter does not allow any
	 * <tt>LogRecord</tt> to be formatted, the attachment may be omitted from the
	 * email.
	 * 
	 ...
	 * 
	 * @param filters a non <tt>null</tt> array of filters. A <tt>null</tt> index
	 *                value is allowed. A <tt>null</tt> value means that all records
	 *                are allowed for the attachment at that index.
	 * @throws SecurityException         if a security manager exists and the caller
	 *                                   does not have
	 *                                   <tt>LoggingPermission("control")</tt>.
	 * @throws NullPointerException      if <tt>filters</tt> is <tt>null</tt>
	 * @throws IndexOutOfBoundsException if the number of attachment name formatters
	 *                                   do not match the number of attachment
	 *                                   formatters.
	 * @throws IllegalStateException     if called from inside a push.
	 */
	...
	/**
	 * Gets the attachment formatters. This <tt>Handler</tt> is using attachments
	 * only if the returned array length is non zero.
	 * 
	...
	/**
	 * Sets the attachment <tt>Formatter</tt> object for this handler. The number of
	 * formatters determines the number of attachments per email. This method should
	 * be the first attachment method called. To remove all attachments, call this
	...
	public final void setAttachmentFormatters(Formatter... formatters) {
		...
		if (formatters.length == 0) { // Null check and length check.
			...
			formatters = Arrays.copyOf(formatters, formatters.length, Formatter[].class);
	...
	/**
	 * Gets the attachment name formatters. If the attachment names were set using
	 * explicit names then the names can be returned by calling <tt>toString</tt> on
	 * each attachment name formatter.
	 * 
	...
	/**
	 * Sets the attachment file name for each attachment. All control characters are
	 * removed from the attachment names. This method will create a set of custom
	 * formatters.
	 * 
	 ...
	 *                                   does not have
	 *                                   <tt>LoggingPermission("control")</tt>.
	 * @throws IndexOutOfBoundsException if the number of attachment names do not
	 *                                   match the number of attachment formatters.
	 ...
	 * attachment formatter will see only the <tt>LogRecord</tt> objects that passed
	 * its attachment filter during formatting. The format method will typically
	 * return an empty string. Instead of being used to format records, it is used
	 * to gather information about the contents of an attachment. The
	 * <tt>getTail</tt> method should be used to construct the attachment file name
	 * and reset any formatter collected state. All control characters will be
	 * removed from the output of the formatter. The <tt>toString</tt> method of the
	 * given formatter should be overridden to provide a useful attachment file
	 * name, if possible.
	 * 
	...
	/**
	 * Gets the formatter used to create the subject line. If the subject was
	 * created using a literal string then the <tt>toString</tt> method can be used
	 ...
	 * formatter will see all <tt>LogRecord</tt> objects that were published to this
	 * <tt>Handler</tt> during formatting and will typically return an empty string.
	 * This formatter is used to gather information to create a summary about what
	 * information is contained in the email. The <tt>getTail</tt> method should be
	 * used to construct the subject and reset any formatter collected state. All
	 * control characters will be removed from the formatter output. The
	 * <tt>toString</tt> method of the given formatter should be overridden to
	 * provide a useful subject, if possible.
	 * 
	 ...
	 * <tt>Level.SEVERE.getName()</tt>. This allows the receiving error manager to
	 * determine if the <tt>msg</tt> parameter is a simple error message or a raw
	 * email message.
	 * 
	...
	protected void reportError(String msg, Exception ex, int code) {
		try {
			if (msg != null) {
				errorManager.error(Level.SEVERE.getName().concat(": ").concat(msg), ex, code);
			} else {
	...
	/**
	 * Determines the mimeType of a formatter from the getHead call. This could be
	 * made protected, or a new class could be created to do this type of
	 * conversion. Currently, this is only used for the body since the attachments
	 * are computed by filename. Package-private for unit testing.
	 * 
	...
	/**
	 * Determines the mimeType of a formatter by the class name. This method avoids
	 * calling getHead and getTail of content formatters during verify because they
	 * might trigger side effects or excessive work. The name formatters and subject
	 * are usually safe to call. Package-private for unit testing.
	 *
	...
	final String contentTypeOf(final Formatter f) {
		...
		if (f != null) {
			...
			for (Class<?> k = f.getClass(); k != Formatter.class; k = k.getSuperclass()) {
				...
				for (int idx = name.indexOf('$') + 1; (idx = name.indexOf("ml", idx)) > -1; idx += 2) {
					if (idx > 0) {
	...
	/**
	 * Determines if the given throwable is a no content exception. It is assumed
	 * Transport.sendMessage will call Message.writeTo so we need to ignore any
	 * exceptions that could be layered on top of that call chain to infer that
	 * sendMessage is failing because of writeTo. Package-private for unit testing.
	 * 
	...
	/**
	 * Converts a mime message to a raw string or formats the reason why message
	 * can't be changed to raw string and reports it.
	 * 
	 ...
	 *
	 * The current LogManager code will stop closing all remaining handlers if an
	 * error is thrown during resetLogger. This is a workaround for GLASSFISH-21258
	 * and JDK-8152515.
	 * 
	...
	/**
	 * Determines the mimeType from the given file name. Used to override the body
	 * content type and used for all attachments.
	 * 
	...
	/**
	 * Sets the capacity for this handler. This method is kept private because we
	 * would have to define a public policy for when the size is greater than the
	 * capacity. E.G. do nothing, flush now, truncate now, push now and resize.
	 * 
	...
	/**
	 * Gets the attachment filters using a happens-before relationship between this
	 * method and setAttachmentFilters. The attachment filters are treated as
	 * copy-on-write, so the returned array must never be modified or published
	 * outside this class.
	 * 
	...
	private boolean alignAttachmentNames() {
		...
		if (current != expect) {
			this.attachmentNames = Arrays.copyOf(attachmentNames, expect, Formatter[].class);
			fixed = current != 0;
		}
	...
	private boolean alignAttachmentFilters() {
		...
		if (current != expect) {
			this.attachmentFilters = Arrays.copyOf(attachmentFilters, expect, Filter[].class);
	 ...
	 * 
	 * @param props the given mail properties. Maybe null and are never captured by
	 *              this handler.
	 * @throws SecurityException if a security manager exists and the caller does
	 *                           not have <tt>LoggingPermission("control")</tt>.
	 */
	...
	/**
	 * Interns the error manager, formatters, and filters contained in this handler.
	 * The comparator is not interned. This method can only be called from init
	 * after all of formatters and filters are in a constructed and in a consistent
	 * state.
	 * 
	...
	private void intern() {
		...
		try {
			...
			reportError(skip.getMessage(), new InvocationTargetException(skip), ErrorManager.OPEN_FAILURE);
		}
	}

	/**
	 * If possible performs an intern of the given object into the map. If the
	 * object can not be interned the given object is returned.
	 * 
	 ...
	 * @throws SecurityException if this operation is not allowed by the security
	 *                           manager.
	...
	private Object intern(Map<Object, Object> m, Object o) throws Exception {
		...
		/**
		 * The common case is that most objects will not intern. The given object has a
		 * public no argument constructor or is an instance of a TailNameFormatter.
		 * TailNameFormatter is safe use as a map key. For everything else we create a
		 * clone of the given object. This is done because of the following: 1. Clones
		 * can be used to test that a class provides an equals method and that the
		 * equals method works correctly. 2. Calling equals on the given object is
		 * assumed to be cheap. 3. The intern map can be filtered so it only contains
		 * objects that can be interned, which reduces the memory footprint. 4. Clones
		 * are method local garbage. 5. Hash code is only called on the clones so bias
		 * locking is not disabled on the objects the handler will use.
		 */
		final Object key;
	...
	private void initAttachmentFilters(final String p) {
		...
		if (!isEmpty(list)) {
			...
			if (alignAttachmentFilters()) {
				reportError("Attachment filters.", attachmentMismatch("Length mismatch."), ErrorManager.OPEN_FAILURE);
			}
		} else {
			this.attachmentFilters = emptyFilterArray();
	...
	private void initAttachmentNames(final String p) {
		...
		if (!isEmpty(list)) {
			...
			if (alignAttachmentNames()) { // Any null indexes are repaired.
				reportError("Attachment names.", attachmentMismatch("Length mismatch."), ErrorManager.OPEN_FAILURE);
			}
		} else {
			this.attachmentNames = emptyFormatterArray();
	...
	/**
	 * Check if any attachment would actually format the given <tt>LogRecord</tt>.
	 * This method does not check if the handler is level is set to OFF or if the
	 * handler is closed.
	 * 
	 * @param record a <tt>LogRecord</tt>
	...
	/**
	 * Used to send the generated email or write its contents to the error manager
	 * for this handler. This method does not hold any locks so new records can be
	 * added to this handler during a send or failure.
	 * 
	...
	/**
	 * Performs a sort on the records if needed. Any exception thrown during a sort
	 * is considered a formatting error.
	 */
	private void sort() {
		assert Thread.holdsLock(this);
		if (comparator != null) {
	...
	/**
	 * Formats all records in the buffer and places the output in a Message. This
	 * method under most conditions will catch, report, and continue when exceptions
	 * occur. This method holds a lock on this handler.
	 * 
	 * @param code the error manager code.
	 * @return null if there are no records or is currently in a push. Otherwise a
	 *         new message is created with a formatted message and attached session.
	 */
	...
	private Message writeLogRecords0() throws Exception {
		...
		/**
		 * Parts are lazily created when an attachment performs a getHead call.
		 * Therefore, a null part at an index means that the head is required.
		 */
		...
		final MimePart body;
		if (parts.length == 0) {
			msg.setDescription(descriptionFrom(getFormatter(), getFilter(), subjectFormatter));
			body = msg;
		} else {
	...
	/**
	 * Checks all of the settings if the caller requests a verify and a verify was
	 * not performed yet and no verify is in progress. A verify is performed on
	 * create because this handler may be at the end of a handler chain and
	 * therefore may not see any log records until LogManager.reset() is called and
	 * at that time all of the settings have been cleared.
	 * 
	...
	/**
	 * Checks all of the settings using the given setting. This triggers the
	 * LogManagerProperties to copy all of the mail settings without explictly
	 * knowing them. Once all of the properties are copied this handler can handle
	 * LogManager.reset clearing all of the properties. It is expected that this
	 * method is, at most, only called once per session.
	 * 
	...
	private void verifySettings0(Session session, String verify) {
		...
		if (!"local".equals(verify) && !"remote".equals(verify) && !"limited".equals(verify)
			...
			reportError("Verify must be 'limited', local', " + "'resolve', 'login', or 'remote'.",
					new IllegalArgumentException(verify), ErrorManager.OPEN_FAILURE);
			return;
		}
				...
				for (int i = 0; i < from.length; ++i) {
					if (from[i].equals(sender)) {
						MessagingException ME = new MessagingException(
								"Sender address '" + sender + "' equals from address.");
	...
	/**
	 * Handles all exceptions thrown when save changes is called on a message that
	...
	/**
	 * Cache common session properties into the LogManagerProperties. This is a
	 * workaround for JDK-7092981.
	 *
	 ...
	 * @throws SecurityException if security manager is present and doesn't allow
	 *                           access to check connect permission.
	 * @since JavaMail 1.5.0
	 */
	...
	/**
	 * Calls validate for every address given. If the addresses given are null,
	 * empty or not an InternetAddress then the check is skipped.
	 * 
	...
	/**
	 * Creates and sets the message content from the given Throwable. When verify
	 * fails, this method fixes the 'abort' message so that any created envelope
	 * data can be used in the error manager.
	 * 
	...
	/**
	 * Used to update the cached session object based on changes in mail properties
	 * or authenticator.
	 * 
	...
	/**
	 * Creates all of the envelope information for a message. This method is safe to
	 * call outside of a lock because the message provides the safe snapshot of the
	 * mail properties.
	 * 
	...
	private MimeBodyPart createBodyPart(int index) throws MessagingException {
		...
		part.setDescription(
				descriptionFrom(attachmentFormatters[index], attachmentFilters[index], attachmentNames[index]));
		setAcceptLang(part);
		return part;
	}

	/**
	 * Gets the description for the MimeMessage itself. The push level and filter
	 * are included because they play a role in formatting of a message when
	...
	private String descriptionFrom(Formatter f, Filter filter, Formatter name) {
		return "Formatted using " + getClassId(f) + ", filtered with "
				+ (filter == null ? "no filter" : filter.getClass().getName()) + ", and named by " + getClassId(name)
				+ '.';
	}

	/**
	 * Gets a class name represents the behavior of the formatter. The class name
	 * may not be assignable to a Formatter.
	 * 
	...
	/**
	 * Constructs a file name from a formatter. This method is called often but,
	...
	/**
	 * It is assumed that file names are short and that in most cases getTail will
	 * be the only method that will produce a result.
	 * 
	 * @param part  to append to.
	 * @param chunk non null string to append.
	 */
	...
	/**
	 * It is assumed that subject lines are short and that in most cases getTail
	 * will be the only method that will produce a result.
	 * 
	 * @param msg   to append to.
	 * @param chunk non null string to append.
	 */
	...
	/**
	 * Gets the locale for the given log record from the resource bundle. If the
	 * resource bundle is using the root locale then the default locale is returned.
	 * 
	...
	/**
	 * Appends the content language to the given mime part. The language tag is only
	 * appended if the given language has not been specified. This method is only
	 * used when we have LogRecords that are localized with an assigned resource
	 * bundle.
	 * 
	...
	/**
	 * Sets the accept language to the default locale of the JVM. If the locale is
	 * the root locale the header is not added.
	 * 
	...
	/**
	 * Used when a log record was loggable prior to being inserted into the buffer
	 * but at the time of formatting was no longer loggable. Filters were changed
	 * after publish but prior to a push or a bug in the body filter or one of the
	 * attachment filters.
	 * 
	...
	private void reportFilterError(final LogRecord record) {
		assert Thread.holdsLock(this);
		final Formatter f = createSimpleFormatter();
		final String msg = "Log record " + record.getSequenceNumber() + " was filtered from all message parts.  "
	...
	/**
	 * Reports equals implementations that do not discriminate between objects of
	 * different types or subclass types.
	 * 
	 * @param o     the test object must be non null.
	 * @param found the possible intern, must be non null.
	 * @throws NullPointerException if any argument is null.
	...
	private void reportNonDiscriminating(final Object o, final Object found) {
		reportError("Non discriminating equals implementation.",
				new IllegalArgumentException(
						o.getClass().getName() + " should not be equal to " + found.getClass().getName()),
				ErrorManager.OPEN_FAILURE);
	}

	/**
	 * Used to outline the bytes to report a null pointer exception. See BUD ID
	 * 6533165.
	 * 
	 * @param code the ErrorManager code.
	 */
	...
	/**
	 * Used to signal that body parts are missing from a message. Also used when
	 * LogRecords were passed to an attachment formatter but the formatter produced
	 * no output, which is allowed. Used during a verify because all parts are
	 * omitted, none of the content formatters are used. This is not used when a
	 * filter prevents LogRecords from being formatted. This header is defined in
	 * RFC 2156 and RFC 4021.
	 * 
	...
	/**
	 * Signals that this message was generated by automatic process. This header is
	 * defined in RFC 3834 section 5.
	 * 
	 * @param msg the message.
	 * @since JavaMail 1.4.6
	 */
	...
	/**
	 * Computes the default to-address if none was specified. This can fail if the
	 * local address can't be computed.
	 * 
	 * @param msg  the message
		...
		if (!isEmpty(sender)) {
			try {
				...
				if (address.length > 0) {
					...
					if (address.length > 1) {
						reportError("Ignoring other senders.", tooManyAddresses(address, 1),
	...
	/**
	 * Converts an email message to a raw string. This raw string is passed to the
	 * error manager to allow custom error managers to recreate the original
	 * MimeMessage object.
	 * 
	...
	private String toMsgString(final Throwable t) {
		...
		final String charset = getEncodingName();
		try {
			final ByteArrayOutputStream out = new ByteArrayOutputStream(MIN_HEADER_SIZE);

			// Create an output stream writer so streams are not double buffered.
			try (OutputStreamWriter ows = new OutputStreamWriter(out, charset); PrintWriter pw = new PrintWriter(ows)) {
	 ...
	 * 
	 * @param ccl null for boot class loader, a class loader, a class used to get
	 *            the class loader, or a source object to get the class loader.
	 * @return null for the boot class loader, a class loader, or a marker object to
	 *         signal that no modification was required.
	 * @since JavaMail 1.5.3
	 */
	...
	private String getLocalHost(final Service s) {
		...
		} catch (SecurityException | NoSuchMethodException | LinkageError ignore) {
	...
	private static final class DefaultAuthenticator extends Authenticator {
		...
		/**
		 * Creates an Authenticator for the given password. This method is used so class
		 * verification of assignments in MailHandler doesn't require loading this class
		 * which otherwise can occur when using the constructor. Default access to avoid
		 * generating extra class files.
		 *
	...
	private static final class GetAndSetContext implements PrivilegedAction<Object> {
		 ...
		 * Create the action.
		 * 
		 * @param source null for boot class loader, a class loader, a class used to get
		 *               the class loader, or a source object to get the class loader.
		 *               Default access to avoid generating extra class files.
		 */
		...
		/**
		 * Gets the class loader from the source and sets the CCL only if the source and
		 * CCL are not the same.
		 * 
		 * @return the replaced context class loader which can be null or NOT_MODIFIED
	...
	private static final class TailNameFormatter extends Formatter {
		...
		/**
		 * Creates or gets a formatter from the given name. This method is used so class
		 * verification of assignments in MailHandler doesn't require loading this class
		 * which otherwise can occur when using the constructor. Default access to avoid
		 * generating extra class files.
		 *
