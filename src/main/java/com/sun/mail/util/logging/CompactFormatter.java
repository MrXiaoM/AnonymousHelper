 ...
 * LogManager configuration properties where <tt>&lt;formatter-name&gt;</tt>
 * refers to the fully qualified class name or the fully qualified derived class
 * name of the formatter. If properties are not defined, or contain invalid
...
public class CompactFormatter extends java.util.logging.Formatter {
	 ...
	 *
	 * @param format the {@linkplain java.util.Formatter pattern} or null to use the
	 *               LogManager default. The arguments are described in the
	 *               {@linkplain #format(java.util.logging.LogRecord) format}
	 *               method.
	 */
	 ...
	 *
	 * <ol start='0'>
	 * <li>{@code format} - the {@linkplain java.util.Formatter java.util.Formatter}
	 * format string specified in the &lt;formatter-name&gt;.format property or the
	 * format that was given when this formatter was created.</li>
	 ...
	 * <li>{@code logger} - the logger's {@linkplain Class#getSimpleName() simple}
	 ...
	 * {@linkplain LogRecord#getThrown throwable} associated with the log record and
	 * a relevant stack trace element if available; otherwise, an empty string is
	 * used.</li>
	 * <li>{@code message|thrown} The message and the thrown properties joined as
	 * one parameter. This parameter supports
	 * {@linkplain #toAlternate(java.lang.String) alternate} form.</li>
	 * <li>{@code thrown|message} The thrown and message properties joined as one
	 * parameter. This parameter supports {@linkplain #toAlternate(java.lang.String)
	 * alternate} form.</li>
	 * <li>{@code sequence} the {@linkplain LogRecord#getSequenceNumber() sequence
	 * number} if the given log record.</li>
	 * <li>{@code thread id} the {@linkplain LogRecord#getThreadID() thread id} of
	 * the given log record. By default this is formatted as a {@code long} by an
	 * unsigned conversion.</li>
	 * <li>{@code error} the throwable {@linkplain Class#getSimpleName() simple
	 * class name} and {@linkplain #formatError(LogRecord) error message} without
	 * any stack trace.</li>
	 ...
	 * parameter. This parameter supports {@linkplain #toAlternate(java.lang.String)
	 * alternate} form.</li>
	 * <li>{@code backtrace} only the {@linkplain #formatBackTrace(LogRecord) stack
	 * trace} of the given throwable.</li>
	 * <li>{@code bundlename} the resource bundle
	 * {@linkplain LogRecord#getResourceBundleName() name} of the given log
	 * record.</li>
	 * <li>{@code key} the {@linkplain LogRecord#getMessage() raw message} before
	 * localization or formatting.</li>
	 * </ol>
	 *
	 * <p>
	 ...
	 * <p>
	 * This prints only 160 characters of the message|thrown ({@code 7$}) using the
	 * {@linkplain #toAlternate(java.lang.String) alternate} form. The separator is
	 * not included as part of the total width.
	 * 
	 * <pre>
	 ...
	 * <p>
	 * This prints only 20 characters of the message|thrown ({@code 7$}) using the
	 * {@linkplain #toAlternate(java.lang.String) alternate} form. This will perform
	 * a weighted truncation of both the message and thrown properties of the log
	 * record. The separator is not included as part of the total width.
	 * 
	 * <pre>
	 ...
	 * time, thread id ({@code 10$}), source ({@code 2$}), log message ({@code 5$}),
	 * and the throwable with back trace ({@code 6$}).
	 * 
	 * <pre>
	...
	public String format(final LogRecord record) {
		...
		Object[] params = { formatZonedDateTime(record), formatSource(record), formatLoggerName(record),
				formatLevel(record), msg, thrown, new Alternate(msg, thrown), new Alternate(thrown, msg),
				record.getSequenceNumber(), formatThreadID(record), err, new Alternate(msg, err),
	...
	/**
	 * Formats message for the log record. This method removes any fully qualified
	 * throwable class names from the message.
	 *
	...
	/**
	 * Formats the message from the thrown property of the log record. This method
	 * replaces fully qualified throwable class names from the message cause chain
	 * with simple class names.
	 *
	 * @param t the throwable to format or null.
	 * @return the empty string if null was given or the formatted message string
	 *         from the throwable which may be null.
	 */
	...
	/**
	 * Formats the thread id property of the given log record. By default this is
	...
	public Number formatThreadID(final LogRecord record) {
		/**
		 * Thread.getID is defined as long and LogRecord.getThreadID is defined as int.
		 * Convert to unsigned as a means to better map the two types of thread
		 * identifiers.
		 */
	...
	/**
	 * Formats the thrown property of a LogRecord. The returned string will contain
	 * a throwable message with a back trace.
	 *
	 * @param record the record.
	...
	/**
	 * Formats the thrown property of a LogRecord as an error message. The returned
	 * string will not contain a back trace.
	 *
	 * @param record the record.
	 * @return empty string if nothing was thrown or formatted string.
	 * @throws NullPointerException if the given record is null.
	...
	/**
	 * Chooses a single throwable from the cause chain that will be formatted. This
	 * implementation chooses the throwable that best describes the chain.
	...
	/**
	 * Determines if a stack frame should be ignored as the cause of an error. This
	 * does not check for unknown line numbers because code can be compiled without
	 * debugging info.
	 *
	 * @param s the stack trace element.
	 * @return true if this frame should be ignored.
	...
	/**
	 * Determines if a stack trace element has an unknown line number or a native
	 * line number.
	 *
	 * @param s the stack trace element.
	...
	/**
	 * Searches the given message for all instances fully qualified class name with
	 * simple class name based off of the types contained in the given parameter
	 * array.
	 *
	...
	/**
	 * Gets the simple class name from the given class. This is a workaround for BUG
	 * ID JDK-8057919.
	 *
	...
	/**
	 * Converts a fully qualified class name to a simple class name. If the leading
	 * part of the given string is not a legal class name then the given string is
	 * returned.
	 *
	...
	private static String simpleClassName(String name) {
		if (name != null) {
			...
			int sign = -1;
			int dot = -1;
			for (int c, prev = dot; cursor < name.length(); cursor += Character.charCount(c)) {
	...
	/**
	 * Converts a file name with an extension to a file name without an extension.
	 *
	...
	private class Alternate implements java.util.Formattable {
		...
		public void formatTo(java.util.Formatter formatter, int flags, int width, int precision) {
			...
			String l = left;
			String r = right;
			if ((flags & java.util.FormattableFlags.UPPERCASE) == java.util.FormattableFlags.UPPERCASE) {
				l = l.toUpperCase(formatter.locale());
				r = r.toUpperCase(formatter.locale());
			}

			if ((flags & java.util.FormattableFlags.ALTERNATE) == java.util.FormattableFlags.ALTERNATE) {
				l = toAlternate(l);
				r = toAlternate(r);
			}
		...
		private String pad(int flags, String s, int length) {
			...
			if ((flags & java.util.FormattableFlags.LEFT_JUSTIFY) == java.util.FormattableFlags.LEFT_JUSTIFY) {
