...
public class LineInputStream extends FilterInputStream {
	...
	/**
	 * Read a line containing only ASCII characters from the input stream. A line is
	...
	public String readLine() throws IOException {
		...
		while ((c1 = in.read()) != -1) {
			if (c1 == '\n') // Got NL, outa here.
				...
				if (c2 != '\n') {
					/*
					 * If the stream supports it (which we hope will always be the case), reset to
					 * after the first CR. Otherwise, we wrap a PushbackInputStream around the
					 * stream so we can unread the characters we don't need. The only problem with
					 * that is that the caller might stop reading from this LineInputStream, throw
					 * it away, and then start reading from the underlying stream. If that happens,
					 * the pushed back characters will be lost forever.
					 */
					if (in.markSupported())
						in.reset();
					else {
