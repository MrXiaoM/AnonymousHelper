...
public class UUDecoderStream extends FilterInputStream {
	...
	/**
	 * Create a UUdecoder that decodes the specified input stream. The System
	 * property <code>mail.mime.uudecode.ignoreerrors</code> controls whether errors
	 * in the encoded data cause an exception or are ignored. The default is false
	 * (errors cause exception). The System property
	 * <code>mail.mime.uudecode.ignoremissingbeginend</code> controls whether a
	 * missing begin or end line cause an exception or are ignored. The default is
	 * false (errors cause exception).
	 * 
	...
	/**
	 * Read the next decoded byte from this input stream. The byte is returned as an
	 * <code>int</code> in the range <code>0</code> to <code>255</code>. If no byte
	...
	/**
	 * Get the "name" field from the prefix. This is meant to be the pathname of the
	 * decoded file
	 *
		...
		for (;;) {
			...
			if (line.regionMatches(false, 0, "begin", 0, 5)) {
				...
				if (need == 0 || line.length() >= need + 1) {
					/*
					 * Looks like a legitimate encoded line. Pretend we saw the "begin" line and
					 * save this line for later processing in decode().
					 */
					readAhead = line;
	...
	private boolean decode() throws IOException {
		...
		for (;;) {
			/*
			 * If we ignored a missing "begin", the first line will be saved in readAhead.
			 */
			if (readAhead != null) {
				line = readAhead;
				readAhead = null;
			...
			/*
			 * Improperly encoded data sometimes omits the zero length line that starts with
			 * a space character, we detect the following "end" line here.
			 */
			if (line == null) {
				if (!ignoreMissingBeginEnd)
			 ...
			 * atoms) characters in the line. Note that all the code below has to handle the
			 * <SPACE> character that indicates end of encoded stream.
			 */
			count = (count - ' ') & 0x3f;
...
//System.out.println("count " + count + ", need " + need + ", len " + line.length());
		 ...
		 * uuencoders we handle the case where they don't include these "unnecessary"
		 * characters.
		 */
		while (bufsize < count) {
	 ...
	 * System.out.flush(); } catch (Exception e) { e.printStackTrace(); } } end TEST
	 * program
	 ****/
}
