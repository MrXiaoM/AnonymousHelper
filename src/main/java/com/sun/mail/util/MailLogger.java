...
/**
 * A simplified logger used by JavaMail to handle logging to a PrintStream and
 * logging through a java.util.logging.Logger. If debug is set, messages are
 * written to the PrintStream and prefixed by the specified prefix (which is not
 * included in Logger messages). Messages are logged by the Logger based on the
 * configuration of the logging system.
 */

/*
 * It would be so much simpler to just subclass Logger and override the
 * log(LogRecord) method, as the javadocs suggest, but that doesn't work because
 * Logger makes the decision about whether to log the message or not before
 * calling the log(LogRecord) method. Instead, we just provide the few log
...
public final class MailLogger {
	...
	/**
	 * Construct a new MailLogger using the specified Logger name, debug prefix
	 * (e.g., "DEBUG"), debug flag, and PrintStream.
	 *
	...
	/**
	 * Construct a new MailLogger using the specified class' package name as the
	 * Logger name, debug prefix (e.g., "DEBUG"), debug flag, and PrintStream.
	 *
	...
	/**
	 * Construct a new MailLogger using the specified class' package name combined
	 * with the specified subname as the Logger name, debug prefix (e.g., "DEBUG"),
	 * debug flag, and PrintStream.
	 *
	 * @param clazz   the Logger name is the package name of this class
	...
	/**
	 * Construct a new MailLogger using the specified Logger name and debug prefix
	 * (e.g., "DEBUG"). Get the debug flag and PrintStream from the Session.
	 *
	 * @param name    the Logger name
	 * @param prefix  the prefix for debug output, or null for none
	...
	/**
	 * Construct a new MailLogger using the specified class' package name as the
	 * Logger name and the specified debug prefix (e.g., "DEBUG"). Get the debug
	 * flag and PrintStream from the Session.
	 *
	 * @param clazz   the Logger name is the package name of this class
	 * @param prefix  the prefix for debug output, or null for none
	 * @param session where to get the debug flag and PrintStream
	 ...
	 * The new MailLogger uses the same debug flag and PrintStream as this
	 * MailLogger.
	 *
	 * @param name   the Logger name
	 * @param prefix the prefix for debug output, or null for none
	...
	/**
	 * Create a MailLogger using the specified class' package name as the Logger
	 * name and the specified prefix. The new MailLogger uses the same debug flag
	 * and PrintStream as this MailLogger.
	 *
	 * @param clazz  the Logger name is the package name of this class
	 * @param prefix the prefix for debug output, or null for none
	 * @return a MailLogger for the given name and prefix.
	...
	/**
	 * Create a MailLogger that uses a Logger whose name is composed of this
	 * MailLogger's name plus the specified sub-name, separated by a dot. The new
	 * MailLogger uses the new prefix for debug output. This is used primarily by
	 * the protocol trace code that wants a different prefix (none).
	 *
	 * @param subname the Logger name relative to this Logger name
	 * @param prefix  the prefix for debug output, or null for none
	 * @return a MailLogger for the given name and prefix.
	...
	public MailLogger getSubLogger(String subname, String prefix) {
		return new MailLogger(logger.getName() + "." + subname, prefix, debug, out);
	}
	...
	/**
	 * If "debug" is set, or our embedded Logger is loggable at the given level,
	 * return true.
	 * 
	 * @param level the log level.
	...
	/**
	 * Return the package name of the class. Sometimes there will be no Package
	 * object for the class, e.g., if the class loader hasn't created one (see
	 * Class.getPackage()).
	 * 
	...
	/**
	 * A disadvantage of not being able to use Logger directly in JavaMail code is
	 * that the "source class" information that Logger guesses will always refer to
	 * this class instead of our caller. This method duplicates what Logger does to
	 * try to find *our* caller, so that Logger doesn't have to do it (and get the
	 * wrong answer), and because our caller is what's wanted.
	 * 
