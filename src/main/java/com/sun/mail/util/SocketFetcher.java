...
public class SocketFetcher {
	...
	/**
	 * This method returns a Socket. Properties control the use of socket factories
	 * and other socket characteristics. The properties used are:
	 * <ul>
	 ...
	 * </ul>
	 * <p>
	 * If we're making an SSL connection, the ssl.socketFactory properties are used
	 * first, if set.
	 * <p>
	 *
	 ...
	 * qualified classname of a javax.net.SocketFactory subclass. In this case, the
	 * class is dynamically instantiated and a socket created by that SocketFactory
	 * is returned.
	 * <p>
	 *
	 ...
	 * <p>
	 *
	 * If the connectiontimeout property is set, the timeout is passed to the socket
	 * connect method.
	 * <p>
	 *
	 ...
	 * to. If the localport property is also set, it's used as the local port number
	 * to bind to.
	 * <p>
	 *
	 * If the usesocketchannels property is set, and we create the Socket ourself,
	 * and the selection of other properties allows, create a SocketChannel and get
	 * the Socket from it. This allows us to later retrieve the SocketChannel from
	 * the Socket and use it with Select.
	 *
	...
	public static Socket getSocket(String host, int port, Properties props, String prefix, boolean useSSL)
		...
		if (logger.isLoggable(Level.FINER))
			logger.finer(
					"getSocket" + ", host " + host + ", port " + port + ", prefix " + prefix + ", useSSL " + useSSL);
		if (prefix == null)
			prefix = "socket";
		if (props == null)
		...
		try {
			/*
			 * If using SSL, first look for SSL-specific class name or factory instance.
			 */
	...
	/**
	 * Create a socket with the given local address and connected to the given host
	 * and port. Use the specified connection timeout and read timeout. If a socket
	 * factory is specified, use it. Otherwise, use the SSLSocketFactory if useSSL
	 * is true.
	 */
	private static Socket createSocket(InetAddress localaddr, int localport, String host, int port, int cto, int to,
		...
		if (socket == null) {
			if (socksHost != null) {
				socket = new Socket(
						new java.net.Proxy(java.net.Proxy.Type.SOCKS, new InetSocketAddress(socksHost, socksPort)));
		...
		/*
		 * If we want an SSL connection and we didn't get an SSLSocket, wrap our plain
		 * Socket with an SSLSocket.
		 */
		...
		/*
		 * No matter how we created the socket, if it turns out to be an SSLSocket,
		 * configure it.
		 */
	 ...
	 * protocols. This version for compatibility with possible third party code that
	...
	private static void configureSSLSocket(Socket socket, String host, Properties props, String prefix,
		...
		if (protocols != null)
			...
			/*
			 * The UW IMAP server insists on at least the TLSv1 protocol for STARTTLS, and
			 * won't accept the old SSLv2 or SSLv3 protocols. Here we enable only the
			 * non-SSL protocols. XXX - this should probably be parameterized.
			 */
		...
		/*
		 * Force the handshake to be done now so that we can report any errors (e.g.,
		 * certificate errors) to the caller of the startTLS method.
		 */
	...
	private static boolean matchCert(String server, X509Certificate cert) {
		...
		/*
		 * First, try to use sun.security.util.HostnameChecker, which exists in Sun's
		 * JDK starting with 1.4.1. We use reflection to access it in case it's not
		 * available in the JDK we're running on.
		 */
		try {
		...
		/*
		 * Lacking HostnameChecker, we implement a crude version of the same checks
		 * ourselves.
		 */
		try {
			/*
			 * Check each of the subjectAltNames. XXX - only checks DNS names, should also
			 * handle case where server name is a literal IP address
			 */
	...
	/**
	 * Does the server we're expecting to connect to match the given name from the
	...
	/**
	 * Use the HTTP CONNECT protocol to connect to a site through an HTTP proxy
	 * server.
	 * <p>
	 *
	 * Protocol is roughly:
	 ...
	 * </pre>
	 */
	private static void proxyConnect(Socket socket, String proxyHost, int proxyPort, String host, int port, int cto)
			throws IOException {
	...
	/**
	 * Convenience method to get our context class loader. Assert any privileges we
	 * might have and then call the Thread.getContextClassLoader method.
	 */
