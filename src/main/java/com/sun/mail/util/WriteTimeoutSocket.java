...
/**
 * A special Socket that uses a ScheduledExecutorService to implement timeouts
 * for writes. The write timeout is specified (in milliseconds) when the
 * WriteTimeoutSocket is created.
 *
 * @author Bill Shannon
 */
...
/**
 * An OutputStream that wraps the Socket's OutputStream and uses the
 * ScheduledExecutorService to schedule a task to close the socket (aborting the
 * write) if the timeout expires.
 */
