...
public class Ntlm {
	...
	/**
	 * Create an NTLM authenticator. Username may be specified as domain\\username
	 * in the Authenticator. If this notation is not used, then the domain will be
	 * taken from the ntdomain parameter.
	 *
	...
	/**
	 * Convert a 7 byte array to an 8 byte array (for a des key with parity). Input
	...
	/*
	 * Key is a 21 byte array. Split it into 3 7 byte chunks, convert each to 8 byte
	 * DES keys, encrypt the text arg with each key and return the three results in
	 * a sequential [].
	 */
