...
public class SMTPTransport extends Transport {
	 ...
	 *
	 * @param authzid the authorization ID to use for authentication.
	 *
	 * @since JavaMail 1.4.4
	 */
	...
	/**
	 * Get the list of SASL mechanisms to consider if SASL authentication is
	 * enabled. If the list is empty or null, all available SASL mechanisms are
	 * considered.
	 *
	...
	/**
	 * Set the list of SASL mechanisms to consider if SASL authentication is
	 * enabled. If the list is empty or null, all available SASL mechanisms are
	 * considered.
	 *
	...
	/**
	 * Is the NOOP command required to return a response code of 250 to indicate
	 * success?
	 *
	...
	/**
	 * Set whether the NOOP command is required to return a response code of 250 to
	 * indicate success.
	 *
	...
	protected synchronized boolean protocolConnect(String host, int port, String user, String password)
		...
		/*
		 * If anything goes wrong, we need to be sure to close the connection.
		 */
	...
	private boolean authenticate(String user, String passwd) throws MessagingException {
		...
		while (st.hasMoreTokens()) {
			...
			if (mechs == defaultAuthenticationMechanisms) {
				...
				boolean disabled = PropUtil.getBooleanSessionProperty(session, dprop, !a.enabled());
				if (disabled) {
					if (logger.isLoggable(Level.FINE))
	...
	private abstract class Authenticator {
		...
		/**
		 * Start the authentication handshake by issuing the AUTH command. Delegate to
		 * the doAuth method to do the mechanism-specific part of the handshake.
		 */
		boolean authenticate(String host, String authzid, String user, String passwd) throws MessagingException {
			...
			try {
				...
				if (ir != null)
					resp = simpleCommand("AUTH " + mech + " " + (ir.length() == 0 ? "=" : ir));
				else
		...
		/**
		 * Provide the initial response to use in the AUTH command, or null if not
		 * supported. Subclasses that support the initial response capability will
		...
		abstract void doAuth(String host, String authzid, String user, String passwd)
				throws MessagingException, IOException;
	}
	...
	private class LoginAuthenticator extends Authenticator {
		...
		void doAuth(String host, String authzid, String user, String passwd) throws MessagingException, IOException {
			// send username
			resp = simpleCommand(BASE64EncoderStream.encode(user.getBytes(StandardCharsets.UTF_8)));
			if (resp == 334) {
				// send passwd
				resp = simpleCommand(BASE64EncoderStream.encode(passwd.getBytes(StandardCharsets.UTF_8)));
			}
		}
	}
	...
	private class NtlmAuthenticator extends Authenticator {
		...
		String getInitialResponse(String host, String authzid, String user, String passwd)
			...
			flags = PropUtil.getIntProperty(session.getProperties(), "mail." + name + ".auth.ntlm.flags", 0);
	...
	private boolean sasllogin(String[] allowed, String realm, String authzid, String u, String p)
		...
		if (saslAuthenticator == null) {
			try {
				...
				Constructor<?> c = sac.getConstructor(new Class<?>[] { SMTPTransport.class, String.class,
						Properties.class, MailLogger.class, String.class });
				saslAuthenticator = (SaslAuthenticator) c
						.newInstance(new Object[] { this, name, session.getProperties(), logger, serviceHost });
			} catch (Exception ex) {
	 ...
	 * <code>mail.smtp.sendpartial</code> property is not set, sending is aborted.
	 ...
	 *
	 * If some of the <code>addresses</code> fail the SMTP check, and the
	 * <code>mail.smtp.sendpartial</code> property is set to true, the message is
	...
	public synchronized void sendMessage(Message message, Address[] addresses)
		...
		try {
			...
			if (chunkSize > 0 && supportsExtension("CHUNKING")) {
				/*
				 * Use BDAT to send the data in chunks. Note that even though the BDAT command
				 * is able to send messages that contain binary data, we can't use it to do that
				 * because a) we still need to canonicalize the line terminators for text data,
				 * which we can't tell apart from the message content, and b) the message
				 * content is encoded before we even know that we can use BDAT.
				 */
	...
	public synchronized boolean isConnected() {
		...
		try {
			...
			/*
			 * NOOP should return 250 on success, however, SIMS 3.2 returns 200, so we work
			 * around it.
			 *
			 * Hotmail didn't used to implement the NOOP command at all so assume any kind
			 * of response means we're still connected. That is, any response except 421,
			 * which means the server is shutting down the connection.
			 *
			 * Some versions of Exchange return 451 instead of 421 when timing out a
			 * connection.
			 *
			 * Argh!
			 *
			 * If mail.smtp.noop.strict is set to false, be tolerant of servers that return
			 * the wrong response code for success.
			 */
	...
	/**
	 * Notify all TransportListeners. Keep track of whether notification has been
	 * done so as to only notify once per send.
	 *
	 * @since JavaMail 1.4.2
	 */
	@Override
	...
	protected void mailFrom() throws MessagingException {
		...
		if (supportsExtension("AUTH")) {
			...
			if (submitter != null) {
				try {
					String s = xtext(submitter, allowutf8 && supportsExtension("SMTPUTF8"));
					cmd += " AUTH=" + s;
		...
		try {
			...
			case 501:
				...
				try {
					ex.setNextException(
							new SMTPSenderFailedException(new InternetAddress(from), cmd, retCode, ex.getMessage()));
	 ...
	 * lists that are reported in the thrown exception: invalid addr: 550, 501, 503,
	 * 551, 553 valid addr: 552 (quota), 450, 451, 452 (quota), 421 (srvr abort)
	 */
	...
	protected void startTLS() throws MessagingException {
		...
		try {
			serverSocket = SocketFetcher.startTLS(serverSocket, host, session.getProperties(), "mail." + name);
			initStreams();
		} catch (IOException ioex) {
			closeConnection();
	...
	private void openServer(String host, int port) throws MessagingException {
		...
		try {
			...
			if ((r = readServerResponse()) != 220) {
				...
				if (logger.isLoggable(Level.FINE))
					logger.fine(
							"could not connect to host \"" + host + "\", port: " + port + ", response: " + r + "\n");
				throw new MessagingException(
						"Could not connect to SMTP host: " + host + ", port: " + port + ", response: " + r);
			} else {
				if (logger.isLoggable(Level.FINE))
	...
	private void openServer() throws MessagingException {
		...
		try {
			...
			if ((r = readServerResponse()) != 220) {
				...
				if (logger.isLoggable(Level.FINE))
					logger.fine(
							"got bad greeting from host \"" + host + "\", port: " + port + ", response: " + r + "\n");
				throw new MessagingException(
						"Got bad greeting from SMTP host: " + host + ", port: " + port + ", response: " + r);
			} else {
				if (logger.isLoggable(Level.FINE))
	...
	/**
	 * Temporarily turn off protocol tracing, e.g., to prevent tracing the
	 * authentication sequence, including the password.
	 */
	...
	public String getExtensionParameter(String ext) {
		return extMap == null ? null : extMap.get(ext.toUpperCase(Locale.ENGLISH));
	}
	...
	/**
	 * Convert the String to either ASCII or UTF-8 bytes depending on allowutf8.
	 */
	...
	private class BDATOutputStream extends SMTPOutputStream {
		...
		/**
		 * Create a BDATOutputStream that wraps a ChunkedOutputStream of the given size
		 * and built on top of the specified underlying output stream.
		 *
	...
	/**
	 * An OutputStream that buffers data in chunks and uses the RFC 3030 BDAT SMTP
	 * command to send each chunk.
	 */
	private class ChunkedOutputStream extends OutputStream {
		...
		/**
		 * Create a ChunkedOutputStream built on top of the specified underlying output
		 * stream.
		 *
		 * @param out  the underlying output stream
		 * @param size the chunk size
