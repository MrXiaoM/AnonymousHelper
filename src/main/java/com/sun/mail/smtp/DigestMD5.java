...
public class DigestMD5 {
	...
	private Map<String, String> tokenize(String serverResponse) throws IOException {
		...
		String key = null;
		int ttype;
		StreamTokenizer tokens = new StreamTokenizer(new InputStreamReader(
				new BASE64DecoderStream(new ByteArrayInputStream(bytes, 4, bytes.length - 4)), "iso-8859-1" // really
																											// ASCII?
		));
		...
		while ((ttype = tokens.nextToken()) != StreamTokenizer.TT_EOF) {
			...
			case '"':
				if (logger.isLoggable(Level.FINE))
					logger.fine("Received => " + key + "='" + tokens.sval + "'");
