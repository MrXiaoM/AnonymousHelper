...
public class POP3Store extends Store {
	...
	public POP3Store(Session session, URLName url, String name, boolean isSSL) {
		...
		this.name = name;
		logger = new MailLogger(this.getClass(), "DEBUG POP3", session);
	 ...
	 * and close it when it's done. If we then ask whether the Store's connected we
	 * want the answer to be true, as long as we can reconnect at that point. This
	 * means that we need to be able to reconnect the Store on demand.
	 */
	@Override
	...
	synchronized Protocol getPort(POP3Folder owner) throws IOException {
		...
		p = new Protocol(host, portNum, logger, session.getProperties(), "mail." + name, isSSL);
		...
		/*
		 * If we haven't explicitly disabled use of the TOP command, and the server has
		 * provided its capabilities, and the server doesn't support the TOP command,
		 ...
		 * have a port. In that case, the getPort call will come from Folder.open, but
		 * we need to keep track of the port in the Store so that a later call to
		 * Folder.isOpen, which calls Store.isConnected, will use the same port.
		 */
	...
	/**
	 * Return a Map of the capabilities the server provided, as per RFC 2449. If the
	 * server doesn't support RFC 2449, an emtpy Map is returned. The returned Map
	 * can not be modified. The key to the Map is the upper case capability name as
	 * a String. The value of the entry is the entire String capability line
