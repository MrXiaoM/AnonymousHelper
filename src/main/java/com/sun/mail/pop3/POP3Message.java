...
public class POP3Message extends MimeMessage implements ReadableMime {
	 ...
	 * to close it and notify of connection lost events.
	 */
	...
	public int getSize() throws MessagingException {
		try {
			...
			/*
			 * Use LIST to determine the entire message size and subtract out the header
			 * size (which may involve loading the headers, which may load the content as a
			 * side effect). If the content is loaded as a side effect of loading the
			 * headers, it will set the size.
			 *
			 * Make sure to call loadHeaders() outside of the synchronization block. There's
			 * a potential race condition here but synchronization will occur in
			 * loadHeaders() to make sure the headers are only loaded once, and again in the
			 * following block to only compute msgSize once.
			 */
			if (headers == null)
				loadHeaders();
	...
	/**
	 * Produce the raw bytes of the message. The data is fetched using the POP3 RETR
	 * command. If skipHeader is true, just the content is returned.
	 */
	private InputStream getRawStream(boolean skipHeader) throws MessagingException {
				...
				if (rawcontent == null) {
					...
					if (rawcontent == null) {
						...
						throw new MessageRemovedException(
								"can't retrieve message #" + msgnum + " in POP3Message.getContentStream"); // XXX - what
																											// else?
					}
		...
		/*
		 * We have a cached stream, but we need to return a fresh stream to read from
		 * the beginning and that can be safely closed.
		 */
	...
	/**
	 * Produce the raw bytes of the content. The data is fetched using the POP3 RETR
	 * command.
	 *
	 * @see #contentStream
	 */
	...
	protected synchronized InputStream getContentStream() throws MessagingException {
		...
		/*
		 * Keep a hard reference to the data if we're using a file cache or if the
		 * "mail.pop3.keepmessagecontent" prop is set.
		 */
		TempFile cache = folder.getFileCache();
	...
	/**
	 * Fetch the header of the message and the first <code>n</code> lines of the raw
	...
	/**
	 * Set the value for this header_name. Throws IllegalWriteException because POP3
	 * messages are read-only.
	 *
	 * @param name  header name
	 * @param value header value
	 * @see javax.mail.internet.MimeUtility
	 * @exception IllegalWriteException because the underlying implementation does
	...
	/**
	 * Remove all headers with this name. Throws IllegalWriteException because POP3
	 * messages are read-only.
	 *
	 * @exception IllegalWriteException because the underlying implementation does
	 *                                  not support modification
	 ...
	 * the property "mail.pop3.cachewriteto" is set to "true", and ignoreList is
	 * null, and the message hasn't already been cached as a side effect of other
	 * operations, the message content is cached before being written. Otherwise,
	 * the message is streamed directly to the output stream without being cached.
	 *
	...
	private void loadHeaders() throws MessagingException {
		...
		try {
			 ...
			 *
			 * Do we need to fetch the entire mesage content in order to load the headers as
			 * a side effect? Yes, there's a race condition here - multiple threads could
			 * decide that the content needs to be fetched. Fortunately, they'll all
			 * synchronize in the getContentStream method and the content will only be
			 * loaded once.
			 */
			if (fetchContent) {
				InputStream cs = null;
