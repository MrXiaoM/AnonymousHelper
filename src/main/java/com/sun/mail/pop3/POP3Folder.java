...
public class POP3Folder extends Folder {
	...
	protected POP3Folder(POP3Store store, String name) {
		...
		if (name.equalsIgnoreCase("INBOX"))
			exists = true;
		logger = new MailLogger(this.getClass(), "DEBUG POP3", store.getSession());
	}
	...
	/**
	 * Always returns <code>false</code>; the POP3 protocol doesn't support creating
	 * folders.
	 *
	 * @return false
	 */
	...
	public synchronized void close(boolean expunge) throws MessagingException {
		...
		try {
			 ...
			 * prevent such messages from being deleted before the client deletes them, you
			 * can set the mail.pop3.rsetbeforequit property to true. This causes us to
			 * issue a POP3 RSET command to clear all the "marked for deletion" flags. We
			 * can then explicitly delete messages as desired.
			 */
	...
	/**
	 * Always throws <code>MethodNotSupportedException</code> because the POP3
	 * protocol doesn't support expunging messages without closing the folder; call
	 * the {@link #close close} method with the <code>expunge</code> argument set to
	 ...
	 * contains <code>FetchProfile.Item.ENVELOPE</code>, the headers and size of all
	 * messages are fetched using the POP3 TOP and LIST commands.
	 */
	@Override
	public synchronized void fetch(Message[] msgs, FetchProfile fp) throws MessagingException {
		checkReadable();
		if (!doneUidl && store.supportsUidl && fp.contains(UIDFolder.FetchProfileItem.UID)) {
			/*
			 * Since the POP3 protocol only lets us fetch the UID for a single message or
			 * for all messages, we go ahead and fetch UIDs for all messages here, ignoring
			 * the msgs parameter. We could be more intelligent and base this decision on
			 * the number of messages fetched, or the percentage of the total number of
			 * messages fetched.
			 */
	...
	/**
	 * Return the size of this folder, as was returned by the POP3 STAT command when
	...
	/**
	 * Return the sizes of all messages in this folder, as returned by the POP3 LIST
	 * command. Each entry in the array corresponds to a message; entry <i>i</i>
	 * corresponds to message number <i>i+1</i>.
	 *
	...
	/*
	 * private void checkWritable() throws IllegalStateException { if (!opened ||
	 * mode != READ_WRITE) throw new
	 * IllegalStateException("Folder is not Writable"); }
	 */

	/**
	 * Centralize access to the Protocol object by POP3Message objects so that they
	 * will fail appropriately when the folder is closed.
	 */
