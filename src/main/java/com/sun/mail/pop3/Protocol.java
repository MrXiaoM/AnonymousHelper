...
class Protocol {
	...
	/**
	 * Get the value of a boolean property. Print out the value if logging is
	 * enabled.
	 */
	...
	/**
	 * Gets the APOP message digest. From RFC 1939:
	 *
	 * The 'digest' parameter is calculated by applying the MD5 algorithm [RFC1321]
	 * to a string consisting of the timestamp (including angle-brackets) followed
	 * by a shared secret. The 'digest' parameter itself is a 16-octet value which
	 * is sent in hexadecimal format, using lower-case ASCII characters.
	 *
	...
	synchronized Status stat() throws IOException {
		...
		/*
		 * Normally the STAT command shouldn't fail but apparently it does when
		 * accessing Hotmail too often, returning: -ERR login allowed only every 15
		 * minutes (Why it doesn't just fail the login, I don't know.) This is a serious
		 * failure that we don't want to hide from the user.
		 */
		if (!r.ok)
	...
	synchronized InputStream retr(int msg, int size) throws IOException {
		...
		if (batch) {
			...
			/*
			 * Many servers return a response to the RETR command of the form: +OK 832
			 * octets If we don't have a size guess already, try to parse the response for
			 * data in that format and use it if found. It's only a guess, but it might be a
			 * good guess.
			 */
	...
	/**
	 * Retrieve the specified message and stream the content to the specified
	 * OutputStream. Return true on success.
	 */
	synchronized boolean retr(int msg, OutputStream os) throws IOException {
		...
		try {
			while ((b = input.read()) >= 0) {
				...
				/*
				 * Keep writing unless we get an error while writing, which we defer until all
	...
	/**
	 * Start TLS using STLS command specified by RFC 2595. If already using SSL,
	 * this is a nop and the STLS command is not issued.
	 */
	...
	/**
	 * Get server capabilities using CAPA command specified by RFC 2449. Returns
	...
	private Response readResponse() throws IOException {
		...
		try {
			...
			/*
			 * If we get a timeout while using the socket, we have no idea what state the
			 * connection is in. The server could still be alive, but slow, and could still
			 * be sending data. The only safe way to recover is to drop the connection.
			 */
			try {
				socket.close();
			} catch (IOException cex) {
			...
			/*
			 * If we get an error while using the socket, we have no idea what state the
			 * connection is in. The server could still be alive, but slow, and could still
			 * be sending data. The only safe way to recover is to drop the connection.
			 */
			try {
	...
	/**
	 * Issue a POP3 command that expects a multi-line response. <code>size</code> is
	 * an estimate of the response size.
	 */
	...
	/**
	 * Read the response to a multiline command after the command response. The size
	 * parameter indicates the expected size of the response; the actual size can be
	 * different. Returns an InputStream to the response bytes.
	 */
