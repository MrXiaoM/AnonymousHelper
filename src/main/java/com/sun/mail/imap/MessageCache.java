...
/**
 * A cache of IMAPMessage objects along with the mapping from message number to
 * IMAP sequence number.
 *
 * All operations on this object are protected by the messageCacheLock in
 * IMAPFolder.
 */
public class MessageCache {
	/*
	 * The array of IMAPMessage objects. Elements of the array might be null if no
	 * one has asked for the message. The array expands as needed and might be
	 * larger than the number of messages in the folder. The "size" field indicates
	 * the number of entries that are valid.
	 */
	private IMAPMessage[] messages;

	/*
	 * A parallel array of sequence numbers for each message. If the array pointer
	 * is null, the sequence number of a message is just its message number. This is
	 * the common case, until a message is expunged.
	 */
	private int[] seqnums;

	/*
	 * The amount of the messages (and seqnum) array that is valid. Might be less
	...
	/**
	 * Grow the array by at least this much, to avoid constantly reallocating the
	 * array.
	 */
	...
	MessageCache(int size, boolean debug) {
		this.folder = null;
		logger = new MailLogger(this.getClass(), "messagecache", "DEBUG IMAP MC", debug, System.out);
		if (logger.isLoggable(Level.CONFIG))
	...
	/**
	 * Get the message object for the indicated message number. If the message
	 * object hasn't been created, create it.
	 *
	...
	/**
	 * Get the message object for the indicated sequence number. If the message
	 * object hasn't been created, create it. Return null if there's no message with
	...
	/**
	 * Remove all the expunged messages from the array, returning a list of removed
	 * message objects.
	 *
	...
	public IMAPMessage[] removeExpungedMessages() {
		...
		/*
		 * Walk through the array compressing it by copying higher numbered messages
		 * further down in the array, effectively removing expunged messages from the
		 * array. oldnum is the index we use to walk through the array. newnum is the
		 * index where we copy the next valid message. oldnum == newnum until we
		 * encounter an expunged message.
		 */
		int oldnum = 1;
		int newnum = 1;
		while (oldnum <= size) {
	...
	/**
	 * Remove expunged messages in msgs from the array, returning a list of removed
	 * message objects. All messages in msgs must be IMAPMessage objects from this
	 * folder.
	 *
	...
	public IMAPMessage[] removeExpungedMessages(Message[] msgs) {
		...
		/*
		 * Copy the message numbers of the expunged messages into a separate array and
		 * sort the array to make it easier to process later.
		 */
		 ...
		 * encounter an expunged message.
		 *
		 * Even though we know the message number of the first possibly expunged
		 * message, we still start scanning at message number 1 so that we can check
		 * whether there's any message whose sequence number is different than its
		 * message number. If there is, we can't throw away the seqnums array when we're
		 * done.
		 */
		int oldnum = 1;
		int newnum = 1;
		...
		while (oldnum <= size) {
			/*
			 * Are there still expunged messsages in msgs to consider, and is the message
			 * we're considering the next one in the list, and is it expunged?
			 */
			if (mnumi < mnum.length && oldnum == mnum[mnumi] && seqnumOf(oldnum) <= 0) {
				IMAPMessage m = getMessage(oldnum);
				mlist.add(m);
				/*
				 * Just in case there are duplicate entries in the msgs array, we keep advancing
				 * mnumi past any duplicates, but of course stop when we get to the end of the
				 * array.
				 */
	...
	/**
	 * Shrink the messages and seqnums arrays. newend is one past last valid
	 * element. oldend is one past the previous last valid element.
	 */
	...
	/**
	 * Add count messages to the cache. newSeqNum is the sequence number of the
	 * first message added.
	 *
	...
	/*
	 * Make sure the arrays are at least big enough to hold "newsize" messages.
	 */
