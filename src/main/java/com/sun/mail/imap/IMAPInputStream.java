...
public class IMAPInputStream extends InputStream {
	...
	private void fill() throws IOException {
		/*
		 * If we've read the last buffer, there's no more to read. If we know the total
		 * number of bytes available from this section, let's check if we have consumed
		 * that many bytes.
		 */
	 ...
	 * <p>
	 *
	 * Returns the total number of bytes read into the buffer, or <code>-1</code> if
	 * there is no more data.
	 * <p>
	 *
	 * Note that this method mimics the "weird !" semantics of BufferedInputStream
	 * in that the number of bytes actually returned may be less that the requested
	 * value. So callers of this routine should be aware of this and must check the
	 * return value to insure that they have obtained the requisite number of bytes.
	 */
	@Override
	 ...
	 * <p>
	 *
	 * Returns the total number of bytes read into the buffer, or <code>-1</code> is
	 * there is no more data.
	 * <p>
	 *
	 * Note that this method mimics the "weird !" semantics of BufferedInputStream
	...
	/**
	 * Normally the SEEN flag will have been set by now, but if not, force it to be
	 * set (as long as the folder isn't open read-only and we're not peeking). And
	 * of course, if there's no folder (e.g., a nested message) don't do anything.
	 */
