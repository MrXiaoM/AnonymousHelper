...
public class IMAPBodyPart extends MimeBodyPart implements ReadableMime {
	 ...
	 * IllegalWriteException. This will permit IMAPBodyParts to be inserted in newly
	 * crafted MimeMessages, especially when forwarding or replying to messages.
	 */
	@Override
		...
		synchronized (message.getMessageCacheLock()) {
			try {
				...
				if (p.isREV1() && (message.getFetchBlockSize() != -1))
					return new IMAPInputStream(message, sectionId, message.ignoreBodyStructureSize() ? -1 : bs.size,
							pk);
	...
	public InputStream getMimeStream() throws MessagingException {
		/*
		 * The IMAP protocol doesn't support returning the entire part content in one
		 * operation so we have to fake it by concatenating the header stream and the
		 * content stream.
		 */
