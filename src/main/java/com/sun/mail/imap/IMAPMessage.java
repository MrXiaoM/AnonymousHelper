 ...
 * IMAPMessage lock protects the message flags, sort of.
 *
 * XXX - I'm not convinced that all fields of IMAPMessage are properly protected
 * by locks.
 */
...
public class IMAPMessage extends MimeMessage implements ReadableMime {
	...
	/**
	 * A map of the extension FETCH items. In addition to saving the data in this
	 * map, an entry in this map indicates that we *have* the data, and so it
	 * doesn't need to be fetched again. The map is created only when needed, to
	 * avoid significantly increasing the effective size of an IMAPMessage object.
	 *
	 ...
	 * isHeaderLoaded() and getHeaderLoaded() to keep track of those headers we've
	 ...
	 * FolderClosedException, if the protocol connection is not available.
	 *
	...
	/**
	 * Return the UID for this message. Returns -1 if not known; use
	 * UIDFolder.getUID() in this case.
	 *
	 * @return the UID
	...
	/**
	 * Return the modification sequence number (MODSEQ) for this message. Returns -1
	 * if not known.
	 *
	...
	public Address[] getFrom() throws MessagingException {
		...
		/*
		 * Per RFC 2822, the From header is required, and thus the IMAP spec also
		 * requires that it be present, but we know that in practice it is often
		 * missing. Some servers fill in the From field with the Sender field in this
		 * case, but at least Exchange 2007 does not. Use the same fallback strategy
		 * used by MimeMessage.
		 */
	...
	public Address[] getReplyTo() throws MessagingException {
		...
		loadEnvelope();
		/*
		 * The IMAP spec requires that the Reply-To field never be null, but at least
		 * Exchange 2007 fails to fill it in in some cases. Use the same fallback
		 * strategy used by MimeMessage.
		 */
	 ...
	 * message, header and body included. Note also that if the size of the message
	 * is greater than Integer.MAX_VALUE (2GB), this method returns
	 * Integer.MAX_VALUE.
	 */
	@Override
	 ...
	 *
	 * Generate this header from the BODYSTRUCTURE. Append parameters as well.
	 */
	@Override
	 ...
	 * MimeMessage. This method is ultimately used by the DataHandler to obtain the
			...
			try {
				...
				if (p.isREV1() && (getFetchBlockSize() != -1)) // IMAP4rev1
					return new IMAPInputStream(this, toSection("TEXT"),
							bs != null && !ignoreBodyStructureSize() ? bs.size : -1, pk);
	...
	public synchronized DataHandler getDataHandler() throws MessagingException {
		...
		if (dh == null && !bodyLoaded) {
			...
			/*
			 * Special-case Multipart and Nested content. All other cases are handled by the
			 * superclass.
			 */
			if (bs.isMulti())
	 ...
	 * content. This overrides the default value from the "mail.imap.peek" property.
	 *
	 ...
	 * Subsequent accesses of this information will cause it to be fetched from the
	 * server.
	 *
	 * @since JavaMail 1.3.3
	 */
	...
	/**
	 * This class implements the test to be done on each message in the folder. The
	 * test is to check whether the message has already cached all the items
	 * requested in the FetchProfile. If any item is missing, the test succeeds and
	 * breaks out.
	 */
	public static class FetchProfileCondition implements Utility.Condition {
		...
		/**
		 * Create a FetchProfileCondition to determine if we need to fetch any of the
		 * information specified in the FetchProfile.
		 *
		...
		/**
		 * Return true if we NEED to fetch the requested information for the specified
		 * message.
		 */
		@Override
		public boolean test(IMAPMessage m) {
			...
			if (needBodyStructure && m._getBodyStructure() == null && !m.bodyLoaded)
		...
		// Check for header items
			...
			if (!isHeader) {
				...
				if (headers == null || allHeaders)
					...
					/*
					 * This is really painful. A second fetch of the same headers (which might occur
					 * because a new header was added to the set requested) will return headers we
					 * already know about. In this case, only load the headers we haven't seen
					 * before to avoid adding duplicates of headers we already have.
					 *
					 * XXX - There's a race condition here if another thread is reading headers in
					 * the same message object, because InternetHeaders is not thread safe.
					 */
	...
	/**
	 * Apply the data in the extension FETCH items to this message. This method adds
	 * all the items to the items map. Subclasses may override this method to call
	 * super and then also copy the data to a more convenient form.
	 *
	 * ASSERT: Must hold the messageCacheLock.
	 *
	...
	/**
	 * Fetch an individual item for the current message. Note that
	 * handleExtensionFetchItems will have been called to store this item in the
	 * message before this method returns.
	 *
	...
	/**
	 * Return the data associated with the FetchItem. If the data hasn't been
	 * fetched, call the fetchItem method to fetch it. Returns null if there is no
	...
	/*
	 * Convert the given FETCH item identifier to the approriate section-string for
	 * this message.
	 */
	...
	/***********************************************************
	 * accessor routines to make available certain private/protected fields to other
