 ...
 * <p>
 *
 * The {@link #getQuota getQuota} and {@link #setQuota setQuota} methods support
 * the IMAP QUOTA extension. Refer to
 * <A HREF="http://www.ietf.org/rfc/rfc2087.txt">RFC 2087</A> for more
 * information.
 * <p>
 *
 ...
 * support the IMAP ACL extension. Refer to
 * <A HREF="http://www.ietf.org/rfc/rfc2086.txt">RFC 2086</A> for more
 * information.
 * <p>
 *
 * The {@link #getSortedMessages getSortedMessages} methods support the IMAP
 * SORT extension. Refer to <A HREF="http://www.ietf.org/rfc/rfc5256.txt">RFC
 * 5256</A> for more information.
 * <p>
 *
 * The {@link #open(int,com.sun.mail.imap.ResyncData) open(int,ResyncData)}
 * method and {@link com.sun.mail.imap.ResyncData ResyncData} class supports the
 * IMAP CONDSTORE and QRESYNC extensions. Refer to
 * <A HREF="http://www.ietf.org/rfc/rfc4551.txt">RFC 4551</A> and
 * <A HREF="http://www.ietf.org/rfc/rfc5162.txt">RFC 5162</A> for more
 * information.
 * <p>
 *
 * The {@link #doCommand doCommand} method and {@link IMAPFolder.ProtocolCommand
 * IMAPFolder.ProtocolCommand} interface support use of arbitrary IMAP protocol
 * commands.
 * <p>
 *
 ...
 * acquiring the folder lock. <p>
 *
 * When a folder is opened, it creates a messageCache (a Vector) of empty
 * IMAPMessage objects. Each Message has a messageNumber - which is its index
 * into the messageCache, and a sequenceNumber - which is its IMAP
 * sequence-number. All operations on a Message which involve communication with
 * the server, use the message's sequenceNumber. <p>
 *
 * The most important thing to note here is that the server can send unsolicited
 * EXPUNGE notifications as part of the responses for "most" commands. Refer RFC
 * 3501, sections 5.3 & 5.5 for gory details. Also, the server sends these
 * notifications AFTER the message has been expunged. And once a message is
 * expunged, the sequence-numbers of those messages after the expunged one are
 * renumbered. This essentially means that the mapping between *any* Message and
 * its sequence-number can change in the period when a IMAP command is issued
 * and its responses are processed. Hence we impose a strict locking model as
 * follows: <p>
 *
 * We define one mutex per folder - this is just a Java Object (named
 * messageCacheLock). Any time a command is to be issued to the IMAP server
 * (i.e., anytime the corresponding IMAPProtocol method is invoked), follow the
 * below style:
 * 
 * synchronized (messageCacheLock) { // ACQUIRE LOCK issue command ()
 * 
 * // The response processing is typically done within // the handleResponse()
 * callback. A few commands (Fetch, // Expunge) return *all* responses and hence
 * their // processing is done here itself. Now, as part of the // processing
 * unsolicited EXPUNGE responses, we renumber // the necessary sequence-numbers.
 * Thus the renumbering // happens within this critical-region, surrounded by //
 * locks. process responses () } // RELEASE LOCK
 *
 * This technique is used both by methods in IMAPFolder and by methods in
 * IMAPMessage and other classes that operate on data in the folder. Note that
 * holding the messageCacheLock has the side effect of preventing the folder
 * from being closed, and thus ensuring that the folder's protocol object is
 * still valid. The protocol object should only be accessed while holding the
 * messageCacheLock (except for calls to IMAPProtocol.isREV1(), which don't need
 * to be protected because it doesn't access the server).
 * 
 * Note that interactions with the Store's protocol connection do not have to be
 * protected as above, since the Store's protocol is never in a "meaningful"
 * SELECT-ed state.
 */
...
public class IMAPFolder extends Folder implements UIDFolder, ResponseHandler {
	 ...
	 * (a non 7bit character) to indicate that we havent yet determined what the
	 * separator character is. We use '\u0000' (NUL) to indicate that no separator
	 * character exists, i.e., a flat hierarchy
	 */
	 ...
	 *
	 * If reallyClosed is false, then a FolderClosedException is generated when a
	 * method is invoked on any Messaging object owned by this folder. If
	 * reallyClosed is true, then the IllegalStateException runtime exception is
	 * thrown.
	 */
	 ...
	 * above). While executing the IDLE command we can't hold either of these locks
	 * or it would prevent other threads from entering Folder methods even far
	 * enough to check whether an IDLE command is in progress. We need to check
	 * before issuing another command so that we can abort the IDLE command.
	 *
	 * The idleState field is protected by the messageCacheLock. The RUNNING state
	 * is the normal state and means no IDLE command is in progress. The IDLE state
	 * means we've issued an IDLE command and are reading responses. The ABORTING
	 * state means we've sent the DONE continuation command and are waiting for the
	 * thread running the IDLE command to break out of its read loop.
	 *
	 * When an IDLE command is in progress, the thread calling the idle method will
	 * be reading from the IMAP connection while holding neither the folder lock nor
	 * the messageCacheLock. It's obviously critical that no other thread try to
	 * send a command or read from the connection while in this state. However,
	 * other threads can send the DONE continuation command that will cause the
	 * server to break out of the IDLE loop and send the ending tag response to the
	 * IDLE command. The thread in the idle method that's reading the responses from
	 * the IDLE command will see this ending response and complete the idle method,
	 * setting the idleState field back to RUNNING, and notifying any threads
	 * waiting to use the connection.
	 *
	 * All uses of the IMAP connection (IMAPProtocol object) must be done while
	 * holding the messageCacheLock and must be preceeded by a check to make sure an
	 * IDLE command is not running, and abort the IDLE command if necessary. While
	 * waiting for the IDLE command to complete, these other threads will give up
	 * the messageCacheLock, but might still be holding the folder lock. This check
	 * is done by the getProtocol() method, resulting in a typical usage pattern of:
	 *
	 * synchronized (messageCacheLock) { IMAPProtocol p = getProtocol(); // may
	 * block waiting for IDLE // ... use protocol }
	 */
	...
	public static class FetchProfileItem extends FetchProfile.Item {
		 ...
		 * indicates that the headers for messages in the specified range are desired to
		 * be prefetched.
		 * <p>
		 * 
		 * An example of how a client uses this is below: <blockquote>
		 * 
		 * <pre>
		 *
		 ...
		 * <code>FetchProfile</code> during a fetch request to a Folder. This item
		 * indicates that the sizes of the messages in the specified range are desired
		 * to be prefetched.
		 * <p>
		 *
		 ...
		 * <code>FetchProfile</code> during a fetch request to a Folder. This item
		 * indicates that the entire messages (headers and body, including all
		 * "attachments") in the specified range are desired to be prefetched. Note that
		 * the entire message content is cached in memory while the Folder is open. The
		 * cached message will be parsed locally to return header information and
		 * message content.
		 * <p>
		 * 
		 * An example of how a client uses this is below: <blockquote>
		 ...
		 * <code>FetchProfile</code> during a fetch request to a Folder. This item
		 * indicates that the IMAP INTERNALDATE values (received date) of the messages
		 * in the specified range are desired to be prefetched.
		 * <p>
		 * 
		 * An example of how a client uses this is below: <blockquote>
		 * 
	 ...
	 * @param separator   the default separator character for this folder's
	 *                    namespace
	...
	protected IMAPFolder(String fullName, char separator, IMAPStore store, Boolean isNamespace) {
		...
		logger = new MailLogger(this.getClass(), "DEBUG IMAP", store.getSession());
		connectionPoolLogger = store.getConnectionPoolLogger();

		/*
		 * Work around apparent bug in Exchange. Exchange will return a name of
		 * "Public Folders/" from LIST "%".
		 *
		 * If name has one separator, and it's at the end, assume this is a namespace
		 * name and treat it accordingly. Usually this will happen as a result of the
		 * list method, but this also allows getFolder to work with namespace names.
		 */
	 ...
	 * attempt to validate it with the server again. Note that this can result in a
	 * possible loss of sync with the server. ASSERT: Must be called with this
	 * folder's synchronization lock held.
	 */
	 ...
	 * in this folder. If the message number is out of range, we ping the server to
	 * obtain any pending new message notifications from the server.
	 */
	...
	public synchronized String getName() {
		/*
		 * Return the last component of this Folder's full name. Folder components are
		 * delimited by the separator character.
		 */
		if (name == null) {
			try {
	...
	private synchronized Folder[] doList(final String pattern, final boolean subscribed) throws MessagingException {
		...
		/*
		 * The UW based IMAP4 servers (e.g. SIMS2.0) include current folder (terminated
		 * with the separator), when the LIST pattern is '%' or '*'. i.e, <LIST ""
		 * mail/%> returns "mail/" as the first LIST response.
		 *
		 * Doesn't make sense to include the current folder in this case, so we filter
		 * it out. Note that I'm assuming that the offending response is the *first*
	...
	public synchronized List<MailEvent> open(int mode, ResyncData rd) throws MessagingException {
		...
		synchronized (messageCacheLock) { // Acquire messageCacheLock
			...
			/*
			 * Add response handler right away so we get any alerts or notifications that
			 * occur during the SELECT or EXAMINE. Have to be sure to remove it if we fail
			 * to open the folder.
			 */
			...
			try {
				/*
				 * Enable QRESYNC or CONDSTORE if needed and not enabled. QRESYNC implies
				 * CONDSTORE, but servers that support QRESYNC are not required to support just
				 * CONDSTORE per RFC 5162.
				 */
				if (rd != null) {
					if (rd == ResyncData.CONDSTORE) {
						if (!protocol.isEnabled("CONDSTORE") && !protocol.isEnabled("QRESYNC")) {
				...
				/*
				 * Handle SELECT or EXAMINE failure. Try to figure out why the operation failed
				 * so we can report a more reasonable exception.
				 *
			...
			if (mi.mode != mode) {
				if (mode == READ_WRITE && mi.mode == READ_ONLY && ((IMAPStore) store).allowReadOnlySelect()) {
					; // all ok, allow it
			...
			if (mi.responses != null) {
				...
				for (IMAPResponse ir : mi.responses) {
					if (ir.keyEquals("VANISHED")) {
						...
						if (msg != null)
							openEvents.add(new MessageChangedEvent(this, MessageChangedEvent.FLAGS_CHANGED, msg));
					}
				}
			}
		} // Release lock
	...
	/**
	 * Return the IMAP FETCH items to request in order to load all the "envelope"
	 * data. Subclasses can override this method to fetch more data when
	 * FetchProfile.Item.ENVELOPE is requested.
	 *
	 ...
	 * Subclasses of IMAPFolder may override this method to create a subclass of
	 * IMAPMessage.
	 *
	...
	private void close(boolean expunge, boolean force) throws MessagingException {
		...
		synchronized (messageCacheLock) {
			/*
			 * If we already know we're closed, this is illegal. Can't use checkOpened()
			 * because if we were forcibly closed asynchronously we just want to complete
			 * the closing here.
			 */
	...
	/*
	 * Get results of STATUS command for this folder, checking cache first. ASSERT:
	 * Must be called with this folder's synchronization lock held. ASSERT: The
	 * folder must be closed.
	 */
	...
	public synchronized Message[] getMessages() throws MessagingException {
		/*
		 * Need to override Folder method to throw FolderClosedException instead of
		 * IllegalStateException if not really closed.
		 */
		checkOpened();
	...
	/**
	 * Append the given messages into this folder. Return array of AppendUID objects
	 * containing UIDs of these messages in the destination folder. Each element of
	 * the returned array corresponds to an element of the <code>msgs</code> array.
	 * A null element means the server didn't return UID information for the
	 * appended message.
	 * <p>
	 *
	 ...
	 * must be open. Each element of the returned array corresponds to an element of
	 * the <code>msgs</code> array. A null element means the server didn't return
	 * UID information for the appended message.
	 * <p>
	 *
	 * Depends on the APPENDUID response code defined by the UIDPLUS extension -
	 * <A HREF="http://www.ietf.org/rfc/rfc4315.txt">RFC 4315</A>.
	...
	/**
	 * Copy the specified messages from this folder, to the specified destination.
	 * Return array of AppendUID objects containing UIDs of these messages in the
	 * destination folder. Each element of the returned array corresponds to an
	 * element of the <code>msgs</code> array. A null element means the server
	 * didn't return UID information for the copied message.
	 * <p>
	 *
	 * Depends on the COPYUID response code defined by the UIDPLUS extension -
	 * <A HREF="http://www.ietf.org/rfc/rfc4315.txt">RFC 4315</A>.
	 *
	 ...
	 * destination folder. Each element of the returned array corresponds to an
	 * element of the <code>msgs</code> array. A null element means the server
	 * didn't return UID information for the moved message.
	 * <p>
	 *
	 * Depends on the MOVE extension
	 * (<A HREF="http://www.ietf.org/rfc/rfc6851.txt">RFC 6851</A>) and the COPYUID
	 * response code defined by the UIDPLUS extension
	 * (<A HREF="http://www.ietf.org/rfc/rfc4315.txt">RFC 4315</A>).
	 *
	 * @param msgs   the messages to move
	 * @param folder the folder to move the messages to
	...
	/**
	 * Copy or move the specified messages from this folder, to the specified
	 * destination.
	 *
	 * @since JavaMail 1.5.4
	 */
	...
	/**
	 * Copy or move the specified messages from this folder, to the specified
	 * destination. Return array of AppendUID objects containing UIDs of these
	 * messages in the destination folder. Each element of the returned array
	 * corresponds to an element of the <code>msgs</code> array. A null element
	 * means the server didn't return UID information for the copied message.
	 * <p>
	 *
	 * Depends on the COPYUID response code defined by the UIDPLUS extension -
	 * <A HREF="http://www.ietf.org/rfc/rfc4315.txt">RFC 4315</A>. Move depends on
	 * the MOVE extension - <A HREF="http://www.ietf.org/rfc/rfc6851.txt">RFC
	 * 6851</A>.
	 *
	 * @param msgs   the messages to copy
	 * @param folder the folder to copy the messages to
	...
	private synchronized AppendUID[] copymoveUIDMessages(Message[] msgs, Folder folder, boolean move)
		...
		synchronized (messageCacheLock) {
			try {
				...
				/*
				 * Correlate source UIDs with destination UIDs. This won't be time or space
				 * efficient if there's a lot of messages.
				 *
				 * In order to make sense of the returned UIDs, we need the UIDs for every one
				 * of the original messages. We fetch them above, to make sure we have them.
				 * This is critical for MOVE since after the MOVE the messages are
				 * gone/expunged.
				 *
				 * Assume the common case is that the messages are in order by UID. Map the
				 * returned source UIDs to their corresponding Message objects. Step through the
				 * msgs array looking for the Message object in the returned source message
				 * list. Most commonly the source message (UID) for the Nth original message
				 * will be in the Nth position in the returned source message (UID) list. Thus,
				 * the destination UID is in the Nth position in the returned destination UID
				 * list. But if the source message isn't where expected, we have to search the
				 * entire source message list, starting from where we expect it and wrapping
				 * around until we've searched it all. (Gmail will often return the lists in an
				 * unexpected order.)
				 *
				 * A possible optimization: If the number of UIDs returned is the same as the
				 * number of messages being copied/moved, we could sort the source messages by
				 * message number, sort the source and destination parallel arrays by source
				 * UID, and the resulting message and destination UID arrays will correspond.
				 *
				 * If the returned UID array size is different, some message was expunged while
				 * we were trying to copy/move it. This should be rare but would mean falling
	...
	/**
	 * Search whole folder for messages matching the given term. If the property
	 * <code>mail.imap.throwsearchexception</code> is true, and the search term is
	 * too complex for the IMAP protocol, SearchException is thrown. Otherwise, if
	 * the search term is too complex, <code>super.search</code> is called to do the
	 * search on the client.
	 *
	 * @param term the search term
	 * @return the messages that match
	 * @exception SearchException    if mail.imap.throwsearchexception is true and
	 *                               the search is too complex for the IMAP protocol
	 ...
	 * matching messages. Returns an empty array if no matching messages are found.
	 */
	@Override
	...
	/**
	 * Sort the messages in the folder according to the sort criteria. The messages
	 * are returned in the sorted order, but the order of the messages in the folder
	 * is not changed.
	 * <p>
	 *
	 ...
	 * Sort the messages in the folder according to the sort criteria. The messages
	 * are returned in the sorted order, but the order of the messages in the folder
	 * is not changed. Only messages matching the search criteria are considered.
	 * <p>
	 *
	 * Depends on the SORT extension -
	 * <A HREF="http://www.ietf.org/rfc/rfc5256.txt">RFC 5256</A>.
	...
	/*
	 * Override Folder method to keep track of whether we have any message count
	 * listeners. Normally we won't have any, so we can avoid creating message
	 * objects to pass to the notify method. It's too hard to keep track of when all
	 * listeners are removed, and that's a rare case, so we don't try.
	 */
	@Override
	 ...
	 * appended to this folder. If the folder is closed, the STATUS command is used
	 * to retrieve this value. If the folder is open, the value returned from the
	 * SELECT or EXAMINE command is returned. Note that messages may have been
	 * appended to the folder while it was open and thus this value may be out of
	 * date.
	 * <p>
	 *
	 * Servers implementing RFC2060 likely won't return this value when a folder is
	 * opened. Servers implementing RFC3501 should return this value when a folder
	 * is opened.
	 * <p>
	 *
	 ...
	 * <p>
	 *
	 * <code>uids.length()</code> elements are returned. If any UID in the array is
	 * invalid, a <code>null</code> entry is returned for that element.
	 */
	@Override
	 ...
	 * (<A HREF="http://www.ietf.org/rfc/rfc4315.txt">RFC 4315</A>) may indicate
	 * that this folder does not support persistent UIDs; that is, UIDVALIDITY will
	 * be different each time the folder is opened. Only valid when the folder is
	 * open.
	 *
	...
	/**
	 * Get the messages that have been changed since the given MODSEQ value. Also,
	 * prefetch the flags for the messages.
	 * <p>
	 *
	...
	/**
	 * Get the quotas for the quotaroot associated with this folder. Note that many
	 * folders may have the same quotaroot. Quotas are controlled on the basis of a
	 * quotaroot, not (necessarily) a folder. The relationship between folders and
	 * quotaroots depends on the IMAP server. Some servers might implement a single
	 * quotaroot for all folders owned by a user. Other servers might implement a
	 * separate quotaroot for each folder. A single folder can even have multiple
	 * quotaroots, perhaps controlling quotas for different resources.
	 *
	 * @return array of Quota objects for the quotaroots associated with this folder
	 * @exception MessagingException if the server doesn't support the QUOTA
	 *                               extension
	 */
	 ...
	 * @exception MessagingException if the server doesn't support the ACL extension
	 */
	...
	/**
	 * Remove any access control list entry for the given identifier from the access
	 * control list for this folder.
	 *
	...
	/**
	 * Add the rights specified in the ACL to the entry for the identifier specified
	 * in the ACL. If an entry for the identifier doesn't already exist, add one.
	 *
	 ...
	 * <A HREF="http://www.ietf.org/rfc/rfc2177.txt">RFC 2177</A>), if supported by
	 * the server, to enter idle mode so that the server can send unsolicited
	 * notifications of new messages arriving, etc. without the need for the client
	 * to constantly poll the server. Use an appropriate listener to be notified of
	 * new messages or other events. When another thread (e.g., the listener thread)
	 * needs to issue an IMAP comand for this folder, the idle mode will be
	 * terminated and this method will return. Typically the caller will invoke this
	 * method in a loop.
	 * <p>
	 *
	 * The mail.imap.minidletime property enforces a minimum delay before returning
	 * from this method, to ensure that other threads have a chance to issue
	 * commands before the caller invokes this method again. The default delay is 10
	 * milliseconds.
	 *
	 * @exception MessagingException    if the server doesn't support the IDLE
	 *                                  extension
	 * @exception IllegalStateException if the folder isn't open
	 *
	...
	/**
	 * Like {@link #idle}, but if <code>once</code> is true, abort the IDLE command
	 * after the first notification, to allow the caller to process any notification
	 * synchronously.
	 *
	...
	public void idle(boolean once) throws MessagingException {
		synchronized (this) {
			/*
			 * We can't support the idle method if we're using SocketChannels because
			 * SocketChannels don't allow simultaneous read and write. If we're blocked in a
			 * read waiting for IDLE responses, we can't send the DONE message to abort the
			 * IDLE. Sigh. XXX - We could do select here too, like IdleManager, instead of
			 * blocking in read, but that's more complicated.
			 */
		...
		/*
		 * We gave up the folder lock so that other threads can get into the folder far
		 * enough to see that we're in IDLE and abort the IDLE.
		 *
		 * Now we read responses from the IDLE command, especially including unsolicited
		 * notifications from the server. We don't hold the messageCacheLock while
		 * reading because it protects the idleState and other threads need to be able
		 * to examine the state.
		 *
		 * The messageCacheLock is held in handleIdle while processing the responses so
		 * that we can update the number of messages in the folder (for example).
		 */
		for (;;) {
			if (!handleIdle(once))
				break;
		...
		/*
		 * Enforce a minimum delay to give time to threads processing the responses that
		 * came in while we were idle.
		 */
	...
	/**
	 * Start the IDLE command and put this folder into the IDLE state. IDLE
	 * processing is done later in handleIdle(), e.g., called from the IdleManager.
	 *
		...
		synchronized (this) {
			...
			Boolean started = (Boolean) doOptionalCommand("IDLE not supported", new ProtocolCommand() {
				...
				public Object doCommand(IMAPProtocol p) throws ProtocolException {
					...
					if (idleState == IDLE && im != null && im == idleManager)
	...
	/**
	 * Read a response from the server while we're in the IDLE state. We hold the
	 * messageCacheLock while processing the responses so that we can update the
	 * number of messages in the folder (for example).
	 *
	 * @param once only do one notification?
	 * @return true if we should look for more IDLE responses, false if IDLE is done
			...
			try {
				synchronized (messageCacheLock) {
					if (r.isBYE() && r.isSynthetic() && idleState == IDLE) {
						/*
						 * If it was a timeout and no bytes were transferred we ignore it and go back
						 * and read again. If the I/O was otherwise interrupted, and no bytes were
						 * transferred, we take it as a request to abort the IDLE.
						 */
					...
					boolean done = true;
					try {
						if (protocol == null || !protocol.processIdleResponse(r))
							return false; // done
						done = false;
					} finally {
						if (done) {
		...
		while (idleState != RUNNING) {
			...
			try {
				...
				if (logger.isLoggable(Level.FINEST))
					logger.finest("waitIfIdle: wait to be not idle: " + Thread.currentThread());
				messageCacheLock.wait();
				if (logger.isLoggable(Level.FINEST))
					logger.finest("waitIfIdle: wait done, idleState " + idleState + ": " + Thread.currentThread());
			} catch (InterruptedException ex) {
				// restore the interrupted state, which callers might depend on
				Thread.currentThread().interrupt();
	...
	/*
	 * Send the DONE command that aborts the IDLE and wait for the response; used by
	 * IdleManager.
	 */
	void idleAbortWait() {
		synchronized (messageCacheLock) {
	...
	/**
	 * Send the IMAP ID command (if supported by the server) and return the result
	 * from the server. The ID command identfies the client to the server and
	 * returns information about the server to the client. See
	 * <A HREF="http://www.ietf.org/rfc/rfc2971.txt">RFC 2971</A>. The returned Map
	 * is unmodifiable.
	 *
	 * @param clientParams a Map of keys and values identifying the client
	 * @return a Map of keys and values identifying the server
	 * @exception MessagingException if the server doesn't support the ID extension
	 * @since JavaMail 1.5.1
	 */
	...
	/**
	 * Use the IMAP STATUS command to get the indicated item. The STATUS item may be
	 * a standard item such as "RECENT" or "UNSEEN", or may be a server-specific
	 * item. The folder must be closed. If the item is not found, or the folder is
	 * open, -1 is returned.
	 *
	 ...
	 * 'folder' lock or 'message' lock (i.e., any synchronized methods on IMAPFolder
	 * or IMAPMessage) since that will result in violating the locking hierarchy.
	 */
	@Override
	...
	/**
	 * Process a FETCH response. The only unsolicited FETCH response that makes
	 * sense to me (for now) is FLAGS updates, which might include UID and MODSEQ
	 * information. Ignore any other junk.
	 */
	private Message processFetchResponse(FetchResponse fr) {
		...
		if (msg != null) { // should always be true
			...
			if (modseq != null && msg._getModSeq() != modseq.modseq) {
				 ...
				 *
				 * if (modseq.modseq > highestmodseq) highestmodseq = modseq.modseq;
				 */
				notify = true;
			}
	 ...
	 *
	 * When acquiring a store protocol object, it is important to use the following
	 * steps:
	 *
	 * <blockquote>
	 * 
	...
	protected synchronized Object doProtocolCommand(ProtocolCommand cmd) throws ProtocolException {
		/*
		 * Check whether we have a protocol object, not whether we're opened, to allow
		 * use of the exsting protocol object in the open method before the state is
		 * changed to "opened".
		 */
		if (protocol != null) {
			synchronized (messageCacheLock) {
	...
	/**
	 * Release the store protocol object. If we borrowed a protocol object from the
	 * connection pool, give it back. If we used our own protocol object, nothing to
	 * do.
	 *
	 * ASSERT: Must be called with this folder's synchronization lock held.
	 *
	 ...
	 * noop over the store's connection.
	 *
	 * ASSERT: This method must be called only when holding the messageCacheLock
	 *
	...
	/**
	 * Get the message object for the given sequence number. If none found, null is
	 * returned.
	 *
	 * ASSERT: This method must be called only when holding the messageCacheLock
	 *
	...
	protected IMAPMessage getMessageBySeqNumber(int seqnum) {
		if (seqnum > messageCache.size()) {
			...
			if (logger.isLoggable(Level.FINE))
				logger.fine("ignoring message number " + seqnum + " outside range " + messageCache.size());
			return null;
		}
