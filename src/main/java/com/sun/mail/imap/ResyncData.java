 ...
 * (<A HREF="http://www.ietf.org/rfc/rfc5162.txt">RFC 5162</A>). An instance of
 * <CODE>ResyncData</CODE> is supplied to the
 * {@link com.sun.mail.imap.IMAPFolder#open(int,com.sun.mail.imap.ResyncData)
 * IMAPFolder open} method. The CONDSTORE <CODE>ResyncData</CODE> instance is
 * used to enable the CONDSTORE extension
 * (<A HREF="http://www.ietf.org/rfc/rfc4551.txt">RFC 4551</A>). A
 * <CODE>ResyncData</CODE> instance with uidvalidity and modseq values is used
 * to enable the QRESYNC extension.
 *
...
public class ResyncData {
	...
	/**
	 * Used to report on changes since the specified modseq. If the UIDVALIDITY of
	 * the folder has changed, no message changes will be reported. The application
	 * must check the UIDVALIDITY of the folder after open to make sure it's the
	 * expected folder.
	 *
	...
	/**
	 * Used to limit the reported message changes to those with UIDs in the
	 * specified range.
	 *
	 * @param uidvalidity the UIDVALIDITY
	 * @param modseq      the MODSEQ
	...
	/*
	 * Package private. IMAPProtocol gets this data indirectly using
