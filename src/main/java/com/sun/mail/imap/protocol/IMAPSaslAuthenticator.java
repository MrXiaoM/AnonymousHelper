...
public class IMAPSaslAuthenticator implements SaslAuthenticator {
	...
	/*
	 * This is a hack to initialize the OAUTH SASL provider just before, and only
	 * if, we might need it. This avoids the need for the user to initialize it
	 * explicitly, or manually configure the security providers file.
	 */
	static {
		try {
	...
	@Override
	public boolean authenticate(String[] mechs, final String realm, final String authzid, final String u,
		...
		synchronized (pr) { // authenticate method should be synchronized
			...
			boolean isXGWTRUSTEDAPP = sc.getMechanismName().equals("XGWTRUSTEDAPP")
					&& PropUtil.getBooleanProperty(props, "mail." + name + ".sasl.xgwtrustedapphack.enable", true);
			...
			/*
			 * If we're using the Novell Groupwise XGWTRUSTEDAPP mechanism to run as a
			 * specified authorization ID, we have to issue a LOGIN command to select the
			 * user we want to operate as.
			 */
