...
public class FetchResponse extends IMAPResponse {
	/*
	 * Regular Items are saved in the items array. Extension items (items handled by
	 * subclasses that extend the IMAP provider) are saved in the extensionItems
	 * map, indexed by the FETCH item name. The map is only created when needed.
	 *
	 * XXX - Should consider unifying the handling of regular items and extension
	 * items.
	 */
	private Item[] items;
	...
	/**
	 * Return the first fetch response item of the given class for the given message
	 * number.
	 *
	...
	/**
	 * Return all fetch response items of the given class for the given message
	 * number.
	 *
	 * @param r     the responses
	 * @param msgno the message number
	...
	/**
	 * Return a map of the extension items found in this fetch response. The map is
	 * indexed by extension item name. Callers should not modify the map.
	 *
	...
	private void parse() throws ParsingException {
		...
		do {
			...
			if (i != null)
				...
				throw new ParsingException("error in FETCH parsing, unrecognized item at index " + index
						+ ", starts with \"" + next20() + "\"");
	...
	/**
	 * Parse the item at the current position in the buffer, skipping over the item
	 * if successful. Otherwise, return null and leave the buffer position
	 * unmodified.
	 */
	...
	private boolean parseExtensionItem() throws ParsingException {
		...
		for (int i = 0; i < fitems.length; i++) {
			if (match(fitems[i].getName())) {
				...
				extensionItems.put(fitems[i].getName(), fitems[i].parseItem(this));
				return true;
			}
		}
		return false;
	...
	/**
	 * Does the current buffer match the given item name? itemName is the name of
	 * the IMAP item to compare against. NOTE that itemName *must* be all uppercase.
	 * If the match is successful, the buffer pointer (index) is incremented past
	 * the matched item.
	 */
