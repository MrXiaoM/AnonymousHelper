	...
	public BODYSTRUCTURE(FetchResponse r) throws ParsingException {
		...
		if (r.peekByte() == '(') { // multipart
			...
			do {
				...
				/*
				 * Even though the IMAP spec says there can't be any spaces between parts, some
				 * servers erroneously put a space in here. In the spirit of
			...
			if (b == '(') {
				...
				throw new ParsingException("BODYSTRUCTURE parse error: " + type + "/" + subtype + ": "
			...
			/*
			 * Domino will fail to return the body structure of nested messages. Fake it by
			 * providing an empty message. Could probably do better with more work...
			 */
			/*
			 * XXX - this prevents the exception, but without the exception the application
			 * has no way to know the data from the message is missing.
			 *
			 * if (parseDebug) System.out.println("DEBUG IMAP: empty body, fake it");
			 * r.readByte(); type = "text"; subtype = "plain"; lines = 0; size = 0;
			 */
			...
			/*
			 * XXX - Work around bug in Exchange 2010 that returns unquoted string.
			 */
			...
			if (type.equalsIgnoreCase("text")) {
			...
			} else if (type.equalsIgnoreCase("message") && subtype.equalsIgnoreCase("rfc822")) {
				// Nested message
				...
				if (Character.isDigit((char) bn)) // number
					throw new ParsingException("BODYSTRUCTURE parse error: server erroneously "
							+ "included ``lines'' element with type " + type + "/" + subtype);
			}
