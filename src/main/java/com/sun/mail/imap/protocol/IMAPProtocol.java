...
public class IMAPProtocol extends Protocol {
	...
	public IMAPProtocol(String name, String host, int port, Properties props, boolean isSSL, MailLogger logger)
		...
		try {
			...
			referralException = PropUtil.getBooleanProperty(props, prefix + ".referralexception", false);
			...
			/*
			 * If we get here because an exception was thrown, we need to disconnect to
			 * avoid leaving a connected socket that no one will be able to use because this
			 * object was never completely constructed.
			 */
			if (!connected)
				disconnect();
		}
	...
	/**
	 * Return an array of FetchItem objects describing the FETCH items supported by
	 * this protocol. Subclasses may override this method to combine their
	 * FetchItems with the FetchItems returned by the superclass.
	 *
	...
	/**
	 * If the response contains a CAPABILITY response code, extract it and save the
	 * capabilities.
	 *
		...
		while ((s = r.readAtom()) != null) {
			if (s.length() == 0) {
				if (r.peekByte() == (byte) ']')
					break;
				/*
				 * Probably found something here that's not an atom. Rather than loop forever or
				 * fail completely, we'll try to skip this bogus capability. This is known to
				 * happen with: Netscape Messaging Server 4.03 (built Apr 27 1999) that returns:
				 * * CAPABILITY * CAPABILITY IMAP4 IMAP4rev1 ... The "*" in the middle of the
				 * capability list causes us to loop forever here.
				 */
				r.skipToken();
			} else {
	 ...
	 *
	 * This method just makes the Protocol.disconnect() method public.
	 */
	@Override
	...
	public synchronized void authlogin(String u, String p) throws ProtocolException {
		...
		try {
			 ...
			 *
			 * Note that the encoded bytes should be sent "as-is" to the server, *not* as
			 ...
			 *
			 * Finally, format the line in a buffer so it can be sent as a single packet, to
		...
		/*
		 * Dispatch untagged responses. NOTE: in our current upper level IMAP classes,
		 * we add the responseHandler to the Protocol object only *after* the connection
		 * has been authenticated. So, for now, the below code really ends up being just
		 * a no-op.
		 */
	...
	/**
	 * The AUTHENTICATE command with AUTH=PLAIN authentication scheme. This is based
	 * heavly on the {@link #authlogin} method.
	 *
	...
	/**
	 * The AUTHENTICATE command with AUTH=NTLM authentication scheme. This is based
	 * heavly on the {@link #authlogin} method.
	 *
	 * @param authzid the authorization id
	 * @param u       the username
	...
	/**
	 * The AUTHENTICATE command with AUTH=XOAUTH2 authentication scheme. This is
	...
	public void sasllogin(String[] allowed, String realm, String authzid, String u, String p) throws ProtocolException {
		boolean useCanonicalHostName = PropUtil.getBooleanProperty(props, "mail." + name + ".sasl.usecanonicalhostname",
				false);
		String serviceHost;
		...
		if (saslAuthenticator == null) {
			try {
				...
				Constructor<?> c = sac.getConstructor(new Class<?>[] { IMAPProtocol.class, String.class,
						Properties.class, MailLogger.class, String.class });
				saslAuthenticator = (SaslAuthenticator) c
						.newInstance(new Object[] { this, name, props, logger, serviceHost });
			} catch (Exception ex) {
	...
	/**
	 * Handle the result response for a LOGIN or AUTHENTICATE command. Look for IMAP
	 * login REFERRAL.
	 *
	 * @param r the response
	 * @exception ProtocolException for protocol failures
	...
	/**
	 * Get the user name used with the PROXYAUTH command. Returns null if PROXYAUTH
	 * was not used.
	 *
	 ...
	 * @param guid the GUID
	 * @exception ProtocolException for protocol failures
	 * @deprecated As of JavaMail 1.5.1, replaced by {@link #id(Map)
	...
	/**
	 * Encode a mailbox name appropriately depending on whether or not the server
	 * supports UTF-8, and add the encoded name to the Argument.
	 *
	 * @since JavaMail 1.6.0
	 */
	...
	/**
	 * Execute the specified LIST-like command (e.g., "LIST" or "LSUB"), using the
	...
	public AppendUID appenduid(String mbox, Flags f, Date d, Literal data, boolean uid) throws ProtocolException {
		...
		if (f != null) { // set Flags in appended message
			 ...
			 * checking/processing of the bytes in it. If I use writeString(), the flag_list
			 * will end up being quoted since it contains "illegal" characters. So I am
			 * depending on implementation knowledge that writeAtom() does not do any
			 * checking/processing - it just writes out the bytes. What we really need is a
			 * writeFoo() that just dumps out its argument.
			 */
	...
	/**
	 * If the response contains an APPENDUID response code, extract it and return an
	 * AppendUID object with the information.
	 */
	...
	protected BODY fetchBody(int msgno, String section, int start, int size, boolean peek, ByteArray ba)
		...
		if (section == null)
			section = "";
		String body = (peek ? "BODY.PEEK[" : "BODY[") + section + "]<" + String.valueOf(start) + "."
				+ String.valueOf(size) + ">";
		return fetchSectionBody(msgno, section, body);
	}

	/**
	 * Fetch the given body section of the given message, using the body string
	 * "body".
	 *
	 * @param msgno   the message number
	 * @param section the body section
	...
	protected BODY fetchSectionBody(int msgno, String section, String body) throws ProtocolException {
		...
		if (response.isOK()) {
			...
			if (logger.isLoggable(Level.FINEST))
				logger.finest("got " + bl.size() + " BODY responses for section " + section);
	...
	/**
	 * Return a buffer to read a response into. The buffer is provided by fetchBody
	...
	/**
	 * Get the sequence number for the given UID. Nothing is returned; the FETCH UID
	 * response must be handled by the reponse handler, along with any possible
	 * EXPUNGE responses, to ensure that the UID is matched with the correct
	 * sequence number.
	 *
	 * @param uid the UID
	 * @exception ProtocolException for protocol failures
	...
	/**
	 * Get the sequence numbers for UIDs ranging from start till end. Since the
	 * range may be large and sparse, an array of the UIDs actually found is
	 * returned. The caller must map these to messages after the FETCH UID responses
	 * have been handled by the reponse handler, along with any possible EXPUNGE
	 * responses, to ensure that the UIDs are matched with the correct sequence
	 * numbers.
	 *
	...
	public long[] fetchSequenceNumbers(long start, long end) throws ProtocolException {
		Response[] r = fetch(String.valueOf(start) + ":" + (end == UIDFolder.LASTUID ? "*" : String.valueOf(end)),
				"UID", true);
	...
	/**
	 * Get the sequence numbers for UIDs specified in the array. Nothing is
	 * returned. The caller must map the UIDs to messages after the FETCH UID
	 * responses have been handled by the reponse handler, along with any possible
	 * EXPUNGE responses, to ensure that the UIDs are matched with the correct
	 * sequence numbers.
	 *
	 * @param uids the UIDs
	 * @exception ProtocolException for protocol failures
	...
	/**
	 * Get the sequence numbers for messages changed since the given modseq and with
	 * UIDs ranging from start till end. Also, prefetch the flags for the returned
	 * messages.
	 *
	 * @param start  first UID
	 * @param end    last UID
	...
	public int[] uidfetchChangedSince(long start, long end, long modseq) throws ProtocolException {
		String msgSequence = String.valueOf(start) + ":" + (end == UIDFolder.LASTUID ? "*" : String.valueOf(end));
		Response[] r = command("UID FETCH " + msgSequence + " (FLAGS) (CHANGEDSINCE " + String.valueOf(modseq) + ")",
				null);
	...
	/**
	 * If the response contains a COPYUID response code, extract it and return a
	 * CopyUID object with the information.
	 *
	...
	/**
	 * Issue the given search criterion on the specified message sets. Returns array
	 * of matching sequence numbers. An empty array is returned if no matches are
	 * found.
	 *
	 ...
	 * array of matching sequence numbers. An empty array is returned if no matches
	 * are found.
	 *
	 * @param term SearchTerm
	 * @return array of matching sequence numbers.
	...
	private int[] search(String msgSequence, SearchTerm term) throws ProtocolException, SearchException {
		 ...
		 * CHARSET <charset> ... The charsets we try to use are UTF-8 and the locale's
		 * default charset. If the server supports UTF-8, great, always use it. Else we
		 * try to use the default charset.
		 */
		...
		for (int i = 0; i < searchCharsets.length; i++) {
			...
			try {
				...
				/*
				 * Server returned NO. For now, I'll just assume that this indicates that this
				 * charset is unsupported. We can check the BADCHARSET response code once that's
				 * spec'd into the IMAP RFC ..
				 */
	...
	/**
	 * Get the SearchSequence object. The SearchSequence object instance is saved in
	 * the searchSequence field. Subclasses of IMAPProtocol may override this method
	 * to return a subclass of SearchSequence, in order to add support for
	...
	/**
	 * Sort messages in the folder according to the specified sort criteria. If the
	 * search term is not null, limit the sort to only the messages that match the
	 * search term. Returns an array of sorted sequence numbers. An empty array is
	 ...
	 *
	 * Returns an array of Quota objects, representing the quotas for this mailbox
	 * and, indirectly, the quotaroots for this mailbox.
	 *
	 * @param mbox the mailbox
	 ...
	 * GETQUOTA Command.
	 *
	 * Returns an array of Quota objects, representing the quotas for this
	 * quotaroot.
	 *
	...
	public void setQuota(Quota quota) throws ProtocolException {
		...
		/*
		 * Quota quota = null; List<Quota> v = new ArrayList<Quota>();
		 * 
		 * // Grab all QUOTA responses if (response.isOK()) { // command succesful for
		 * (int i = 0, len = r.length; i < len; i++) { if (!(r[i] instanceof
		 * IMAPResponse)) continue;
		 * 
		 * IMAPResponse ir = (IMAPResponse)r[i]; if (ir.keyEquals("QUOTA")) { quota =
		 * parseQuota(ir); v.add(quota); r[i] = null; } } }
		 */
	 ...
	 * <p>
	 *
	 * If the server supports the IDLE command extension, the IDLE command is issued
	 * and this method blocks until a response has been received. Once the first
	 * response has been received, the IDLE command is terminated and all responses
	 * are collected and handled and this method returns.
	 * <p>
	 *
	 ...
	 * The response is read with no locks held so that when the read blocks waiting
	 * for the response from the server it's not holding locks that would prevent
	...
	/**
	 * Process a response returned by readIdleResponse(). This method will be called
	 * with appropriate locks held so that the processing of the response is safe.
	 *
	 * @param r the response
	 ...
	 * readIdleResponse() and processIdleResponse() will see and terminate the IDLE
	 * state.
	 *
	 * @since JavaMail 1.4.1
	 */
