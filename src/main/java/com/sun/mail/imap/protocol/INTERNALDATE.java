...
public class INTERNALDATE implements Item {
	...
	/*
	 * Used to parse dates only. The parse method is thread safe so we only need to
	 * create a single object for use by all instances. We depend on the fact that
	 * the MailDateFormat class will parse dates in INTERNALDATE format as well as
	...
	public static String format(Date d) {
		/*
		 * SimpleDateFormat objects aren't thread safe, so rather than create a separate
		 * such object for each request, we create one object and synchronize its use
		 * here so that only one thread is using it at a time. This trades off some
		 * potential concurrency for speed in the common case.
		 *
		 * This method is only used when formatting the date in a message that's being
