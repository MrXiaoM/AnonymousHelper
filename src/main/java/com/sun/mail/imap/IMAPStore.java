 ...
 * setQuota} methods support the IMAP QUOTA extension. Refer to
 * <A HREF="http://www.ietf.org/rfc/rfc2087.txt">RFC 2087</A> for more
 * information.
 * <p>
 *
 * The {@link #id id} method supports the IMAP ID extension; see
 * <A HREF="http://www.ietf.org/rfc/rfc2971.txt">RFC 2971</A>. The fields
 * ID_NAME, ID_VERSION, etc. represent the suggested field names in RFC 2971
 * section 3.3 and may be used as keys in the Map containing client values or
 * server values.
 * <p>
 *
 ...
 *
 * A connected IMAPStore maintains a pool of IMAP protocol objects for use in
 * communicating with the IMAP server. The IMAPStore will create the initial
 * AUTHENTICATED connection and seed the pool with this connection. As folders
 * are opened and new IMAP protocol objects are needed, the IMAPStore will
 * provide them from the connection pool, or create them if none are available.
 * When a folder is closed, its IMAP protocol object is returned to the
 * connection pool if the pool is not over capacity. The pool size can be
 * configured by setting the mail.imap.connectionpoolsize property. <p>
 *
 * Note that all connections in the connection pool have their response handler
 * set to be the Store. When the connection is removed from the pool for use by
 * a folder, the response handler is removed and then set to either the Folder
 * or to the special nonStoreResponseHandler, depending on how the connection is
 * being used. This is probably excessive. Better would be for the Protocol
 * object to support only a single response handler, which would be set before
 * the connection is used and cleared when the connection is in the pool and
 * can't be used. <p>
 *
 * A mechanism is provided for timing out idle connection pool IMAP protocol
 * objects. Timed out connections are closed and removed (pruned) from the
 * connection pool. The time out interval can be configured via the
 * mail.imap.connectionpooltimeout property. <p>
 *
 * The connected IMAPStore object may or may not maintain a separate IMAP
 * protocol object that provides the store a dedicated connection to the IMAP
 * server. This is provided mainly for compatibility with previous
 * implementations of JavaMail and is determined by the value of the
 * mail.imap.separatestoreconnection property. <p>
 *
 * An IMAPStore object provides closed IMAPFolder objects thru its list() and
 * listSubscribed() methods. A closed IMAPFolder object acquires an IMAP
 * protocol object from the store to communicate with the server. When the
 * folder is opened, it gets its own protocol object and thus its own, separate
 * connection to the server. The store maintains references to all 'open'
 * folders. When a folder is/gets closed, the store removes it from its list.
 * When the store is/gets closed, it closes all open folders in its list, thus
 * cleaning up all open connections to the server. <p>
 *
 * A mutex is used to control access to the connection pool resources. Any time
 * any of these resources need to be accessed, the following convention should
 * be followed:
 *
 * synchronized (pool) { // ACQUIRE LOCK // access connection pool resources }
 * // RELEASE LOCK <p>
 *
 * The locking relationship between the store and folders is that the store lock
 * must be acquired before a folder lock. This is currently only applicable in
 * the store's cleanup method. It's important that the connection pool lock is
 * not held when calling into folder objects. The locking hierarchy is that a
 * folder lock must be acquired before any connection pool operations are
 * performed. You never need to hold all three locks, but if you hold more than
 * one this is the order you have to acquire them in. <p>
 *
 * That is: Store > Folder, Folder > pool, Store > pool <p>
 *
 * The IMAPStore implements the ResponseHandler interface and listens to BYE or
 * untagged OK-notification events from the server as a result of Store
 * operations. IMAPFolder forwards notifications that result from Folder
 * operations using the store connection; the IMAPStore ResponseHandler is not
 * used directly in this case. <p>
 */
...
public class IMAPStore extends Store implements QuotaAwareStore, ResponseHandler {
	...
	/*
	 * This field is set in the Store's response handler if we see a BYE response.
	 * The releaseStore method checks this field and if set it cleans up the Store.
	 * Field is volatile because there's no lock we consistently hold while
	 * manipulating it.
	 *
	 * Because volatile doesn't really work before JDK 1.5, use a lock to protect
	 * these two fields.
	 */
	...
	static class ConnectionPool {
		 ...
		 * IMAP command we hold the store's lock. While executing the IDLE command we
		 * can't hold the lock or it would prevent other threads from entering Store
		 * methods even far enough to check whether an IDLE command is in progress. We
		 * need to check before issuing another command so that we can abort the IDLE
		 * command.
		 *
		 * The idleState field is protected by the store's lock. The RUNNING state is
		 * the normal state and means no IDLE command is in progress. The IDLE state
		 ...
		 * be reading from the IMAP connection while not holding the store's lock. It's
		 * obviously critical that no other thread try to send a command or read from
		 * the connection while in this state. However, other threads can send the DONE
		 * continuation command that will cause the server to break out of the IDLE loop
		 * and send the ending tag response to the IDLE command. The thread in the idle
		 * method that's reading the responses from the IDLE command will see this
		 * ending response and complete the idle method, setting the idleState field
		 * back to RUNNING, and notifying any threads waiting to use the connection.
		 *
		 * All uses of the IMAP connection (IMAPProtocol object) must be preceeded by a
		 * check to make sure an IDLE command is not running, and abort the IDLE command
		 * if necessary. This check is made while holding the connection pool lock.
		 * While waiting for the IDLE command to complete, these other threads will give
		 * up the connection pool lock. This check is done by the getStoreProtocol()
		 * method.
		 */
		...
		ConnectionPool(String name, MailLogger plogger, Session session) {
			...
			boolean debug = PropUtil.getBooleanSessionProperty(session, "mail." + name + ".connectionpool.debug",
					false);
			...
			// store connection
			separateStoreConnection = PropUtil.getBooleanSessionProperty(session,
	...
	/**
	 * A special response handler for connections that are being used to perform
	 * operations on behalf of an object other than the Store. It DOESN'T cause the
	 * Store to be cleaned up if a BYE is seen. The BYE may be real or synthetic and
	 * in either case just indicates that the connection is dead.
	 */
	...
	protected IMAPStore(Session session, URLName url, String name, boolean isSSL) {
		...
		logger = new MailLogger(this.getClass(), "DEBUG " + name.toUpperCase(Locale.ENGLISH), session);
		...
		if (!partialFetch) {
			...
			blksize = PropUtil.getIntSessionProperty(session, "mail." + name + ".fetchsize", 1024 * 16);
			if (logger.isLoggable(Level.CONFIG))
		...
		forcePasswordRefresh = PropUtil.getBooleanSessionProperty(session, "mail." + name + ".forcepasswordrefresh",
				false);
		if (forcePasswordRefresh)
			logger.config("enable forcePasswordRefresh");

		// check if enableimapevents is enabled
		enableResponseEvents = PropUtil.getBooleanSessionProperty(session, "mail." + name + ".enableresponseevents",
				false);
		...
		throwSearchException = PropUtil.getBooleanSessionProperty(session, "mail." + name + ".throwsearchexception",
				false);
		...
		if (s != null) {
			...
			try {
				...
				Class<?>[] c = { String.class, char.class, IMAPStore.class, Boolean.class };
	 ...
	 * does not require a password, an empty string or other suitable dummy password
	 * should be used.
	 */
	@Override
	protected synchronized boolean protocolConnect(String host, int pport, String user, String password)
		...
		if (host == null || password == null || user == null) {
			if (logger.isLoggable(Level.FINE))
				logger.fine("protocolConnect returning false" + ", host=" + host + ", user=" + traceUser(user)
		...
		try {
			...
			if (poolEmpty) {
				...
				if (logger.isLoggable(Level.FINE))
					logger.fine("protocolConnect login" + ", host=" + host + ", user=" + traceUser(user) + ", password="
	...
	/**
	 * Create an IMAPProtocol object connected to the host and port. Subclasses of
	 * IMAPStore may override this method to return a subclass of IMAPProtocol that
	 * supports product-specific extensions.
	 *
	...
	private void login(IMAPProtocol p, String u, String pw) throws ProtocolException {
		...
		/*
		 * If marker is still there, capabilities haven't been refreshed, refresh them
		 * now.
		 */
	...
	private void authenticate(IMAPProtocol p, String authzid, String user, String password) throws ProtocolException {
		...
		/*
		 * Loop through the list of mechanisms supplied by the user (or defaulted) and
		 * try each in turn. If the server supports the mechanism and we have an
		 * authenticator for the mechanism, and it hasn't been disabled, use it.
		 */
		...
		while (st.hasMoreTokens()) {
			...
			if (mechs == defaultAuthenticationMechanisms) {
				...
				boolean disabled = PropUtil.getBooleanSessionProperty(session, dprop, m.equals("XOAUTH2"));
				if (disabled) {
					if (logger.isLoggable(Level.FINE))
						logger.fine("mechanism " + m + " disabled by property: " + dprop);
					continue;
				}
			}
	...
	/**
	 * This method is called after the connection is made and TLS is started (if
	 * needed), but before any authentication is attempted. Subclasses can override
	 * this method to issue commands that are needed in the "not authenticated"
	 * state. Note that if the connection is pre-authenticated, this method won't be
	 * called.
	 * <p>
	 *
	 ...
	 * Folder). This value is overridden by any call to the Store's connect method.
	 * <p>
	 *
	 * Some IMAP servers may provide an authentication ID that can be used for more
	 * efficient authentication for future connections. This authentication ID is
	 * provided in a server-specific manner not described here.
	 * <p>
	 *
	...
	IMAPProtocol getProtocol(IMAPFolder folder) throws MessagingException {
		...
		while (p == null) {
			...
			synchronized (pool) {
				...
				// create a new one.
				if (pool.authenticatedConnections.isEmpty() || (pool.authenticatedConnections.size() == 1
					...
					if (logger.isLoggable(Level.FINE))
						logger.fine("connection available -- size: " + pool.authenticatedConnections.size());
					...
					if (lastUsed > pool.serverTimeoutInterval) {
						try {
							/*
							 * Swap in a special response handler that will handle alerts, but won't cause
							 * the store to be closed and cleaned up if the connection is dead.
							 */
					...
					if (proxyAuthUser != null && !proxyAuthUser.equals(p.getProxyAuthUser())
	 ...
	 * steps:
	 *
	 * IMAPProtocol p = null; try { p = getStoreProtocol(); // perform the command }
	 * catch (ConnectionException cex) { throw new StoreClosedException(this,
	...
	/*
	 * Some authentication systems use one time passwords or tokens, so each
	 * authentication request requires a new password. This "kludge" allows a
	 * callback to application code to get a new password.
	 *
	...
	boolean isConnectionPoolFull() {
		...
		synchronized (pool) {
			if (pool.logger.isLoggable(Level.FINE))
				pool.logger.fine("connection pool current size: " + pool.authenticatedConnections.size()
		...
		synchronized (pool) {
			if (protocol != null) {
				...
				if (!isConnectionPoolFull()) {
					...
					if (logger.isLoggable(Level.FINE))
						logger.fine(
								"added an Authenticated connection -- size: " + pool.authenticatedConnections.size());
				} else {
	...
	private void releaseStoreProtocol(IMAPProtocol protocol) {
		...
		/*
		 * Read out the flag that says whether this connection failed before releasing
		 * the protocol object for others to use.
		 */
		boolean failed;
		...
		/*
		 * If the connection died while we were using it, clean up. It's critical that
		 * the store connection be freed and the connection pool not be locked while we
		 * do this.
		 */
	...
	private void timeoutConnections() {
		...
		synchronized (pool) {
			...
			if (System.currentTimeMillis() - pool.lastTimePruned > pool.pruningInterval
				...
				if (pool.logger.isLoggable(Level.FINE)) {
					pool.logger.fine(
							"checking for connections to prune: " + (System.currentTimeMillis() - pool.lastTimePruned));
					pool.logger.fine("clientTimeoutInterval: " + pool.clientTimeoutInterval);
				}
				...
				for (int index = pool.authenticatedConnections.size() - 1; index > 0; index--) {
					p = pool.authenticatedConnections.elementAt(index);
					if (pool.logger.isLoggable(Level.FINE))
						pool.logger.fine("protocol last used: " + (System.currentTimeMillis() - p.getTimestamp()));
	...
	/**
	 * Return true if the specified capability string is in the list of capabilities
	...
	/**
	 * Set the user name to be used with the PROXYAUTH command. The PROXYAUTH user
	 * name can also be set using the <code>mail.imap.proxyauth.user</code> property
	 * when this Store is created.
	 *
	...
	public synchronized boolean isConnected() {
		 ...
		 * obtained from the server (3) fail because the socket.write() to the server
		 * fails, in which case the iap.protocol() code converts the IOException into a
		 * BYE response.
		 *
		 * Thus, our BYE handler will take care of closing the Store in case our
	...
	public synchronized void close() throws MessagingException {
		...
		try {
			...
			/*
			 * Have to drop the lock before calling cleanup. Yes, there's a potential race
			 * here. The pool could become empty after we check, in which case we'll just
			 * waste time getting a new connection and closing it. Or, the pool could be
			 * empty now and not empty by the time we get into cleanup, but that's ok
			 * because cleanup will just close the connection.
			 */
			if (isEmpty) {
			 ...
			 * LOGOUT.
			 *
			 * Note that protocol.logout() closes the server socket connection, regardless
			 * of what happens ..
			 *
			 * Also note that protocol.logout() results in a BYE response (As per RFC 3501,
			 * BYE is a *required* response to LOGOUT). In fact, even if protocol.logout()
			 * fails with an IOException (if the server connection is dead),
			 * iap.Protocol.command() converts that exception into a BYE response. So, I
			 * depend on my BYE handler to set the flag that causes releaseStoreProtocol to
			 * do the Store cleanup.
			 */
			protocol.logout();
		} catch (ProtocolException pex) {
	...
	private synchronized void cleanup() {
		...
		/*
		 * If forceClose is true, some thread ran into an error that suggests the server
		 * might be dead, so we force the folders to close abruptly without waiting for
		 * the server. Used when the store connection times out, for example.
		 */
		boolean force;
		synchronized (connectionFailedLock) {
			force = forceClose;
	...
	/**
	 * Create an IMAPFolder object. If user supplied their own class, use it.
	...
	/**
	 * Create an IMAPFolder object. Call the newIMAPFolder method above with a null
	 * isNamespace.
	 *
	 * @param fullName  the full name of the folder
	 * @param separator the separator character for the folder hierarchy
	 ...
	 * of a quota root, not (necessarily) a folder. The relationship between folders
	 * and quota roots depends on the IMAP server. Some servers might implement a
	 * single quota root for all folders owned by a user. Other servers might
	...
	/**
	 * Set the quotas for the quota root specified in the quota argument. Typically
	 * this will be one of the quota roots obtained from the <code>getQuota</code>
	 ...
	 * notifications without the need for the client to constantly poll the server.
	 * Use a <code>ConnectionListener</code> to be notified of events. When another
	 * thread (e.g., the listener thread) needs to issue an IMAP comand for this
	 * Store, the idle mode will be terminated and this method will return.
	 * Typically the caller will invoke this method in a loop.
	 * <p>
	 *
	 ...
	 * response string. Note that most IMAP servers will not deliver any events when
	 * using the IDLE command on a connection with no mailbox selected (i.e., this
	 * method). In most cases you'll want to use the <code>idle</code> method on
	 * <code>IMAPFolder</code>.
	 * <p>
	 *
	 * NOTE: This capability is highly experimental and likely will change in future
	 * releases.
	 * <p>
	 *
	...
	public void idle() throws MessagingException {
		...
		try {
			...
			/*
			 * We gave up the pool lock so that other threads can get into the pool far
			 ...
			 * notifications from the server. We don't hold the pool lock while reading
			 * because it protects the idleState and other threads need to be able to
			 * examine the state.
			 *
	...
	private String tracePassword(String password) {
		return debugpassword ? password : (password == null ? "<null>" : "<non-null>");
	}
}
