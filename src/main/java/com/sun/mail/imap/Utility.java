...
public final class Utility {
	 ...
	 * ignored.
	 *
	 * ASSERT: Since this method uses and returns message sequence numbers, you
	 * should use this method only when holding the messageCacheLock.
	 *
	 ...
	 * array of messages, apply the given Condition on each message and generate
	 * sets of contiguous sequence-numbers for the successful messages. If a message
	...
	public static MessageSet[] toMessageSetSorted(Message[] msgs, Condition cond) {
		/*
		 * XXX - This is quick and dirty. A more efficient strategy would be to generate
		 * an array of message numbers by applying the condition (with zero indicating
		 * the message doesn't satisfy the condition), sort it, and then convert it to a
		 * MessageSet skipping all the zeroes.
		 */
		msgs = msgs.clone();
	...
	/**
	 * Make the ResyncData UIDSet available to IMAPProtocol, which is in a different
	 * package. Note that this class is not included in the public javadocs, thus
	 * "hiding" this method.
	 *
